...
| `world-project/Assets/LegaiaWorld/Shaders/LegaiaVertexColorAdditive.shader` | Unlit additive (`Blend One One`) for the exporter's `legaia_semi_abr1/2/3` materials - PSX additive prims (window light shafts, glows) read grey under plain alpha blend. Lifts its contribution to display space first: the PSX adds in gamma space, and Unity's Linear pipeline blending in linear reads dim over any non-black background. |
