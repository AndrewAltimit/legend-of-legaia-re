...
//
// - Ambient audio (AddAmbience): five long synthesized 2D beds (base
//   wind/surf, day, night, rain, storm gust) plus spatial emitters at the
...
//   rainLevel / windLevel. Any role can be replaced by a user-supplied
...
namespace LegaiaWorld
{
    ...
    public class LegaiaRealismOptions
    {
        ...
        // Synced-by-clock weather spells (rain, storm, wind, overcast) and
        // the shoreline fishing stations the NPC director sends villagers
        // to. Both are Udon layers: without the SDK they build inert.
    ...
    public static class LegaiaRealism
    {
        ...
        // --- Ambient audio --------------------------------------------------
        //
        // Five 2D beds (base wind/surf, day, night, rain, storm gust) plus
        // three groups of spatial emitters (shore waves and the windmill
        // always on, tree birds by day, wildlife by night), all owned by one
        // LegaiaAmbienceMixer on the `ambience` container. The mixer
        // crossfades day against night on LegaiaDayNight's published
        // dayFactor and takes `rainLevel` / `windLevel` from the weather
        // layer; see Udon/LegaiaAmbienceMixer.cs for that contract.
        //
        ...
        internal const string BED_RAIN = "bed_rain";
            ...
            LegaiaRealismOptions o, LegaiaSceneSettings settings)
        {
            ...
            var rainClip = Clip("rain", "amb_rain.wav", LegaiaAudioGen.RainBed, BED);
            ...
            // The first-generation 12/16 s beds: drop the objects AND their
            // assets, so a rebuilt world can never play them again.
            var stale = root.transform.Find(AMBIENCE);
            if (stale != null)
                Object.DestroyImmediate(stale.gameObject);
            foreach (string legacy in new[]
                     { "ambience_loop.wav", "ambience_day.wav", "ambience_night.wav" })
            ...
            var rainSrc = Bed2D(go.transform, BED_RAIN, rainClip, 0f);
            ...
            if (mixer != null)
            {
                ...
                LegaiaWorldBuilder.SetUdonField(mixer, "rainBed", rainSrc);
                ...
                LegaiaWorldBuilder.SetUdonField(mixer, "rainVolume", v * 1.5f);
