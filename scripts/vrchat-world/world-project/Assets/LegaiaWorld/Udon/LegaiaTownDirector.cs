    ...
    public class LegaiaTownDirector : UdonSharpBehaviour
    {
        ...
        void StartConversation()
        {
            ...
            for (int i = 0; i < brains.Length; i++)
            {
                ...
                if (!Routable(b, b.transform.position, centre))
                    continue;
                seen++;
                if (found < want)
                {
            ...
            float radius, int taken, bool checkRoute, int prefer)
        {
            ...
            if (wanted != null)
                best = wanted;
            if (best != null && checkRoute && !Routable(b, from, best.StandPosition()))
                return null;
            return best;
        }
        ...
        bool Routable(LegaiaNpcBrain b, Vector3 from, Vector3 to)
        {
            if (!routeCheck || routeBudget <= 0)
                return true;
            // The villager's own controller knows the bake's ledge links,
            // so a spot it could hop to is not refused as unreachable.
            if (b != null && b.loco != null)
            {
                routeBudget--;
                return b.loco.CanReachFrom(from, to);
            }
            if (navState == 0)
            {
