...
- Battle / story-flag sweeps: `audit_module_hp_stores.py` (stores to the battle-actor HP triple across the capture-class spell modules), `flag_helper_call_sweep.py` (every disc-wide `jal`/`j` into the story-flag SET/CLEAR/TEST helpers, with the `a0` operand classified constant vs computed).
