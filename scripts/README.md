...
| [`vrc-diorama/`](#vrc-diorama) | From-scratch MIDI transport for the VRChat live battle-diorama: register schema + codegen, the Lua encoder/sink riding the battle-state probe, and the UdonSharp decoder scaffold. No Sony bytes. |
| [`vrchat-world/`](vrchat-world/README.md) | Unity/VRChat world kit for `legaia-engine export-glb` output: the manifest-driven scene builder (world + colliders + NPC/prop placement + looping clips), an UdonSharp NPC-wander behaviour, and the end-to-end private-world guide. See [`docs/tooling/vrchat-world-export.md`](../docs/tooling/vrchat-world-export.md). No Sony bytes. |
...
- `check-wasm-target.sh` - `cargo check` for the wasm32 target, the only gate that compiles the browser host's `#[cfg(target_arch = "wasm32")]` code. Every other cargo gate builds for the host triple and cannot see it. **Hook tier**, spent when a staged file belongs to a crate carrying such a block; CI's own wasm build runs after merge, not on the PR. See [`docs/tooling/host-drift.md`](../docs/tooling/host-drift.md#the-browser-host-is-also-the-one-nothing-here-compiles).
...
- `slot_b_band.py` - the slot-B module band's own structure (frame partition, spawn records, the move-VM width walk that bounds the highest one, and each image's structural content end). Imported by `ci/disc-coverage.py`, `attribute-dump-extents.py` and `inherited_tail.py`, so the three answer identically; mirrors the authority `legaia_asset::slot_b_module`.
...
- Whole-TOC byte accounting: `byte-account-sweep.py` runs `asset account` over every PROT entry and rolls the residue up by entry class and by shape, so the worklist is ordered by the disc rather than by what anybody sampled. Emits a numbers-only CSV under `target/` (gitignored; the per-run `head` hex `--json` prints is disc bytes and is dropped). See [`docs/tooling/byte-accounting.md`](../docs/tooling/byte-accounting.md).
