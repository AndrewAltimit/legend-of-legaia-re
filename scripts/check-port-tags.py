...
Tag shapes recognised (parallel to scripts/port-catalog.py):
...
Usage:
    ...
    python3 scripts/check-port-tags.py --backfill-refs # write //! REF: blocks
"""
...
def parse_tags(text: str) -> tuple[set[str], set[str]]:
    ...
    equivalent for warning purposes; `scripts/port-catalog.py` only trusts
