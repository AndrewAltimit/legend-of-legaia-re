...
`scripts/ghidra-analysis/extract-mednafen-overlay.py`) for sites that build a PSX-GPU
...
USAGE
    scripts/ghidra-analysis/find-addprim-emitters.py overlay.bin --base 0x801C0000

The output groups hits per cluster (rounded to 0x100). Combine with
`scripts/ghidra-analysis/extract-mednafen-overlay.py` to widen the capture window before
running this hunter.
"""
...
# Cmd bytes worth flagging (shared sets from scripts/ghidra-analysis/gpu_packets.py).
