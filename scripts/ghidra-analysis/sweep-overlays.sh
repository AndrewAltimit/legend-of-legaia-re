...
#
# Usage:
#   scripts/ghidra-analysis/sweep-overlays.sh <spec-file>
#   scripts/ghidra-analysis/sweep-overlays.sh --state <path> --label <name> [--state ... --label ...]
#
...
for i in "${!STATES[@]}"; do
    ...
    if [[ "$NO_EXTRACT" -eq 1 ]]; then
        ...
        if ! run scripts/ghidra-analysis/extract-mednafen-overlay.py "$SAVE_EXPANDED" --out "$DUMP"; then
