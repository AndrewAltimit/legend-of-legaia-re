...
-- canonical spar-talk tile (rimelm_npc_press_tetsu: player at world (2762,1782) =
-- tile (21,13), one south of Tetsu, facing +Z). Approach from THIS side.
local TALK_C,TALK_R=21,13
-- field-control struct pointer + dialog state (deref *(SCENE_PTR)):
--   +0x62 = dialog-control byte (0x27/0x28/0x29 = 2/3/4-option picker),
--   +0x0C = option-picker cursor, +0x60 = interact flag. pager lines = 0x801F2740.
local SCENE_PTR=0x801C6EA4; local PAGER=0x801F2740
local SPAR_OPT=tonumber(env.getenv("LEGAIA_SPAR_OPT","0")) or 0  -- which picker row accepts
local OBSERVE=tonumber(env.getenv("LEGAIA_SPAR_OBSERVE","90")) or 90

local START_SAVE = env.getenv("LEGAIA_SSTATE", "")
local OUT_DIR    = env.getenv("LEGAIA_OUT_DIR", "captures/s5_spar")
...
local function pick_tetsu(order)
    -- aim for the canonical talk tile (21,13); fall back to whatever is closest.
    local best,bd=nil,1e9
    for _,t in ipairs(order) do local c,r=t[1],t[2]; local d=math.abs(c-TALK_C)+math.abs(r-TALK_R); if d<bd then bd=d;best={c,r} end end
    return best,bd
end
...
local st = {
    ...
    eng_since=nil, last_dlg=nil, last_cur=nil, picker_since=nil, picked=false,
}
...
bp.arm(FIELD_BP, "Exec", 4, "field_tick", function()
    ...
    if eng~=st.was_engaged then
        ...
        st.was_engaged=eng
        if not eng then st.eng_since=nil; st.last_dlg=nil; st.last_cur=nil; st.picker_since=nil; st.picked=false
        else st.eng_since=st.frame end
    end
    if eng then
        -- read + log the dialog state (deref the field-control struct)
        local sp=ru32(SCENE_PTR)
        local dlg = sp and ru8(sp+0x62) or nil
        local cur = sp and ru8(sp+0x0C) or nil
        local inter = sp and ru8(sp+0x60) or nil
        local pgr = ru8(PAGER)
        if dlg~=st.last_dlg or cur~=st.last_cur then
            log(string.format("[f%d] dlg62=%s cursor=%s interact=%s pager=%s", st.frame,
                dlg and string.format("0x%02X",dlg) or "nil", cur and string.format("0x%02X",cur) or "nil",
                inter and string.format("0x%02X",inter) or "nil", pgr and string.format("0x%02X",pgr) or "nil"))
            st.last_dlg=dlg; st.last_cur=cur
        end
        local is_picker = (dlg==0x27 or dlg==0x28 or dlg==0x29)
        if is_picker then
            -- a Yes/No (or multi) prompt: observe briefly, then drive the cursor
            -- to row SPAR_OPT and confirm with CROSS.
            if st.picker_since==nil then st.picker_since=st.frame; log(string.format("[f%d] OPTION PICKER dlg=0x%02X cursor=0x%02X (selecting row %d)", st.frame, dlg, cur or 0, SPAR_OPT)) end
            if st.frame-st.picker_since < OBSERVE then return end       -- let it settle / log
            local c = cur or 0
            if not st.picked then
                if c > SPAR_OPT then
                    if st.cross_state==1 and st.frame>=st.cross_t then pad.release(pad.BTN.UP); st.cross_state=0; st.cross_t=st.frame+8
                    elseif st.cross_state==0 and st.frame>=st.cross_t then pad.force(pad.BTN.UP); st.cross_state=1; st.cross_t=st.frame+3 end
                elseif c < SPAR_OPT then
                    if st.cross_state==1 and st.frame>=st.cross_t then pad.release(pad.BTN.DOWN); st.cross_state=0; st.cross_t=st.frame+8
                    elseif st.cross_state==0 and st.frame>=st.cross_t then pad.force(pad.BTN.DOWN); st.cross_state=1; st.cross_t=st.frame+3 end
                else
                    -- cursor on the accept row: confirm
                    if st.cross_state==1 and st.frame>=st.cross_t then pad.release(pad.BTN.CROSS); st.cross_state=0; st.cross_t=st.frame+10; st.picked=true; log(string.format("[f%d] confirmed row %d with CROSS", st.frame, SPAR_OPT))
                    elseif st.cross_state==0 and st.frame>=st.cross_t then pad.force(pad.BTN.CROSS); st.cross_state=1; st.cross_t=st.frame+3 end
                end
            else
                -- already confirmed once: keep advancing in case more text follows
                if st.cross_state==1 and st.frame>=st.cross_t then pad.release(pad.BTN.CROSS); st.cross_state=0; st.cross_t=st.frame+12
                elseif st.cross_state==0 and st.frame>=st.cross_t then pad.force(pad.BTN.CROSS); st.cross_state=1; st.cross_t=st.frame+3 end
            end
            return
        end
        -- plain text box: advance with a slow CROSS cadence (no option to mis-pick)
        if st.cross_state==1 and st.frame>=st.cross_t then pad.release(pad.BTN.CROSS); st.cross_state=0; st.cross_t=st.frame+12
        elseif st.cross_state==0 and st.frame>=st.cross_t then pad.force(pad.BTN.CROSS); st.cross_state=1; st.cross_t=st.frame+3 end
        return
    end
