-- autorun_w7c_kor5_tail.lua
--
-- The W6-C spoke walk (`autorun_w6c_spoke_walk.lua`: tile-poke route, flag
-- firehose, flag pokes) plus the two record-spawn instruments the kor5 tail
-- chain needs, and a flag-keyed checkpoint:
--
--   * `FUN_8003BDE0(x, z, record, gate)` - the partition-record dispatcher
--     every walk-on trigger and op `0x44` reaches; one row per call with
--     its four arguments and `ra`, so a record the route expects to spawn
--     either shows a call (and its C1/C2 verdict in the flag firehose) or
--     shows none (the trigger never reached the dispatcher).
--   * `FUN_801D5630(kind, x, z)` - the field overlay's per-frame trigger
--     lookup; logged once per distinct (kind, x, z) so a poked tile the
--     lookup never sees is visible.
--   * LEGAIA_CKPT_FLAG=<hex> - write `<CKPT_LABEL>.rawsstate` the first
--     field vsync that flag reads set (LEGAIA_CKPT_DELAY vsyncs later).
--
-- The kor5 chain, from the MAN partition-2 headers and the scene's `.PCH`
-- walk-on table: P2[3] (32,43) C1 {0x43A} sets 0x43A; P2[4] (32,41)
-- C1 {0x436} C2 {0x43A} sets 0x464; P2[5] (21,52) C1 {0x436} (also spawned
-- by P1[0]'s `44 2A` on entry when 0x464 is set) sets 0x436; P2[8] (32,86)
-- C1 {0x6C4} C2 {0x436} sets 0x6C4.
--
--
...
-- filtered, and `w7c_kor5_flags_first.csv` keeps the first hit per
...
local function shot(name)
    ...
    if ok and ss then
        ...
        PCSX.log("[w7c_kor5] screenshot failed: " .. tostring(ss))
    end
end
...
local CSV = probe.csv_open(probe.out_path("w7c_kor5_route.csv"),
    "vsync,scene,mode,px,pz,tile_x,tile_z,leg,tint_r,tint_g,tint_b")
local HITS = probe.csv_open(probe.out_path("w7c_kor5_hits.csv"),
    "vsync,site,a0,a1,a2,ra,scene,mode")
local LOGF = io.open(probe.out_path("w7c_kor5.log"), "w")
local FLAGS = probe.csv_open(probe.out_path("w7c_kor5_flags.csv"),
    "vsync,op,flag_hex,flag,ra,pc_offset,op_bytes,scene,mode")
local FIRST = probe.csv_open(probe.out_path("w7c_kor5_flags_first.csv"),
    "vsync,op,flag_hex,flag,ra,pc_offset,op_bytes,scene,mode,bit_before")
local first_seen, hit_count = {}, {}
local MIRROR = WANT_MIRROR and probe.csv_open(probe.out_path("w7c_kor5_mirror.csv"),
...
local function arm_all()
    ...
    bp.arm(0x8003BDE0, "Exec", 4, "dispatch", function()
        local r = PCSX.getRegisters()
        local line = string.format("f=%d DISPATCH x=%d z=%d rec=%d gate=%d ra=%s scene=%s", vsync,
            u32n(r.GPR.n.a0), u32n(r.GPR.n.a1), u32n(r.GPR.n.a2), u32n(r.GPR.n.a3), hex32(r.GPR.n.ra), scene_name())
        log(line)
    end)
    ...
    bp.arm(0x801D5630, "Exec", 4, "trig_lookup", function()
        local r = PCSX.getRegisters()
        local k = string.format("%d,%d,%d", u32n(r.GPR.n.a0), u32n(r.GPR.n.a1), u32n(r.GPR.n.a2))
        if seen5630[k] then return end
        seen5630[k] = true
        log(string.format("f=%d TRIG_LOOKUP kind,x,z=%s ra=%s", vsync, k, hex32(r.GPR.n.ra)))
    end)
    armed = true
...
local function on_vsync()
    ...
    if vsync % 60 == 0 then
        local p = player_ptr()
        if p ~= nil then
            log(string.format("f=%d PFLAGS +0x10=%s last_tile=(%d,%d) scene=%s mode=0x%02X", vsync,
                hex32(mem.read_u32(p + 0x10) or 0), mem.read_u32(0x8007BDC8) or -1,
                mem.read_u32(0x8007BDCC) or -1, sc, md))
        end
    end
    ...
    if CKPT_FLAG ~= nil and not ckpt_done and md == 0x03 then
        if ckpt_flag_at == nil and flag_bit(CKPT_FLAG) == 1 then
            ckpt_flag_at = vsync
            log(string.format("checkpoint flag 0x%03X read set at %d", CKPT_FLAG, vsync))
        end
        if ckpt_flag_at ~= nil and vsync - ckpt_flag_at >= CKPT_DELAY then
            ckpt_done = true
            checkpoint()
        end
    end
