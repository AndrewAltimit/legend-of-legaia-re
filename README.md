...
- [**Meth962's "Legend of Legaia 100% Walkthrough"** (GameFAQs)](https://gamefaqs.gamespot.com/ps/197766-legend-of-legaia/faqs/53721) - cataloged per-enemy Drop & Steal tables, per-Seru magic-absorption probabilities (e.g. `Gimard(2)[30%]` = 2 party members need the spell, 30% per-encounter absorption rate), magic-leveling mechanics, and full boss stat blocks (HP / ATK / UDF / LDF / SPD / INT / AGL / MP plus per-boss attack name + MP cost) with their XP / gold / item rewards. Cross-validates `legaia-gamedata`'s `enemies` and `bosses` tables (`data/gamedata/`); the per-boss stat array is a useful sanity check for the binary monster-stat records that `crates/asset/src/monster_archive.rs` decodes from PROT 0867.
