...
| [`site-shell.md`](docs/tooling/site-shell.md) | The static site's app shell across breakpoints: rail / drawer / top bar, and why the rail's destinations must be re-rendered inside the drawer that replaces it. Two silent-shadowing traps, a gate each: a `@media` block adds **no** specificity, so a breakpoint above the rule it overrides loses at every width (`check-css-cascade-order.py`); and a case fold binds to the operand it is written on, so folding the tail of a `+` chain leaves every earlier literal cased (`check-js-case-fold-chains.py`). Also the page layer's delivery rules: one disc reaches a page's `onLoad` exactly once, `js/*.js` busting is content-addressed by `_gen.py` (never hand-written), and the WebGL2-only 3D pages raise a named banner instead of drawing untextured. |
