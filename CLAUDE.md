...
| [`str-fmv-table.md`](docs/formats/str-fmv-table.md) | FMV dispatch table at `0x801D0A6C` (23 × 32-byte slots, master dispatch `FUN_801CEA3C`; nine retail `fmv_id 0..=8` = every disc movie, `MV3.STR` split by frame range; parser `legaia_asset::fmv_dispatch`) + post-play return-scene labels at `0x801CE8AC`. Per-scene trigger assignment is disc-sourced: literal `fmv_id` operands in the scene MAN scripts (`man_field_scripts::scene_fmv_triggers`). |
...
| [`minigame-slot-machine.md`](docs/subsystems/minigame-slot-machine.md) | Casino slot machine: reel state machine (`FUN_801cf0d8`), dual RNG (LCG `FUN_801d30cc` + BIOS-rand feature rolls), **five**-payline payout/jackpot eval (`FUN_801d13e8` - three straight + two diagonal); cash-out commits the overlay-local balance into coin bank `0x800845A4`. The machine is a **3D scene** projected through the GTE via the SCUS wrappers (the overlay has no `cop2` op of its own): reels = textured cylinders (`FUN_801d0fa8`, trig-table `y`/`z`, depth-cue shade), paylines = `RTPS`-projected 3D lines (`FUN_801d3380`), medallions / lamps / pedestals / marquee = billboards (`FUN_801d08e4` -> `FUN_800195a8`), marquee = a 78x13 dot-matrix display (`FUN_801d0e1c`) over a 21-bitmap message bank; geometry tables tile `0x4E68..0x4F38` in PROT 0975 (parser `legaia_asset::minigame_slot_scene`). **Bonus round**: the reels swap to numeral faces `1..=10` (PROT 1200 art index 1, texpage `0x0D`, 4x4 grid of 64x64 cells; numeral `n` = strip value `n + 0xF`, CLUT row 491 column `n-1` - one palette per digit, which is why each is a different colour; blank past ten). Payout is the bare product of the three **centre-row** numerals (`(v-0xF)` each, 1..1000 coins) with no all-equal gate and the winning line forced to centre; the top strip's `0 x 0 x 0` tally is the claimed latch `DAT_801d3d20[reel]` printed from the marquee message bank (ids 6..16 = "0".."10", `0x11` = "x", `0x14` = "coin"), so the tally reads the same rows the payout multiplies. Mode 6 stops **free** (depth 0 / target -1) - it does NOT reuse the guaranteed plan. The **cabinet**'s emitter is unpinned - NOT koin casino-scene geometry (it still draws under a town01 debug warp with no koin meshes resident) and not traced overlay art. Distinct from the prize exchange. |
...
| [`world-map.md`](docs/subsystems/world-map.md) | World map controller (`FUN_801E76D4`); top-view debug toggle; camera scroll globals; dev menu renderer (`FUN_801EAD98`); render pipeline + bulk continent terrain emit mechanism; ocean CLUT cycling = the kingdom slot-5 CLUT-walk table walked by `FUN_8001ADA4` case `0xB` (`legaia_asset::clut_walk`); the script-driven CLUT-cell family (`FUN_801E4C58` / `FUN_801E4794`) carries only the row-498 park one-shots/fades. |
...
| [`translation.md`](docs/tooling/translation.md) | `legaia-rando translate export/init/stats/import` - community language packs. Disc text → editable YAML (SCUS name tables + the `0x1F`-segment dialog corpus: scene-bundle MANs + raw event-script carriers) → same-size in-place reimport (`translation` module; markup codec is byte-exact, per-character encodability errors; non-Latin scripts need a font patch, out of scope). Exported packs carry game text - gitignored (`/translations/`, `legaia_*.yaml`), never commit. |
...
| [`crates/asset`](crates/asset/README.md) | `asset` | Dispatcher, DATA_FIELD streaming, pack format, scene-bundle + effect-bundle + multi-bank-VAB detectors; `categorize` module classifies every PROT entry by format class (disc-gated `categorize_coverage` test asserts ≥99% of corpus bytes are covered). `field_disasm` is the side-effect-free field-VM bytecode disassembler (width/format decoder + `LinearWalker`; `legaia-engine-vm` re-exports it for the executing VM). `inn_costs` locates the scripted gold charges (op-`0x4E` sub-3 gold-gate + `0x3A` debit pairs in scene MANs - retail has no inn cost table). |
...
| [`crates/engine-vm`](crates/engine-vm/README.md) | - | Actor / field / effect / move / **motion** VMs + battle-action SM + `battle_formulas` (damage / MP / accuracy / RNG / escape) + **world-map entity SM** (`FUN_801DA51C`, 5-state encounter/interact port). Re-exports the field-VM disassembler from `legaia-asset` (`field_disasm`). |
...
| [`crates/web-viewer`](crates/web-viewer/README.md) | - | WASM target. Disc browser + TIM thumbnails + software TMD rasteriser running in the browser, plus per-entry MES/SEQ/VAB inspector via `current_entry_info_json`. `field_scene` (`set_scene_field`) assembles a CDNAME scene's **full map** (env mesh pack + `.MAP` placements + terrain tiles + ground heightfield via the shared `engine-core::field_env` kernel) for the viewer page's "full map" button. `rom_patcher` runs the `legaia-rando` randomizer client-side (`patch_rom` → patched-image bytes + summary; the summary is spoiler-safe - random starting items report count-only) for the in-browser ROM-patcher page; nothing is uploaded. `scene_export` is the builder-style `.glb` session behind the site's download buttons (world-overview continent, viewer full-map + single TMD; bake = `legaia_asset::scene_gltf`). |
