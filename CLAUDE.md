...
| [`battle-data-pack.md`](docs/formats/battle-data-pack.md) | Player battle files `data\battle\PLAYER1..4` (extraction 863..866 = retail `battle_data` block; Vahn/Noa/Gala/Terra). Header + LZS `record[0]` (battle-palette chain) + 12-byte `[id, offset, size]` descriptor table (equipment-id sections) + per-slot LZS streams → `[32-byte header + Legaia TMD + texture pool]`; slots tile each file's footprint exactly. The old "16 MB container at 0865" was the extended TOC window over-reading into the monster archive (extraction 867). |
