...
| [`cast-module.md`](docs/subsystems/cast-module.md) | Capture-class cast modules (PROT 0935..0966) at slot-B `0x801F69D8`: image shapes, `ctx+0x279` phase machine, clip-staging ABI, seat-0 damage hardcode. |
...
| [`vrchat-world-export.md`](docs/tooling/vrchat-world-export.md) | `legaia-engine export-glb`: per-scene textured world / NPC / animated-prop `.glb`s + placement manifest for Unity/VRChat world building; Unity kit in `scripts/vrchat-world/`. Output is Sony-derived, gitignored. |
...
| [`crates/engine-audio`](crates/engine-audio/README.md) | `note-trace` | cpal-backed audio mixer + clean-room SPU + SsAPI-shape SEQ sequencer; BGM cross-fade + volume ramp; `audio-webaudio` feature adds `WebAudioOut` (`ScriptProcessorNode`-based) for WASM targets. |
