...
The shading half is true of the **native** renderer, where the synthetic Lambert lives only in `MESH_SHADER_SRC`, the asset-viewer's bare-geometry preview - a viewer aid, not a claim about retail. The **browser** is not yet all the way there: `site/js/webgl-shaders.js` applied that Lambert on *both* its paths, on every 3D page. Its untextured path is now retail; its textured path still carries the term as a stand-in for `texel * packet_colour / 128` until the page uploads per-vertex packet colour. Tracked in [`docs/tooling/host-drift.md`](docs/tooling/host-drift.md).
...
| [`live-audit-triage.md`](docs/tooling/live-audit-triage.md) | Per-anchor verdicts for the `engine-core` / `engine-vm` rows of `port-catalog.py --live-audit` ("undisclosed inert ports"), keyed by address + site so the wiring work is mechanical rather than re-derived. |
...
| [`host-drift.md`](docs/tooling/host-drift.md) | The port ships **three** hosts on one engine (native play-window, browser play page, browser minigames page), so a feature wired into one is invisible in a diff. Five gate tiers, each answering one shape and silent about the rest: builder reachability, paired constants, paired simulation injection sites, trait-override symmetry, and no page-side keyboard table. Also what a waiver may say. |
| [`shipped-bundle-freshness.md`](docs/tooling/shipped-bundle-freshness.md) | `site/wasm/` is uncommitted build output (CI rebuilds it at deploy), but a *local* bundle still goes stale, and its source closure is 27 crates - so an engine edit changes the play page invisibly. Content-addressed stamp + `check-wasm-freshness.py`; the page records why mtime and `git log` both answer "in sync" for a stale bundle. |
| [`site-shell.md`](docs/tooling/site-shell.md) | The static site's app shell across breakpoints: rail / drawer / top bar, and why the rail's destinations must be re-rendered inside the drawer that replaces it. Carries the cascade-order trap - a `@media` block adds **no** specificity, so a breakpoint placed above the rule it overrides loses at every width while still reading like a feature. Gate `scripts/ci/check-css-cascade-order.py`. Also the page layer's delivery rules: one disc reaches a page's `onLoad` exactly once, `js/*.js` busting is content-addressed by `_gen.py` (never hand-written), and the WebGL2-only 3D pages raise a named banner instead of drawing untextured. |
...
| [`crates/engine-ui`](crates/engine-ui/README.md) | - | Renderer-agnostic UI draw-list builders (`TextDraw`/`SpriteDraw`, `ui_overlay`, `ui_menu/`, `ui_title_save/`, `ui_fishing`). The wgpu-free leaf shared by the native renderer and the browser play page - the crate `web-viewer` can depend on where `engine-render` can't (hard wgpu link). |
...
- **Port and document from the disassembly, not the decompiled C.** The label-call idiom above is one form of a broader rule. The C is a rendering, and six distinct rendering artifacts have each already put a false claim into this repo's committed docs - dropped register arguments, `||` rendered as nested `if`s, reordered and dropped stores, hand-written Ghidra annotations taken as evidence, dumps that report `0 instructions` and carry only C, and `unaff_*`/`in_stack_*` mistaken for proof of a fragment. Each one, how to spot it, and what counts as evidence instead: [`docs/tooling/ghidra.md`](docs/tooling/ghidra.md#decompiler-artifacts-that-have-produced-false-claims).
