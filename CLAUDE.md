...
| [`character-mesh.md`](docs/formats/character-mesh.md) | Player-character meshes. Field form = PROT 0874 §0 (low-poly, parser `legaia_asset::character_pack`). Battle form = **assembled per character** from the player battle files' equipment-id sections (`FUN_80052770` case 4 → `FUN_80052FA0` → `FUN_800536BC` splice → `FUN_80053898`; port `legaia_asset::battle_char_assembly`; registered by `FUN_800513F0`/`FUN_800542C8`); PROT 1204 `other5` is the Baka Fighter / default-equipment sibling pack (parser `legaia_asset::battle_char_pack`). Object-local vertices; battle rest pose = frame 0 of the character's own record[0] idle stream (1204's pose source is the PROT 1203 ANM banks, 1204-order only); 256×256 4bpp atlas. |
