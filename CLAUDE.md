...
| [`crates/rando`](crates/rando/README.md) | `legaia-rando` | Randomizer / disc patcher for a user-supplied `.bin`. Same-size in-place PROT-entry + named-file edits (`disc::DiscPatcher` → `legaia_iso::write`), variable-length MAN relocation for doors, `rng::SplitMix64`, PPF 3.0 output. Feature modules: monster/equipment drops, encounters, chests, steals, arts combos, scene + house doors, shops, casino, starting items, item prices, unused content, weapon specialty (per-character favored weapon class, via the player-file arm-cost byte), and battle-tuning (monster combat stats, special-attack move power, element-affinity matrix, spell MP costs) - via `apply`. No Sony bytes. Full reference in [`docs/tooling/randomizer.md`](docs/tooling/randomizer.md). |
