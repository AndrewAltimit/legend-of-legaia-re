...
- `crates/rando/tests/chest_patch_real.rs` - whole-disc chest shuffle on a scratch copy: re-decodes every patched scene MAN and asserts the field-VM `0x39` give-item site offsets are unchanged, the chest-item multiset is preserved, sectors stay valid, and a fixed seed is deterministic; plus a targeted keikoku-chest patch asserting the give operand AND every `0xC2` announcement item-name token both carry the new id.
