...
| [`host-drift.md`](docs/tooling/host-drift.md) | The port ships **three** hosts on one engine (native play-window, browser play page, browser minigames page), so a feature wired into one is invisible in a diff. Five gate tiers, each answering one shape and silent about the rest: builder reachability, paired geometry constants, paired simulation injection sites, trait-override symmetry, and no page-side keyboard table. Also what a waiver may say. |
