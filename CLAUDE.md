...
| [`site-shell.md`](docs/tooling/site-shell.md) | The static site's app shell across breakpoints: rail / drawer / top bar, and why the rail's destinations must be re-rendered inside the drawer that replaces it. Carries the cascade-order trap - a `@media` block adds **no** specificity, so a breakpoint placed above the rule it overrides loses at every width while still reading like a feature. Gate `scripts/ci/check-css-cascade-order.py`. |
