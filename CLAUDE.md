...
| [`arts-command-gauge.md`](docs/subsystems/arts-command-gauge.md) | Arts AP gauge + weapon-specialty arm width. Per-command cost at `DAT_801C9360[char][cmd]+0x74` (arm = cmd `0x0C`); weapon-class penalty over base (favored `0x1E` / off-class `0x2A` / Astral `0x36` — escalating penalty, not a flat ×2). Read by gauge builder `FUN_801D388C` case 9, spent in case `0xB` (AP at `ctx+0x6DC`); execution resolver `FUN_801EC3E4` called from SCUS `0x800478A0`. Off-class penalty writer still open. |
...
| [`crates/rando`](crates/rando/README.md) | `legaia-rando` | Randomizer / disc patcher for a user-supplied `.bin`. Same-size in-place PROT-entry + named-file edits (`disc::DiscPatcher` → `legaia_iso::write`), variable-length MAN relocation for doors, `rng::SplitMix64`, PPF 3.0 output. Feature modules: monster/equipment drops, encounters, chests, steals, arts combos, scene + house doors, shops, casino, starting items, item prices, unused content, and battle-tuning (monster combat stats, special-attack move power, element-affinity matrix, spell MP costs) - via `apply`. No Sony bytes. Full reference in [`docs/tooling/randomizer.md`](docs/tooling/randomizer.md). |
