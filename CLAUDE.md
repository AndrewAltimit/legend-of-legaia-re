...
| [`battle-data-pack.md`](docs/formats/battle-data-pack.md) | Player battle files `data\battle\PLAYER1..4` (extraction 863..866 = retail `battle_data` block; Vahn/Noa/Gala/Terra). Header + LZS `record[0]` (battle-palette chain) + 12-byte `[id, offset, size]` descriptor table (ids = equippable item ids; five sections = the equip slots) + per-slot LZS streams → `[32-byte header + Legaia TMD + texture pool]`; slots tile each file's footprint exactly. The old "16 MB container at 0865" was the extended TOC window over-reading into the monster archive (extraction 867). |
...
| [`character-mesh.md`](docs/formats/character-mesh.md) | Player-character meshes. Field form = PROT 0874 §0 (low-poly, parser `legaia_asset::character_pack`). Battle form = **assembled per character** from the player battle files' equipment-id sections (`FUN_80052770` case 4 → `FUN_80052FA0` → `FUN_800536BC` splice → `FUN_80053898`; port `legaia_asset::battle_char_assembly`; registered by `FUN_800513F0`/`FUN_800542C8`); PROT 1204 `other5` is the Baka Fighter / default-equipment sibling pack (parser `legaia_asset::battle_char_pack`). Object-local vertices + ANM frame-0 rest pose, 256×256 4bpp atlas. |
...
| [`crates/engine-shell`](crates/engine-shell/) | `legaia-engine` | Top-level driver + `BootSession` + `AudioBgmDirector`; boots a CDNAME scene straight from `PROT.DAT`. Subcommands: `info`/`list-scenes`; `play` (tick N frames); `play-window` (960×720 wgpu + keyboard, shop/inn/level-up overlays; flags `--live-loop` Field↔Battle, `--player-battle` HUD + full overworld stage battle with the party assembled from the player battle files' equipment sections (`battle_char_assembly` + `relocate_tsb_cba`; PROT 1204 fallback) under the orbit camera, `--vm-dialogue` inline-script field VM); `save`/`load`; `play-str` (MDEC video player); `config set --binding`. Parity oracles `vram-oracle` / `mode-trace` / `audio-trace`. |
