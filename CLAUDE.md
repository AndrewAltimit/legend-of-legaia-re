...
| [`character-mesh.md`](docs/formats/character-mesh.md) | Player-character mesh pack — two forms, same `DAT_8007C018[0..=4]` slots. **Field form**: PROT 0874 §0 → 5 Legaia TMDs (Vahn/Noa/Gala + 2 auxiliary), resident every field scene; active-party slots ship `nobj=12` with groups 10/11 as equipment-conditional templates that `FUN_8001EBEC` patches over visible groups 0/3/5 per a record byte (`+0x196` / `+0x199` / `+0x19B`). Field-form TMD vertices are in **object-local space** (each object's X centroid ≈ 0); the per-scene ANM frame 0 supplies the rest-pose assembly transform (T from bytes 0..4, R from bytes 5/6/7 per [`anm.md`](docs/formats/anm.md)). Parser `legaia_asset::character_pack`. **Battle form**: PROT 1204 (`other5`) flat streaming-format with 5 TMD2 chunks (asset type `0x09`, typical `nobj` 15/16/15) + 7 inline 256×256 4bpp character TIM atlases at fixed `0x8224` stride (CLUTs at VRAM rows 490..495,497). Battle-form TMD vertices sit in actor-anchored model-space (object centroids spread across the assembled stance, not centered at object origin); the runtime pipeline composes the same ANM `(T, R)` on top of them. Parser `legaia_asset::battle_char_pack`. |
