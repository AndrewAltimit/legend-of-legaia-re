...
| [`anm.md`](docs/formats/anm.md) | ANM animation pack (player / field actors). Per-scene player ANM ships inside each scene's first PROT slot as a **type-0x05 ("MOVE")** section (87 bundles across the corpus; battle-form at PROT 1203 `other5`). Despite the type-0x05 label, content is a canonical ANM container (`marker_1 = 0x080C` records). Per-record layout: 8-byte header + 8-byte prologue + `b` frames × `(a & 0xFF)` bones × 8 bytes per (bone, frame); `record_size = 16 + 8 * (a & 0xFF) * b` falls out byte-exact across all 296 corpus records. The 8 bytes per (bone, frame) decode as 4 little-endian `i16` values; the exact semantic of the four shorts is the still-open thread (working hypothesis: first 3 are Euler angles in PSX 12-bit fixed-point). Parser `legaia_asset::player_anm`. |
...
| [`character-mesh.md`](docs/formats/character-mesh.md) | Player-character mesh pack — two forms, same `DAT_8007C018[0..=4]` slots. **Field form**: PROT 0874 §0 → 5 Legaia TMDs (Vahn/Noa/Gala + 2 auxiliary), resident every field scene; active-party slots ship `nobj=12` with groups 10/11 as equipment-conditional templates that `FUN_8001EBEC` patches over visible groups 0/3/5 per a record byte (`+0x196` / `+0x199` / `+0x19B`). Parser `legaia_asset::character_pack`. **Battle form**: PROT 1204 (`other5`) flat streaming-format with 5 TMD2 chunks (asset type `0x09`, typical `nobj` 15/16/15) + 7 inline 256×256 4bpp character TIM atlases at fixed `0x8224` stride (CLUTs at VRAM rows 490..495,497). Parser `legaia_asset::battle_char_pack`. |
