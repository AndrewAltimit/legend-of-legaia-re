...
| [`field-menu.md`](docs/subsystems/field-menu.md) | Per-character status/party panel renderer `FUN_801D33D8` (Status/Magic/Moves/Skills tabs). Content-only draw (frame is caller-drawn); pixel-pinned offsets from window origin (`a0+0xa/+0xc`); submenu-id fold; three shared string/icon/number primitives; CLUT-staging global `DAT_8007b454` read only by the string primitive. Engine port = `engine-render::status_screen_draws_for` + `menu_window_chrome_draws_for` + `scale_stage_text_draws`. |
