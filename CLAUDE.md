...
| [`crates/rando`](crates/rando/README.md) | `legaia-rando` | Randomizer / disc patcher for a user-supplied `.bin`. Same-size in-place PROT-entry + named-file edits (`disc::DiscPatcher` → `legaia_iso::write`), variable-length MAN relocation (doors + `man_edit::apply_insertions` byte-inserts), `rng::SplitMix64`, PPF 3.0 output. Feature modules: monster/equipment drops, encounters, chests, steals, arts combos, scene + house doors, shops, casino, starting items (incl. explicit `--start-with` picks of any item/accessory id, and a field-VM `GIVE_ITEM`-injection path that lifts the 7-slot direct-seed cap by splicing a guarded grant block into the opening scene's MAN — `starting_bag`), starting level (the new-game seed loop stamps the displayed-level byte `+0x130` on every roster slot, so this levels the whole starting party: `+0x130 = N` party-wide + each growth-capable slot's growth-curve stats (Vahn/Noa/Gala) + the lead's cumulative experience `+0x0` / next-threshold `+0x4`), item prices, unused content, weapon specialty (per-character favored weapon class, via the player-file arm-cost byte), and battle-tuning (monster combat stats, special-attack move power, element-affinity matrix, spell MP costs) - via `apply`. No Sony bytes. Full reference in [`docs/tooling/randomizer.md`](docs/tooling/randomizer.md). |
