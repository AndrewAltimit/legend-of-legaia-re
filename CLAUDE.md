...
- `crates/engine-core/tests/door_randomizer_runtime_e2e.rs` - the door-randomizer counterpart, completing the five-oracle set: patches Rim Elm's (town01) single exit — the `0x3F` named-scene-change at MAN op `0x6f95`, originally → map01 — to a differently-named scene (exercising the variable-length resize) in memory, re-decodes the patched MAN off the patched image, and drives the patched op through the real field VM (`World::load_field_script` + `tick`) to assert the runtime warps to the patched destination (not the original). A baseline pass over the unpatched exit keeps it non-vacuous; same savestate-cache-trap rationale (the scene MAN is resident in RAM the moment you're in the scene).
