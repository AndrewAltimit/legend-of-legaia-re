...
| [`character-mesh.md`](docs/formats/character-mesh.md) | Player-character mesh pack. **PROT 0874 §0** → 5 Legaia TMDs (Vahn/Noa/Gala + 2 auxiliary), installed into `DAT_8007C018[0..=4]` via `FUN_80026B4C`. **Used in both field AND battle** — the field initializer (`FUN_801D6704`→`FUN_80020118`→`FUN_8001E890`) and the battle scene loader (`FUN_800520F0` state `0xb` loads PROT 874 as `etmd.dat`, state `0xc` `tmd_register`s it) install the *same* pack; there is no separate battle-character mesh. Active-party slots ship `nobj=12` with groups 10/11 as equipment-conditional templates that `FUN_8001EBEC` patches over visible groups 0/3/5 per a record byte (`+0x196` / `+0x199` / `+0x19B`). TMD vertices are in **object-local space** (each object's X centroid ≈ 0); the per-scene ANM frame 0 supplies the rest-pose assembly transform (T from bytes 0..4, R from bytes 5/6/7 per [`anm.md`](docs/formats/anm.md)). Parser `legaia_asset::character_pack`. **Baka Fighter minigame roster**: PROT 1203–1221 (`other5`, canonical entry 1204) is the *fist-fight minigame* character pack — NOT the main battle party (Baka Fighter just lets you play as Vahn/Noa/Gala, reusing beefier fighter models). Flat streaming-format, 5 TMD2 chunks (asset type `0x09`, `nobj` 15/16/15/…) + 7 inline 256×256 4bpp atlases at fixed `0x8224` stride (correct Baka-Fighter CLUTs at VRAM rows 490..495,497); loaded by `overlay_baka_fighter`. Parser `legaia_asset::baka_fighter_pack` (historical misnomer — it parses the Baka Fighter pack). |
