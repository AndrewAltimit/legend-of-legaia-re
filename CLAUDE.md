...
| [`character-mesh.md`](docs/formats/character-mesh.md) | Player-character mesh pack: PROT 0874 §0 → 5 Legaia TMDs (Vahn/Noa/Gala + 2 auxiliary), kept resident across every field scene at `DAT_8007C018[0..=4]`. Active-party slots ship `nobj=12` with groups 10/11 as equipment-conditional templates that `FUN_8001EBEC` patches over visible groups 0/3/5 per a per-character record byte (`+0x196` / `+0x199` / `+0x19B`). Parser `legaia_asset::character_pack`. |
