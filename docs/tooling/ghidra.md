...
| Absolute-only address sweeps | A "no static writers" claim from a sweep that searched only the absolute `lui`+offset form. MIPS reaches the same address gp-relative, and that form carries a different immediate, so it is invisible to the scan. `0x8007B8C2` was recorded as writer-less across 2661 dumps; the writer is `0x80015F08 sh v0,0x5aa(gp)`. | Resolve `gp` first (`lui gp` / `addiu gp,gp` in the entry stub), then scan **both** forms - absolute `lui base`+offset and `gp`-relative - before asserting anything about writers. |
