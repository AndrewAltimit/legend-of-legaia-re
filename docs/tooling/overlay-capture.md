...
- All four CLUT fingerprints match `0895_bat_back_dat.BIN` at well-separated offsets - PROT 0895 is the boot `init.pak` bundle (the `bat_back_dat` label is inherited from the CDNAME define at 895 — in raw-TOC space that define lands on the `summon.dat`/`readef.DAT` battle-backdrop files instead; see [`formats/summon-readef.md`](../formats/summon-readef.md) and [`cdname.md` § index space](../formats/cdname.md#which-index-space-are-the-define-numbers-in-open)). Documented in [`subsystems/boot.md` § Boot init.pak](../subsystems/boot.md#boot-initpak-prot-0895).
