...
| `801dd0c0` | `engine-core/src/menu_item_category.rs` | RESOLVED | Was wired-but-inert: reached via `best_equipment_now`'s scoring closure with a table nothing filled. `World::install_menu_overlay_tables` now parses `DAT_801E4B88` off PROT 0899 and `build_equip_session` installs it with the edited party slot, so the check scores real disc data on both hosts. |
