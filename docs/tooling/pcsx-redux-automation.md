...
- **Probes:** [`autorun_minigame_overlay_capture.lua`](../../scripts/pcsx-redux/autorun_minigame_overlay_capture.lua) polls `game_mode` (`0x8007B83C`) per vsync from a minigame-entry save (or a by-hand run with `LEGAIA_NO_SSTATE=1`). On the first `0x18`/`0x19` read it logs the trigger vsync + the `0x3E` sub-id (`0x8007BA34`) + both overlay slot pointers, dumps the overlay window `0x801C0000..0x80200000` at +0/+10/+30 vsyncs, then one full main-RAM dump.
- **What it answers:** The PROT 0896 (`bat_back_dat`) resident-identity window. The OTHER mode pair (24/25) hosts the minigames, but every parked mode-25 state already has the *individual* minigame's overlay resident — and a pre-transition entry save is still mode 3 with the 0896 unique head nowhere in RAM. The only window left is the mode-24 entry itself; the dumps feed `overlay_residency.py` for the byte-match at base `0x801C5818`. See [`reference/open-rev-eng-threads.md`](../reference/open-rev-eng-threads.md) "PROT 0896 overlay identity".
