...
| `autorun_xp_table_reader.lua` | Read bps tiled across `0x8007123C..0x80071300` | Originally written to pin the runtime XP-table reader. **The target was wrong** — that address is an off-by-`0x800` artefact and the bytes there are a sin-LUT slice, not an XP table (see [`subsystems/level-up.md`](../subsystems/level-up.md#xp-table)). Re-target the bps before re-running. The CSV / detail-sidecar shape of the probe is generic and reusable for any tiled-read-bp scan. |
...
| `autorun_battle_palette_source.lua` | Write breakpoints on the party-palette blocks `0x800EBEE8` / `0x800EC0C8` / `0x800EC2A8` (Vahn / Noa / Gala); each hit logs the writing PC + all GPRs and flags any register whose 32 bytes match the block (the source). On the first LZS-range write it dumps the loaded source buffer `0x80180000..0x80186000`. | **Resolved how the in-battle party palette is produced.** Run against `rim_elm_queen_bee_battle` (auto-starts, no input). The writes come from `FUN_8001A55C` (LZS decoder, `pc=0x8001A664`): the palette is **LZS-decompressed at battle load** from inside the loaded scene bundle (the dumped source buffer byte-matches PROT 0022 `town0c` from `0x23430`), as an embedded mid-block stream. Supersedes the "VRAM-residue / not-disc-recoverable" model. See [`formats/character-mesh.md`](../formats/character-mesh.md). The companion `autorun_battle_palette_lzs_src.lua` (Exec bp at the LZS entry, dumps src for palette-targeting decodes) crashes under the battle's heavy decode load — the write-watchpoint variant is the stable one. |
