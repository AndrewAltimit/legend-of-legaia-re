...
| [`autorun_view_build_attribution.lua`](../../scripts/pcsx-redux/autorun_view_build_attribution.lua) | Attributes a frame's drawn geometry to one of the three per-vsync field view builds. Taps `FUN_800172C0`'s entry (caller `ra` + the camera words it is about to read), `0x80017348` (the `TR` that build leaves in the GTE) and the TMD renderer `FUN_8002735C` (the `TR` live when geometry is actually transformed), so the per-vsync ordering says which build the draws ran under rather than which build ran last. |
