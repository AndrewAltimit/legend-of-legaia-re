...
| [`autorun_w7c_kor5_tail.lua`](../../scripts/pcsx-redux/autorun_w7c_kor5_tail.lua) | The W6-C spoke walk (tile-poke route + story-flag firehose) plus an exec-bp on the record dispatcher `FUN_8003BDE0`, one on the field overlay's trigger lookup `FUN_801D5630`, a write-watch on the actor-pool control block `0x8007C34C..0x8007C36F`, a per-second player-flags line and a flag-keyed checkpoint (`LEGAIA_CKPT_FLAG`). Built for the `kor5` chain and the `doman` / `son` spokes. |
