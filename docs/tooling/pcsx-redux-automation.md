...
| `autorun_slot4_source_map.lua` | Read bps tiled across the slot-4 RAM window + an Exec bp on the `FUN_8001E54C` streaming dispatcher, driving the held-Up Drake warp itself. Each read records the full GPR set, so the destination (if any) is recoverable. The Drake capture showed slot-4 is read **in place** by the world-map renderer (`0x801F78D4`) — no transcode; see [`world-map-overlay.md`](../formats/world-map-overlay.md#slot-4-is-read-in-place--there-is-no-transcode-drake-capture). |
