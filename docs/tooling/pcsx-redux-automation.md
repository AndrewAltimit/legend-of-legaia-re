...
| [`autorun_s4_capture.lua`](../../scripts/pcsx-redux/autorun_s4_capture.lua) | S4 wander-to-transition driver: bump-and-turn (hold a direction, turn when the player position stops changing, press CROSS to interact at walls, dismiss NPC dialogue) until the scene name changes, then checkpoints the new scene. Reaches free-roam but not (yet) a door - town01's doors are specific tiles and the pad is camera-remapped, so this needs the [deterministic door-targeting](playthrough-coverage.md#s4-navigation-the-doors-exist-but-need-deterministic-targeting) follow-up. |
| [`autorun_s4_padmap.lua`](../../scripts/pcsx-redux/autorun_s4_padmap.lua) | Measures the pad-direction -> world-displacement mapping: holds each of UP/RIGHT/DOWN/LEFT and records the net `(dX,dZ)` in `player+0x14`/`+0x18`. Confirmed those fields are X/Z (16.16 fixed) and that the camera-remap is position-dependent (the mapping differs by where the player stands), so a single fixed map is not reusable mid-run. |
| [`autorun_s4_navsweep.lua`](../../scripts/pcsx-redux/autorun_s4_navsweep.lua) | Self-calibrating world-space coverage sweep: primes into open space, measures the per-direction displacements online, picks +X/-X/+Z/-Z pad dirs, then serpentines the area with continuous CROSS-interact. Demonstrated the limit (the dynamic camera-remap breaks the fixed map; coverage still misses point-target doors) that motivates the grid-BFS [follow-up](playthrough-coverage.md#s4-navigation-the-doors-exist-but-need-deterministic-targeting). |
