...
| `autorun_slot4_transcoder_hunt.lua` | Write bps across the `0x801BA000`-ish working buffer where cluster A's per-frame inputs live | Cross-kingdom Exec-bp captures show cluster A reads from `0x801BA000`-region per frame, not from slot 4's documented base — slot 4 must be transcoded into that buffer during world-map scene load. This probe arms Write bps at structurally interesting offsets in the working buffer (the exact offsets cross-kingdom captures showed cluster A reading) and logs PC + ra-chain at every write. First write is the transcoder's entry. `LEGAIA_WB_BASE=0x801BA000` is overridable; `LEGAIA_WB_CAP=50` per-bp cap. |
