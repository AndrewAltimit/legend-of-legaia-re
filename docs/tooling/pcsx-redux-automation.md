...
| [`autorun_view_build_attribution.lua`](../../scripts/pcsx-redux/autorun_view_build_attribution.lua) | Attributes a frame's drawn geometry to the view build that was live when it was emitted, and it is what established that a field frame runs **two to five** builds from five sites rather than three ([census](../subsystems/cutscene.md#what-a-build-census-actually-measures)). Taps `FUN_800172C0`'s entry (caller `ra` + the camera words it is about to read), `0x80017348` (the `TR` that build leaves in the GTE), the TMD renderer `FUN_8002735C` and the OT link helper `FUN_8003D2C4`. Both measured runs recorded zero `FUN_8002735C` entries, so the ranking is the link helper's: the prim counter is the tap that carried the answer, and the renderer tap is the one that would sharpen it. |
