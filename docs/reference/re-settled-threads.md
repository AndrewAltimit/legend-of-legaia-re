...
| What does the between-leg dome restore (`FUN_801D0ED8`) do? | resolved (one-shot per contest, and it leaves the accessories alone) | `disassembly` | Its `jal 0x801CEBF0` runs only while `_DAT_8007BAC0 == 0`, so it fires once per contest rather than once per leg. It zeroes `+0x196` / `+0x197` / `+0x198` / `+0x19A`, leaves `+0x199` and the accessory bytes untouched, and restores slot-0 HP / MP / SP from the snapshot at `+0x104` / `+0x108` / `+0x10C`. See [`minigame-muscle-dome.md`](../subsystems/minigame-muscle-dome.md#the-between-leg-restore). |
...
| How does the 46-record text-cell table get seeded? | resolved (four stores per record, in a fixed order, with `+2` a copy of the prior `+0xA`) | `disassembly` | The seeding loop writes `+4`, `+0xC`, `+0xA`, `+2` in that order, and the `+2` store carries the **previous** record's `+0xA` value rather than this record's - which is what makes the table read as offset by one if the order is assumed. See [`script-vm.md`](../subsystems/script-vm.md#text-cell-table-init). |
