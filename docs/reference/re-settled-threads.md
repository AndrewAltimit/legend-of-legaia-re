...
| What gates each **capture-class** arm's dwell? | resolved for twelve of the fourteen trampoline arms; the drain is per arm | `capture` + `disassembly` | The countdown is drawn down by a **per-arm** multiplier of the scratchpad frame byte at `0x1F800393` - its product with `0x1F80037D`, twice it, or once it - so the single-product model held for one family only, and two arms measured as constants 4 and 8 were `1x` and `2x` a byte reading 4. Twelve of the fourteen arms are measured one fight each (PROT 0943's `0xAB` twice), reading the single `jal 0x801F2160` site inside PROT 0898 at `0x801E50C8` as one module tick. Three arms park rather than gate and PROT 0950's arm 6 has no gate at all. Tables in [`cast-module.md`](../subsystems/cast-module.md#frame-gating-measured). |
| Is the battle loader's model-pack pointer `*(gp+0x6BC)` sane at battle time? | resolved (**no** - the battle load reuses the block) | `capture` | Across the catalogued state population the pointer names the same heap block in 97 of 98 states; it is sane in 37 of 37 field states and garbage in 61 of 61 battle states. So the unclamped model-pack registrar inside `FUN_8001E890` - `lw a0,0x6BC(gp)` at `0x8001EAFC`, count off `+0x00` at `0x8001EB10`, `jal 0x80026B4C` per entry at `0x8001EB4C`, on the un-gated side of the `bne v1,2` at `0x8001EA54` - has a garbage block in front of it whenever a battle is loading, with no patched disc involved. |
