...
| What raises the still's fork `_DAT_801D1AE0`? | resolved (the arena init, on **re-entry**) | `disassembly` + `capture` | Not the match teardown: `FUN_801CEA6C` forks on the arena word `_DAT_8007BAC0` and stores `0` to the latch at `0x801CEB54` on a first entry (op `0x3E` leaves the word clear) and `1` at `0x801CEC04` on every later one. A 3600-vsync walk into the arena enters the emitter 79 times and takes the six-tile arm every time. The still arm's packets are read under a forced latch (190 entries, both quads every time); no catalogued save is parked on a re-entry, so the natural arm is pinned by the arming disassembly rather than by a frame. |
...
| What is PROT 0967's residue? | resolved (a head table, a leaf, a prompt pool and a neighbour's tail) | `disassembly` | 408 bytes of head jump table, 2,744 bytes of dumped code, one 92-byte un-dumped leaf ending `jr ra` at file `0xCA4`, 1,757 bytes of ASCII prompt pool (thirty printable runs) and 1,143 bytes inherited from PROT 0966. `disc-coverage.py` reports no un-dumped code run in the image, so nothing is owed a dump. |
| What is in the PROT 0898 head block below `0xDF8`? | resolved (nine runs of in-image jump-table words, two with a consumer) | `disassembly` | `0x801CF1CC` (179 words) is `jr`-dispatched at `0x801EA9FC`, indexed `lbu(0x8007BD0C + $s7) - 4` under `sltiu $v1, 0xB3` at `0x801EA9D8` - the table's own word count - with default `0x801EBDAC`; `0x801CF49C` (5 words) is dispatched after `0x801EB558` on the battle context's `+0x28A` under `sltiu 5`. Both consumers form the address with `lui`/`addiu`, which is why a literal-word sweep finds neither. The other seven runs have no consumer traced yet, and none of the nine is claimed: a pinned-table claim binds a parser's own constant, and no module reads them. |
...
| Thread | Status | Evidence | Answer |
|---|---|---|---|
| Does the byte account cut an overlay's inherited tail the way `disc-coverage.py` does? | resolved (it does now) | `disassembly` | `legaia_asset::inherited_tail` ports the Python fixpoint from `scripts/ghidra-analysis/inherited_tail.py`; the cut removes 91,536 bytes of other modules' code from the worklist across 83 of the 87 mapped images. Every `static-overlays.toml` row is `form = "raw"` with `content_bytes` equal to its PROT entry's file length, so the entry file is the as-loaded image and the cut needs only the PROT directory. PROT 0944 is the one image on which the Rust record walk and the Python band script disagree ([open](open-rev-eng-threads.md#measurement--tooling)). |
