...
| Why does every extracted overlay image end in another image's bytes? | resolved (the build buffer) | `disassembly` | An image's extracted extent runs past its own content into another module's code at the same file offset - a build-buffer leftover, now the `inherited_tail` shape and excluded from an image's own code denominator. **Two rules, two figures, and a citation has to say which:** the committed rule (same `base_va`, a strictly longer donor, run `>= 0x40`) gives **66 of 83** images and 61,597 bytes; the loose rule gives **79 of 83** and 104,700 bytes. Neither is wrong - they answer different questions. The measured case is PROT 0918 `0x801F9388..0x801F99D8` == PROT 0899's file `+0x29B0`, 1616 bytes, byte for byte. See [`disc-coverage.md`](../tooling/disc-coverage.md). |
