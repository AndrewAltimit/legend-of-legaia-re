...
| What consumes the scratchpad window `0x1F8003E8..EB`? | resolved (the renderer's visible tile window; the `0x801F2778..84` mirrors are write-only) | `disassembly` | Four signed bytes `[nearX, nearZ, farX, farZ]`, tile offsets from the camera tile, written by the camera-zone loader `FUN_801DBC20` (mask-kind records' `bytes[1..4]`) and by field-VM op `0x46` (`0x801DF2AC..0x801DF350`). Read by the slot-B render library's cell emitters (`FUN_801F7088` at `0x801F7434..0x801F746C`, siblings at `0x801F6A10` / `0x801F6D6C`), the camera scroll clamp `FUN_801DAA50` (against the walk-region AABB `0x1F800384..87`), the ambient emitter `FUN_801D6058` and dev-menu rows `0x12..0x15`. Invisible to the word scan because every access is `lui 0x1F80; ori 0x314; lb 0xD4(rX)`. Details: [`encounter.md`](../formats/encounter.md#the-scratchpad-window-0x1f8003e8eb). |
| What is the field run-button mask `0x800846DC`? | resolved (`0x48` = Cross \| R1, seeded once by the new-game data init; not configurable) | `disassembly` | The last of four button-mask config words at `0x80084140 + 0x590..0x59C` (`0x44` Cross\|L1, `0x21` Circle\|L2, `0x10` Triangle, `0x48` Cross\|R1), stored by `FUN_80034A6C` at `0x80034AA0..0x80034AB8` and read by the field mover at `0x801D0364` against the held mask `_DAT_8007B850`. No other writer across SCUS + 31 overlays (incl. the options screen); it sits inside the saved block, so a load restores the same value. The engine's host binding latches run off Square, a divergence now disclosed in [`field-locomotion.md`](../subsystems/field-locomotion.md). |
