...
| How a Delilas sibling's signature move stages its clips | resolved (channel + arithmetic; the stage *count* is corroboration) | `disassembly` | The cast modules drive `actor[+0x1DA]` as a running monster-archive entry index. Lu's (PROT 0960) writes explicit constants - `10`, then `14`, `12`, `13` in file order - reproducing the probe-traced `14 -> 12 -> 13`. Che's (0959) seeds `10` and carries one `lbu +0x1DA` / `addiu +1` / `sb +0x1DA` advance: `10 -> 11`. Gi's (0958) seeds `10` the same way, carries three of those `+1` advances plus one `addiu v0,v0,-2` back toward the seed, and seeds `13` elsewhere - so his index walks consecutively up from `10`, the shape `10 -> 11 -> 12` predicts and clip shape alone cannot separate from three arbitrary entries. |
