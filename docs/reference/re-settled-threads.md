...
| What does selector 8 (Antidote) clear? | resolved (both poison bits, and it skips a dead target) | `disassembly` | `0x80041BCC` / `0x80041C04` in `FUN_800402F4`'s selector-8 arm; the engine's cure had cleared Venom only and did not test for a dead target. |
...
| Does the field Magic screen charge the MP-saver discount? | resolved (yes, on every path) | `disassembly` | SCUS `0x8003118C..0x800311A4` inlines the discount and greys a row on `+0x10A < cost` (`0x80031204`); PROT 0899 debits the value `FUN_80035394` returns (single cast `0x801D93C0`, `0x801D9404..0x801D9418`; group cast `0x801D972C`), the re-cast gates compare against it and both panels display it. The engine now prices field, battle and dome casts through one discounted kernel ([`battle-formulas.md`](../subsystems/battle-formulas.md)). |
...
| Is the field fog half-sheet a world-space quad? | resolved (a view-space billboard) | `disassembly` + `capture` | `FUN_8003F348` folds `RotMatrixX(0x400)` into the base matrix (`S * I`, `S = 6`) at scratchpad `0x1F800334`, `FUN_8003F3FC` makes the particle's view point its translation, and `FUN_8003F86C` `RTPT`s `(dx0, 0, 0x80)` and `(dx1, 0, 0)` through it: screen-aligned at the particle's depth, `H * 0x80 * S / vz` px tall. The capture holds the matrix as `[[S,0,0],[0,0,-S],[0,S,0]]`, and the port's billboard puts 103 of `keikoku_chest_preload`'s 104 walked fog packets within a pixel ([`field-ambient-fx.md`](../subsystems/field-ambient-fx.md#the-sheet-is-a-view-space-billboard)). |
| What does the field drop shadow draw? | resolved (four textured quads over a projected 3x3 grid) | `disassembly` + `capture` | `FUN_800460AC` `RTPT`s three rows of three points `0x20` apart at the actor's own `y`, writing `[SXY, SZ]` pairs from scratchpad `0x1F800020`; `FUN_8001C394` links four `POLY_FT4` (`0x2E808080`, tpage `0x1F`, CLUT `0x7F86`, the 16x16 blob at u `0xE0`) at `((ΣSZ + 0xA0) >> 4) >> DAT_1F8003A4`, ten slots nearer under `0x800000`. Rebuilt from state actor lists the packets match retail's 12 of 12 on `town01` and 4 of 4 on `map01` / `map03` ([`renderer.md`](../subsystems/renderer.md#the-field-drop-shadow-fun_8001c394)). |
| Which field actors cast a drop shadow? | resolved (flags with `0x01020000` and without `0x200000`) | `disassembly` + `capture` | `FUN_8001B964` `0x8001BE20..0x8001BE48` tests the flags staged at `0x1F8002D0`; its `+0x72 == 0` early-out jumps to this gate, so an actor at render scale 0 still casts one. Retail also skips an actor with `+0x10` bit `0x2`, which the port does not ([`renderer.md`](../subsystems/renderer.md#the-field-drop-shadow-fun_8001c394)). |
| Why did `opdeene`'s jungle plants draw black natively? | resolved (a native-only restage keyed on colour) | `capture` | The native asset path gave the prologue's dim ambient `0x20` to every vertex whose colour read `0x80` - the builder's fill for light-source rows, but also the authored word on the jungle's one-quad billboards - and the grade took `0x20` to `V = 2`. Retail's colour words are identical at the crawl start and mid-crawl and its billboards draw as FT4 packets at `(98, 94, 42)`; the ambient is now staged by lit-row mask ([`host-drift.md`](../tooling/host-drift.md)). |
| Does the port's battle tint pass match retail? | resolved (yes, distance fade included) | `capture` | `FUN_8004A908` recomputed from RAM reproduces the stored `+0x74` / `+0x78` words for 258 of 266 bodies over 97 states, 54 on the distance-fade arm (a body beyond half its radius fades toward a darker copy; outdoor stages invert). The engine test measures 158 of 160 over the 58 loadable states ([`battle.md`](../subsystems/battle.md)). |
| What is the party body radius in battle? | resolved (640) | `capture` | `*(+0x22C)+0x58` reads 640 on 212 of 213 party seats; the engine's `PARTY_SEPARATION_RADIUS` (448) is unpinned and unchanged. |
