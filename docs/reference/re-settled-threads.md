...
| What does `0x801F696C` gate? | resolved (the strike loop's per-frame swing drift) | `disassembly` | `FUN_801EED1C` clears it at its head (`0x801EED88`); the Miracle arm (`sw` at `0x801EF5B8`, a `j` delay slot), the Super tail match (`0x801EFBD4`) and the auto-combo tail (`0x801F0518`) set it. Its one reader, `lw` at `0x801E3840`, arms the drift that moves the actor and its target along their facings each frame when the committed clip carries a header byte and the latched clip is outside `0x10..=0x1A` ([`battle-action.md`](../subsystems/battle-action.md)). |
