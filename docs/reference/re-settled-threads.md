...
| Thread | Status | Evidence | Answer |
|---|---|---|---|
| What triggers retail's in-battle steal? | resolved (the killing blow, once per battle) | `disassembly` + `capture` | Not a command and not a hit: `FUN_8004AD80`'s death-spoils arm (`0x8004B29C..0x8004B65C`) runs when a monster's knockdown clip ends at HP 0, and sets the latch `ctx[+0x27]` at `0x8004B3E4` **before** testing the killer, so the battle's first kill spends the attempt. The gates, the roll (doubled by Items Up), the caption record and the table's two readers are on [`steal-table.md`](../formats/steal-table.md). |
