...
**Falsified:** the coordinate, not the writer. The mid-battle `_DAT_8007B64A`
writer is real, but there is no routine at `0x801FD150` and no store at
`0x801FD514` - both are phantom printings from a base-tag-less
`overlay_0897`-program dump (`overlay_0897_xxx_dat_801fd150.txt`), `+0x167E8`
above the truth.

Three independent checks converge on the real home: the store's byte pattern
(`24020003 a082b64a` - `li v0,3; sb v0,-0x49b6(a0)`) occurs in **no** PROT
entry but 0898, at file `0x18510`; under 0898's tagged base `0x801CE818` that
is `0x801E6D28`, which lands inside the already-dumped-and-documented
`FUN_801E6968` (the Lost Grail Final Heal sweep, whose *tail* the arm is);
and only at that base do the arm's `j 0x801E6A7C` / `j 0x801E6CE8` exits fall
inside their own function - at the phantom base they point `0x16000` below
the "function", which is the self-consistency tell that should have been
checked first.

The near-miss trap: the phantom dump is internally coherent - clean prologue,
sane guard logic, real SCUS `jal` targets (those are base-independent) - so
everything *except* the overlay-local jumps corroborates the wrong address.
Worse, reading the body at the phantom coordinate turns the settled Lost
Grail sweep (`0xE7` = the Lost Grail item id, the accessory-slot consume)
back into an unknown "0xE7 record scan", re-opening a closed thread under a
new name.
...
Before naming a new function in an aliased band, byte-search the PROT corpus
for a distinctive word pair and check the overlay-local `j`/`jal` targets
land inside the claimed extent - and grep the reference indexes for the
*re-keyed* address, not just the printed one.
