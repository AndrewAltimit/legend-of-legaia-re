...
| ... and the six-anchor row's "sprite passes" | falsified (they are model passes) | The digit-strip row above is right that the anchors are the editor band and not the strips, but its "sprite passes" inherited the archive reading: `FUN_801D49E8` is the afterimage of a 3D fighter and `FUN_801D6310` a 3D walk-on. The editor rows are unreachable on a retail disc and are now `REPLACED-BY` ([`minigame-baka-fighter.md`](../subsystems/minigame-baka-fighter.md#the-developer-keyframe-editor)). |
...
Icon: this is the Attack x2 second pass - the refill that lifts `ctx[+0x16]`
from `0` to `1`. It is one of that byte's two writers; the stage site's bump at
`0x801E37AC..0x801E37BC` is the other.
...
| A cue whose category's slot is closed falls back to the class-2 bank | falsified (it is silent) | Plausible: both play hosts did exactly that, and a fallback bank is what a single-bank port reaches for. The drainer tests the slot's `+0xB` enable byte and skips the cue (`0x80016CE4..0x80016CEC`); the VAB close zeroes it. |
| The Muscle Dome's hub holds the class-2 bank | falsified (it holds the field bank; slot 2 is closed) | Plausible: the dome's rounds run the battle frame driver, which loads PROT 0869, and the format page recorded that as an inference. A retail state at the hub (mode `0x19`) has slot 2 closed and slot 6 open over PROT 0876's header, the bytes the field states hold ([`audio.md`](../subsystems/audio.md#retail-capture-of-the-slot-2--slot-6-residency)). |
| The engine keys on about half retail's key-on rate | falsified (a pairing of the track's opening bars against a mid-track window) | Plausible: `0.244` against `0.488` came off the key-on rate, the comparand the envelope defects had just left standing. A trace only as long as the retail window has nowhere to slide; aligned (engine frame `3111` / `3112`) the ratios are `1.148` and `1.060` ([`audio.md`](../subsystems/audio.md)). |
| `FUN_80035B50` is a delay queue | falsified (a round-robin ring of four) | Plausible: the port's `SfxScheduler::enqueue` is one, and was tagged with this address. Retail writes the next slot with no free-slot search and a fifth cue replaces one; the delay is set by the sibling `FUN_80035BAC`. |
...
| `FUN_8001E54C`'s types `2` / `0xC` are VRAM rect uploads, and cases `1` / `3` decode LZS | falsified (SEQ installs and VAB transfers; no LZS arm) | Plausible: the installer is reached from scene loads that also upload textures, and a staging copy looks like a blit. Type `2` detaches and closes the SEQ player, copies the payload into the slot's staging buffer and opens it with `SsSeqOpen` through `FUN_80026410`; `0xC` opens the fixed record `0x800705AC`; `1` / `3` are `FUN_8002630C`. All 90 disc streams with a score past offset 0 carry it as a type-2 chunk ([settled](re-settled-threads.md#audio)). |
| `FUN_801D4A60` parks on a BGM request / ack pair | falsified (the side-band sound-bank pair) | Plausible: the guard at `0x801D4B58` polls a request word against an acknowledge, which is the BGM handshake's shape. The pair is `_DAT_8007BABC` / `_DAT_8007BAA0`, the side-band bank the engine already ran as `World::audio.sound_stream`, so the tag's blocker was not a blocker ([`field-locomotion.md`](../subsystems/field-locomotion.md#openers-and-closers)). |
| Every `music_01` entry is a self-contained `[VAB][SEQ]` pair | falsified (75 of 81 are) | Plausible: the bank was sampled at its tracks, and each sampled entry was a pair. The installer walk over all 81 finds slot 72 a score with no bank, 76..79 one-sector fills and 80 a bank with no score ([settled](re-settled-threads.md#audio)). |
...
| Thread | Verdict | Why |
|---|---|---|
| `FUN_801D362C` is a cutscene dialogue routine | falsified (the move VM's `0x2F` extension dispatcher) | Plausible: the actor-VM page listed it among cutscene routines. Its only reference is the move VM's op-`0x2F` arm; sub-op `0x2F` (`0x801D45D4`) stores the frame-step floor, which is how `opdeene`'s prescript installs its cadence of 3 ([`actor-vm.md`](../subsystems/actor-vm.md)). |
| The narration crawl's geometry is a per-scene table, with 18 px line spacing in `opdeene` | falsified (a config block seeded by the script; pitch fixed at 16) | Plausible: a pixel capture fitted each scene's window and speed, and `opdeene`'s lines measured about 18 px apart. The roller reads `*0x801C6EA4` `+0x4C..+0x52`, which `FUN_8003A024` resets and a `CC F8 E8` op seeds before each block; the pitch is `addiu s3,s3,0x10`, and no retail store produces 18. |
| The title menu's third row opens Options | falsified (the title menu has two rows) | Plausible: the port's `TitleOutcome` enum had an `Options` arm and both hosts routed it. The tick wraps the cursor with `andi v1,v1,0x1` at `0x801DDC00` and the confirm branches on row 0 against everything else; retail reaches Options from the pause menu. |
| The native window draws the boot Options screen twice, and its tests read the unframed copy | falsified (it drew once, unframed; no test read it) | Plausible: the host-drift page said so, and two copies of one screen is a common drift shape. The native title installed a bare session painted at a fixed pen and never reached the framed screen. |
...
| Thread | Verdict | Why |
|---|---|---|
| `kor5`'s `0x619` is set by a talk body, or never by retail's entry | falsified (a spawn-section write) | Plausible: a cold entry set it on the engine and a post-chain capture held it clear, and the probe that watched a retail entry saw no set. `P1[2]`'s `SET 0x619` sits before its first `0x21`, so `FUN_8003A1E4` runs it at each MAN-loading entry; `FUN_8003AEB0` skips the spawn loop on a same-scene reload (`0x8003B8A0`). The engine had re-run the section on every talk. |
...
| `FUN_801D1EC4` is the clip base's writer, on four arms | falsified (the pad step writes it every frame; `FUN_801D1EC4` on one arm) | Plausible: four `sw ...,-0x4228` stores sit together from `0x801D21AC`. Three are in the hop phase machine `FUN_801D2298` (`6` / `7` / `1`); the pad controller's (`0x801D0424..0x801D04A4`) and the system channel's (`0x80039D94`) were not on the list. |
...
| The play page and the native window walk `town01` along different paths | falsified (both stop at `(2588, 2898)` under one pad script) | Plausible: a native reference frame disagreed with the page. `field_walk_host_parity.rs` drives both hosts from one script to the same stop; the reference had come from a stale binary (`inference`). |
| Op `0x3E` with `op0 < 100` is the field interact that arms an actor's interaction script | falsified (it installs a scripted battle, the same body as `op0 = 0xFF`) | Plausible: `0x3E` with `op0 >= 100` is a door and `0xFF` a boss fight, so the remaining range looked like the talk case, and the pointer it stores in `sys[+0x94]` looked like a script. The arm reads `op0` only at `0x801E06FC` / `0x801E0704` to fork off the door-warp; the pointer is a MAN formation-table row, and the body ends in the reroll and the battle mode request. The engine opened dialogue at all ten non-`0xFF` sites ([settled](re-settled-threads.md#field--locomotion)). |
...
| `FUN_801D25EC`'s chained second actor is an emitter | falsified (a release watcher) | Plausible: the port named its routine `spawn_arc_with_emitter`, and a second allocation after a motion helper reads like a particle trail. The second template `0x801F22AC` carries handler `FUN_801D5D60`, which clears the halt bit `0x400` when the arc lands and optionally runs the follow-camera ease. |
| `kor5`'s P2[5] needs about 17,500 vsyncs to write `0x436` | falsified (3,336 after `0x464` clears) | Plausible: a probe run stalled that long without the write. The probe's `!464` leg counted its own pokes instead of reading the flag, so after the battle's reload it poked the trigger tile again, re-dispatched P2[4] over the running P2[5] and locked the player. |
...
| `FUN_801F1E48` is a hub sub-menu state machine | falsified (it is the Incense wear-off notice) | Plausible: it sits in the submode driver's slot table beside real menus and runs three states. Entry kind `0x0B` reaches it from the walk tick's Incense zero edge (`0x801D0CEC..0x801D0D24`), and it shows one window naming Incense and hands back ([`script-vm.md`](../subsystems/script-vm.md)). |
| The overworld runs no walk-regen tick, so Incense neither drains nor gates there | falsified | Plausible: the port's world map ran no walk tick. Retail's kingdom maps run PROT 0897's `FUN_801D1344`, which calls the walk tick at `0x801D16EC`; the `sebucus_overworld_resident` state holds the walk tick, the Incense encounter test and the notice byte-identical to the 0897 image ([`world-map.md`](../subsystems/world-map.md)). |
...
| The port's fog runs dense because the spawner compares a stale live count | falsified (the stale count was modelled; the cause was the random numbers) | Plausible: the render walk zeroes and recounts the live word, so the spawner does compare last frame's count. The excess came from handing the spawner a raw LCG state whose low nibble cycles with period 16, where retail's BIOS `rand` returns the high half. |
| `FUN_801D31B0` is shared by the dialog, cutscene and world-map overlays and reads its instruction operands | falsified (one field-overlay caller; the operands are padding) | Plausible: capture dumps labelled with those modes carried it. They are 0897-hosted modes; the one reference on the disc is the `jal` at `0x801D44C8`, and the routine overwrites `a1` at `0x801D31C4` before reading it. |
...
| `0x801D32F8` / `0x801D3444` / `0x801D3748` are move-VM extension sub-handlers | falsified (interior addresses) | Plausible: they were listed beside `FUN_801D31B0`. Every word of the jump table at `0x801CE868` lands in `0x801D3680..0x801D48FC`; none of these is one. |
...
| A fog half-sheet is a world-space quad `0x80` units tall | falsified (a view-space billboard) | Plausible: the particle has a world position and the sheet was drawn at the right place. `FUN_8003F86C` `RTPT`s its corners through a matrix whose rotation is the base matrix with `RotMatrixX(0x400)` folded in and whose translation is the particle's view point, so the sheet is screen-aligned at the particle's depth; projecting world corners foreshortened it by the pitch's cosine, about `0.84` on `map01` ([settled](re-settled-threads.md#rendering--camera)). |
| The overworld fog reads brighter than retail's because of its colour law | falsified (the colour matches packet for packet) | Plausible: a brighter frame suggests a brighter colour. Every one of the 104 fog packets in `keikoku_chest_preload`'s walked table equals the engine's `grey * tint * brightness >> 15` for its record rolled back two passes; the "72 records at median 41" figure was the pool's current ages, not the ages the drawn table was built from. The residue is draw order ([open](open-rev-eng-threads.md#battle--rendering)). |
| `FUN_800271A8` builds a depth ramp for a GTE reset | falsified (the overworld screen-Y curvature table) | Plausible: it fills a `0x4000`-entry quadratic ramp. It then `RTPS`es points along that ramp and keeps `SY - 0x78` in a second table that the overworld leaves, the fog emitter and the drop shadow add to `SY` ([settled](re-settled-threads.md#world-map--kingdom-bundles)). |
| `FUN_800460AC` is a GTE `NCDS` billboard helper, and `FUN_8001C394` draws a 2x2 gouraud grid | falsified (an `RTPT` grid projector and four textured quads) | Plausible: the pair runs GTE ops and emits a grid, and the ignore list filed them as replaced by the rasteriser. `cop2 0x280030` is `RTPT` over a 3x3 grid `0x20` apart, stored from scratchpad `0x1F800020`; `FUN_8001C394` emits `POLY_FT4` (`0x2E`) with the blob texture - the field drop shadow, which neither host drew ([settled](re-settled-threads.md#rendering--camera)). |
| `opdeene`'s black plants come from the sepia law or from retail's rewrite missing their words | falsified (a native-only restage keyed on colour) | Plausible: 1565 of the scene's prims carry colour words the grade takes to black. Retail's words are identical at the crawl's start and middle and its billboards draw at `(98, 94, 42)`; the native asset path had given the prologue's dim ambient to every `0x80`-colour vertex, authored billboard words included, and bisecting its draw list by pack slot removes every silhouette ([settled](re-settled-threads.md#rendering--camera)). |
...
| A retail battle load raises a first-chance exception in PCSX-Redux | falsified (only a patched image does) | Plausible: the runner's isolated profile zeroed the exception mask on that reading. With the mask armed on a staged image the same field-to-battle load reaches battle mode clean; the one exception on record was `w3a/registrar_sol`, whose patched `0898` jumped into an arena the retail state held as zeros. |
| PROT 0896's `jal` misses are a mis-based dump | falsified (a foreign-build link) | Plausible: the `0x801C5818` import was mis-based. Imported at its own base `0x801D4DF0` the misses remain - rebasing cannot move a decoded target - and each lands mid-body in this disc's executable ([`call-target-integrity.md`](../tooling/call-target-integrity.md#the-image-at-its-own-base-still-misses)). |
...
| ... and the fishing venue actors did reach the browser | falsified (they were native-only) | The first row of this set also said the fishing venue actors reach the browser. They did not: only the native window ticked them, and the play page gained them when the kernel moved into the engine as `fishing_venue::tick_fishing_venue_on_host`, which both play hosts now call. The Baka round chrome it listed as missing did reach both browser pages; only its `glyph_u` cell rect was native-only ([`host-drift.md`](../tooling/host-drift.md)). |
