...
| The Baka Fighter editor's actor prototype is at `0x801D7670`, its sibling at `0x801D7688` | falsified (four bytes lower) | Plausible: the callback word is what identifies a record, and stepping back to a recognisable constant lands on the `0xFFFF0000` field rather than on the head. Five `0x18`-byte records run from `0x801D7618` and end immediately before the `Vahn` name pool at `0x801D769C`, with the callback at `+0x08`; the editor's own record starts at `0x801D766C` and the mirrored sprite pass's at `0x801D7684`. The conclusion the citation supported - that the routine is linked rather than dead - is unaffected. |
