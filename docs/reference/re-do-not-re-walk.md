...
| The audio-trace oracle can converge on a mednafen save-state comparand | falsified (structurally unreachable) | Plausible: `0 converged` out of 19 looks exactly like a port that plays no music, and the engine really did have three missing scene-entry steps underneath. Fixing them does not move the line, because an SPU voice leaves ADSR `Off` on key-on and returns only when a key-off release runs to zero: a mid-playthrough freeze reports the residue of every cue since boot (15 of 19 snapshots read 24 of 24 voices non-`Off`) while a faithful engine score holds single digits, so the superset rule cannot be met. Read `0 converged` on this comparand as a statement about the comparand. |
...
| The Baka Fighter editor's actor prototype is at `0x801D7670`, or the band is five records from `0x801D7618` | falsified twice (eight, from `0x801D75DC`) | Plausible both times: the callback word identifies a record, so stepping back to a recognisable constant lands on the `0xFFFF0000` field rather than the head - and the pass that fixed that kept a band start which is itself inside a record. `0x801D7618` is the **fourth** record's `+0x0C`. Eight `0x18`-byte records `[0, 0xFFFF0000, cb, 0x00020080, 0, x]` tile `0x801D75DC..0x801D769C`, ending exactly at the `Vahn` name pool; the editor's is at `0x801D766C`, its mirrored sibling at `0x801D7684`. `FUN_80020DE0` spawns one record per site (`lw v0, 8(s1)` into `actor[+0x0C]`): `0x801CF184` takes `0x801D75DC`, `0x801D01C4` `0x801D7624`. |
...
| The last build before the draw wins, so the trio's last reader is the frame's camera | falsified (almost nothing draws under the last build) | Plausible: the last write before a read is the one a read sees, which is true of the GTE registers and says nothing about when the primitives were emitted. Ranking 31046 ordering-table links by which build was live when each was linked gives 16810 under the first, 3562 under the second and **14** under the third. The one measured two-build divergence in 749 build-carrying vsyncs was the second against the third - i.e. on the build the frame does not draw under. |
