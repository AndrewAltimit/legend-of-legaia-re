...
| `engine-core` has no host hook for field-VM op `0x34` sub-0 | falsified (the hook is live) | Plausible: the port tag said so, and a tag is the one place a reader expects the wiring answer to be. `World::op34_sub0_color_intensity_setup` is reached on both hosts. The real gap is a **representation** conflict rather than a missing call - the renderers read an `effect_tint` ramp while the op fills a push pool nothing reads - and those need different work, so a blocker naming the wrong one directs effort at a caller that already exists. |
