...
| Thread | Verdict | Why |
|---|---|---|
| Magic is forbidden on the Muscle Dome's Master course | falsified (no dome round raises the bit) | Plausible: the restriction word `0x8007BAC0` really does carry a magic-ban bit (`0x200`), the dome really does write that word, and the highest course really is the one a player would expect to be hardest. What the bytes say is that the bit's **only** writers are SCUS battle init keyed on the *first enemy monster id* - `0xAF` at `0x800519DC`, and `0x3D..=0x3F` under mode `0xC`/`0x15` at `0x8005200C` - while the dome ladder tops out at monster `0xAA`. The dome's own arena init `FUN_801D0088` writes only the word's **low byte** (the `(course << 4) + round + 1` cell), never a restriction bit. |
...
| Thread | Verdict | Why |
|---|---|---|
| A slot-B module carries un-dumped code above its frame partition | falsified (it is the spawn-record band) | Plausible: the uncovered run scores as plausible MIPS, because a record's move-VM bytecode contains a `lui $rt,0x80xx` word by accident and the `in_data_segment` test rejects any run that does. It is `[i16 model_sel][u16 flags][bytecode]` records, addressed by the consumer's own `lui`/`addiu` and handed to `FUN_80021B04` / `FUN_80050ED4` in `$a2`; 62 of 64 images carry one. |
| A slot-B image is laid out head table, then code, then data past the last function | falsified (the regions interleave) | Plausible: it is the layout a linker would produce and the one most images loosely resemble. PROT 0943 has bodies to `+0xD98`, then records, then more bodies at `+0x135C..+0x17E0`; PROT 0961 has the same shape at `+0x10DC` and `+0x1C60..+0x1D90`. Treating everything past the last found function as data swallows five real routines. |
