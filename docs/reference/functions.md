...
| `8004FCC8` | **Menu cue dispatch + voice trigger.** `(id)`. For `id < 0x100` enqueues a UI cue into the 4-entry ring at `&DAT_8007B6D8` (write index at `*(gp+0xA0C)+9`, wraps at 4): `id < 0x40` stores `id-1`, `0x40..0x100` stores `id`, both skipping the currently-selected cue `DAT_8007B724`. For `id >= 0x100` (gated on `*(gp+0xA0C)+0x276 == 0`) triggers a streamed voice via `FUN_8003D53C`: slot `= (id-0x100)>>3` (remapped 1→0x1A, 3→0x1B, 5→0x1C), sub-mode `id & 7`, pitch `= (DAT_800788B8[idx]*0x3C + 99)/100`. `see ghidra/scripts/funcs/8004fcc8.txt`. |
