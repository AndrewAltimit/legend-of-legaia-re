...
| `801D77F4` | Overlay-resident actor allocator (alt to `FUN_80021B04`). Looks up `DAT_8007C018[(i16)param_2]` for the TMD, then `_DAT_8007B7DC + (i16)param_1*4 + 4` for an entry in the VDF (case-7) buffer, then copies the vertex pool out of the TMD's sub-records into the actor's `+0x90` slot. 125 instr. |
