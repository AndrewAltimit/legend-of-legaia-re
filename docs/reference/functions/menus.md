...
| `801D1290` | **Equip stat-compare panel** (window 25). Draws the active character's name, then one stat pair or triple, each row printing the live value and - only when it differs - a rise/fall arrow plus the trial-equip value. There is **no jump table** in it; the row set comes from a three-way unsigned split on one category byte. Two early-outs precede the name draw: `DAT_801E46B0 == 0` (`beq` at `0x801D12BC`) and a resolved slot `>= 3` (`slti` at `0x801D1340`); the browsed slot's equip byte comes off a row table, row `0` from `0x8007B42C` and others via `0x801E43E8`. See [`field-menu.md`](../../subsystems/field-menu.md#equip-stat-compare-panels-windows-25-and-41). Ported: `engine-ui::equip_compare_panel_fields`. |
