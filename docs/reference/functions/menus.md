...
| `801D1290` | **Equip stat-compare panel** (window 25). Draws the active character's name, then one stat pair or triple, each row printing the live value and - only when it differs - a rise/fall arrow plus the trial-equip value. There is **no jump table** in it; the row set comes from a three-way unsigned split on one category byte, and `slti v0, s0, 4` silences it for the four gear rows, so only the three Goods rows resolve a category. Two early-outs precede the name draw: `DAT_801E46B0 == 0` (`beq` at `0x801D12BC`) and a resolved slot `>= 3` (`slti` at `0x801D1340`). The browse row is a two-table slot map - row `0` weapon, from `0x8007B42C`; rows `1`+ from `0x801E43E8`. Ported: `engine-ui::equip_compare_panel_fields`. |
