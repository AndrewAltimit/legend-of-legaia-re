...
| `801D0D38` | **Field party readout** (field overlay 0897). Its entry block `0x801D0D38..0x801D0DBC` is the whole suppress gate, four terms in order: `_DAT_8007B868`, `_DAT_800845C4 == 2`, `_DAT_8007B850 & 0xF000` and scratchpad `0x1F800394 & 0x00800000`; any one of them jumps to the epilogue. Ported as the shared predicate `engine-core::world_map_panel_host::field_hud_suppressed`. `see ghidra/scripts/funcs/overlay_0897_801d0d38.txt`. The readout's world-map-side context is on [`world-map.md`](../../subsystems/world-map.md). |
| `801D095C` | **Passive-ability badge column** - the readout's tail, not a screen of its own. One caller disc-wide, the `jal` at `0x801D130C` inside `FUN_801D0D38`, eight bytes above the epilogue the suppress arm jumps to, so the badges inherit the readout's suppression exactly and have none of their own. A host gating the two on different predicates draws the badges over dialogs, cutscenes and battles. The field-VM side of the badge column is on [`script-vm.md`](../../subsystems/script-vm.md). |
...
| Address | Role |
|---|---|
| `801D8734` | **Throw Out confirm** (pause menu, PROT 0899). The one routine on the disc that writes the item bag **without** going through the five SCUS helpers: it stores zero straight into the bag array at `0x801D88FC` and `0x801D8910`, keyed by the list kernel's selected-row payload `_DAT_8007BB88`, which it treats as a raw slot index rather than a display row. A port that removes by row agrees with it only while the shown list is the bag in order ([open](../open-rev-eng-threads.md#field--locomotion)). |
| `80042DBC` | **Seru-magic unequip** (spell-list pop): `(char_idx, spell_id, dst_slot)`. `char` is the character record `0x80084708 + char_idx*0x414` (stride `0x414`; the same bytes [`spell-table.md`](../../formats/spell-table.md) cites as `+0x705` / `+0x729` off `0x80084140`). Searches the spell list `[char + 0x13D ..]` for `spell_id`, writes the matched entry back out to the active-spell slot at `[char + 0x2B0 + dst_slot*0x14]` (word `+0x8 + i*4` split little-endian into slot bytes `+1..+4`, level byte `+0x161 + i` into slot `+5`), shifts ids / levels / words down over `i`, then `count@+0x13C -= 1`. **Quirk:** when `spell_id` is absent the search ends at `i == count`, the shift does nothing, and the count is decremented anyway - an absent-id unequip silently drops the last list entry. |
...
| `801D5510` | **Buy-quantity prompt panel.** `(window)`. Draws "Have" + the owned count of the highlighted item (`FUN_80042EE0`; the sentinel `0x100` means none held, and the panel prints " None" instead), then "How many will you buy?", then the **pair** `quantity DAT_801E46B4` and the purchase bound `DAT_801E46B8` with a separator glyph between them (`FUN_8003C1F8(6)`; the second number call is at `0x801D563C`) - not a product. The running total `qty * price` follows, its digit field width chosen by magnitude against `99` / `999` / `9999`, and a currency pictogram draws at `(WX + 0x58, WY + 0x24)`. Ported: `engine-core::shop::{shop_buy_quantity_panel, shop_total_digit_field}`. |
...
| `801DA2A0` | **Save/menu sub-screen `0x15`** - one body serving three per-character lists (ability bitfield `+0xF4` population count, Ra-Seru-gated spell count `+0x13C`, a byte at `+0x185`), selected by the step counter. Its browse half `0x801DA6D0..0x801DA9CC` is a clamping picker with seven-row page steps, the two-press exchange latch (bit `0x1000` of the second cursor word = nothing latched) and a two-stage cancel that drops a pending latch before leaving. The "debug-editor page-navigation SM" label is falsified. Kernels ported as `engine-core::save_subscreen::sub15_*`; see [`save-screen.md`](../../subsystems/save-screen.md#sub-screen-0x15---the-per-character-list-screen-fun_801da2a0). |
...
| `801D1290` | **Equip stat-compare panel** (window 25). Draws the active character's name, then one stat pair or triple, each row printing the live value and - only when it differs - a rise/fall arrow plus the trial-equip value. There is **no jump table** in it; the row set comes from a three-way unsigned split on one category byte, and `slti v0, s0, 4` silences it for the four gear rows, so only the three Goods rows resolve a category. Two early-outs precede the name draw: `DAT_801E46B0 == 0` (`beq` at `0x801D12BC`) and a resolved slot `>= 3` (`slti` at `0x801D1340`). The browse row is a two-table slot map - row `0` weapon, from `0x8007B42C`; rows `1`+ from `0x801E43E8`. Ported: `engine-ui::equip_compare_panel_fields`. Write-up: [`field-menu.md`](../../subsystems/field-menu.md). |
