...
| `8001ADA4` | **Per-actor RENDER dispatcher** (2456 bytes). Switch on `actor[+0x56]` (render mode 1..0xB). Case 4 (multi-target) dispatches on `actor[+0x9e]`: bit `0x4000` -> `FUN_8002A5A4`, bit `0x2000` -> `FUN_801CFA48` (overlay-resident), else `FUN_80028158`. Case 5 (full TMD) iterates the mesh chain at `actor[+0x44]` and calls `FUN_80043390` (textured) / `FUN_80029888` (env-mapped, `actor[+0x7a] != 0`) / `FUN_8002735C` (bone-animated). Case `0xB` is the ocean CLUT-walk emitter: `acc += DAT_1F800393` into `actor[+0x68]`, and on `acc >= hold` it fires the 16x1 `MoveImage` for the actor's kingdom-slot-5 entry and advances the frame index. Called 6x per frame through `FUN_8001D140`, over the same actor lists as the tick pass. [details ↓](#8001ada4-writes-the-gte-light-matrix-inline) |
