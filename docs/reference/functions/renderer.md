...
| `8003F86C` | **Field-fog sheet emitter.** Allocates `0x28` bytes (a ten-word packet) off the scratchpad primitive cursor `[0x1F800314]+0x8C`, projects the sheet's four corners with `RTPT` (`cop2 0x280030`), culls on the transformed X/Y bounds, copies the caller's four UV words into the packet's `+0x0C` / `+0x14` / `+0x1C` / `+0x24` slots, and links it with a tag whose top byte is `0x09` - nine body words, a `POLY_FT4`. Semi-transparent, texture page `0x27` (VRAM `(448, 256)`, 4bpp), CLUT `0x7640`, OT slot `SZ2 >> 5`. Returns `1` on emit / `0` on cull. Saves and restores `s0..s7` through the scratchpad slots at `0x1F800000..0x1F80001C`. |
