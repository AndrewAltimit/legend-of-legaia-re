...
| Address | Role |
|---|---|
| `801F69EC` (PROT 0900, file `+0x14`, 860 B) | **Depth-cued ground-cell emitter.** Per visible cell it runs `GTE.dpcs` at `0x801F6C44` and writes the depth-cued colour register into the packet with `swc2 $22,4($t5)` at `0x801F6C4C`. Gate `cell & 0x1000`. |
| `801F6D48` (PROT 0900, file `+0x370`, 832 B) | **Flat sibling of the row above** - the same loop with those two instructions replaced by a plain `sw $s2,4($t5)` at `0x801F6F88`, so no depth cue is applied. The selector at `0x801F79A0` picks between them on `_DAT_8007BB4C`, calling `SetFarColor` first on the non-zero arm. They are a pair, not one emitter shipped twice. |
| `8002735C` | Legaia TMD renderer. 60 GTE ops; per-mode descriptor table at `DAT_8007326C`. **Gated, and cold in everything sampled.** Each of its three SCUS `jal` sites (`0x8001B594`, `0x8001BD88`, `0x80048FE4`) is the far arm of an `lh`/`lhu r, 0x42(s0)` then `bne` pair; the near arm takes `FUN_80029888` when `actor[+0x7A]` is non-zero and `FUN_80043390` otherwise. A census over 720 vsyncs of four states and three game modes reads the gate 5089 times, always zero, and enters this routine zero times against 10621 entries into `FUN_80043390` - so the measured mesh leaf is the per-prim one. [details](../re-settled-threads.md#rendering--camera) |
...
| `8001ADA4` | **Per-actor RENDER dispatcher** (2456 bytes). Switch on `actor[+0x56]` (render mode 1..0xB). Case 4 (multi-target) dispatches on `actor[+0x9e]`: bit `0x4000` -> `FUN_8002A5A4`, bit `0x2000` -> `FUN_801CFA48` (overlay-resident), else `FUN_80028158`. Case 5 (full TMD) iterates the mesh chain at `actor[+0x44]` and forks on `actor[+0x42]` (`bne` at `0x8001B454`): the far arm is `FUN_8002735C`, the near one `FUN_80029888` when `actor[+0x7A] != 0` and `FUN_80043390` otherwise. Case `0xB` is the ocean CLUT-walk emitter. Called 6x per frame through `FUN_8001D140`. [details](#8001ada4-writes-the-gte-light-matrix-inline) |
| `80017EC8` | **Tile-window background render pass.** `(cx, cz, sx, sz)`. Sets the scroll-origin globals `_DAT_1F8003F8 = sx` / `_DAT_1F8003FA = sz`, refreshes the per-tile region-attribute mask via `FUN_800180EC`, then walks a tile window centred on `(cx, cz)` (`cx - 0xF + i`, `cz - 0xD + j`) emitting one per-cell background draw each. The engine renders field/board backgrounds through its own wgpu path, so this per-frame GP0 tile emission is not ported - scope row `[render_pipeline]` in `scripts/ci/port-catalog-ignore.toml`, not a worklist gap. `see ghidra/scripts/funcs/80017ec8.txt`. |
...
| `800460AC` | **Drop-shadow grid projector.** Three `RTPT`s (cop2 `0x280030`, not `NCDS`) over rows `z + 0x20`, `z`, `z - 0x20` by columns `x - 0x20`, `x`, `x + 0x20`, all at the actor's own `y`; writes `[SXY, SZ]` pairs from scratchpad `0x1F800020`, `0x18` bytes per row, for `FUN_8001C394`. Port `engine-core::drop_shadow::shadow_grid`, live on both hosts. See [`renderer.md`](../../subsystems/renderer.md#the-field-drop-shadow-fun_8001c394). |
...
| `801D8280` | `DAT_8007C018` table walker (overlay-resident, in every world-map / cutscene-mapview / 0897 overlay variant). Iterates entries `0..=DAT_8007BB38` - the bound is `DAT_8007BB38 + 1` because the global holds the **last issued** id, not a count - and for each pointed-to TMD calls `FUN_801D5E20` on each 0x1C-byte sub-record. 55 instr. |
...
| `8001B964` | **Per-actor animated character-mesh renderer.** `(actor)`. Builds the actor's GTE transform from its record (Euler→matrix `FUN_80026988` over `actor+0x24`), selects the camera-relative rotation path (`FUN_8001CF50` when `actor+0x52 & 0x780`, else the plain matrix-vector `FUN_8005B3A8`), then walks the bones decoding each per-(bone, frame) entry via `FUN_8001BE80` and emits the assembled skinned mesh to the OT. The runtime consumer of the `legaia_asset::player_anm` layout; the from-scratch engine reproduces it via wgpu + the ported `BoneTransform::decode` (`FUN_8001BE80`). `see ghidra/scripts/funcs/8001b964.txt`. |
| `8001BE80` | **Per-part pose decoder** for the animated renderer. Unpacks an 8-byte clip entry's three 12-bit signed translations (`0x8001BF04..0x8001BF6C`) into `0x1F8002C0` and `MVMVA`s them (`0x8001C0E0`); decodes three `byte << 4` rotation angles (`0x8001C0CC..0x8001C0FC`); blends against the next frame through `FUN_8001D088` when `clip.flags & 1`. Entries are frame-major: `entry(f, p) = body + 8 + (f * part_count + p) * 8`. |
| `8001C204` | **World-map object-effect GTE transform builder.** `(actor)`. Composes the scratchpad working transform at `0x1F800314` for object-effect kind `*(actor+0x42) - 1`: a per-type param table at `0x80083FF8` (stride `0x14`; rotation angles at `+0/+2/+4`, extra u16s copied to `0x1F800380/382` from `+0x10/+0x12`) is rotated (`FUN_800461A4/629C/638C` = per-axis RotMatrix) and composed with the actor's own `+0x24` Euler and `+0x14` position through the GTE matrix ops `FUN_8003D20C/D1A4/D344`; the resulting translation lands at `0x1F800328/32C/330`. Reached from the world-map object dispatch (see [`world-map.md`](../../subsystems/world-map.md)). Render-track (GTE); replaced by the engine's own wgpu transform - scope row `[libgte]`. `see ghidra/scripts/funcs/8001c204.txt`. |
| `8001C394` | **Field drop shadow.** `(actor)`. Four `POLY_FT4` cells (packet tag `0x09000000`, command word `0x2E808080`, tpage `0x1F`, CLUT `0x7F86`, one 8x8 cell each of the 16x16 blob at u `0xE0`) over the grid `FUN_800460AC(actor+0x14)` projects - textured quads, not a gouraud grid. Linked into the scratchpad OT `_DAT_1F8003F4` at `((ΣSZ + 0xA0) >> 4) >> (DAT_1F8003A4 & 0x1F)`, ten slots nearer when `*(actor+0x10) & 0x800000`; on the overworld (`_DAT_1F800394 & 1`) each `SY` takes the curvature entry `_DAT_8007BB04[(ΣSZ >> 2) >> 5]`, with no `+1`. Caller `FUN_8001B964`, gated on flags `0x01020000` in and `0x200000` out. Port `engine-core::drop_shadow`, live on both hosts. `see ghidra/scripts/funcs/8001c394.txt`. |
| `800271A8` | **Overworld screen-Y curvature table builder** - not a depth-ramp / GTE-reset init. Called `(0x28, 0x2AB980)` by `FUN_8003AEB0` under `_DAT_8007B6A8`; fills `0x8007BB08` with a quadratic drop, then `RTPS`es points along it at `H = 0x3C0` into the `SY - 0x78` table at `0x8007BB04`; sets `_DAT_1F800394` bit 0. Port `engine-core::overworld_curvature`. See [memory map](../memory-map.md) and [`renderer.md`](../../subsystems/renderer.md#frame-setup--present). |
...
| `80031D00` | Per-frame **retained-widget** tick, and the SCUS driver of the window emitter `FUN_8002C69C`. Walks the actor list at `gp[+0x148]` and dispatches on `actor[+0x1C]` alone: cases 0/1/D/11 render text via `FUN_80036888`/`FUN_8003CC98`, cases 4/6/C/21 hand off to sub-routines, and `jal 0x800323E4` reaches the nine-slice window frame. It runs every frame a battle is up, which is why PROT 0898 carrying no `jal` to `FUN_8002C69C` does not mean the battle report windows are drawn some other way ([falsified](../re-do-not-re-walk.md#menus--ui)). Kind `0xD` - the battle tutorial's wait - has no draw arm. |
...
| `8003E8A8` | PROT-by-index size lookup. Reads `start_lba = PROT_TOC[p+2]` and `next_lba = PROT_TOC[p+3]` (TOC base `0x801C70F0`) and returns `next_lba - start_lba` (LBA count for the entry). Also stows `start_lba` at `gp[+0x8F0]` and the entry index at `gp[+0x90C]` so the matching `FUN_8003E800` read can pick them up. **Its `p` is the runtime index**, which is the extraction index `+ 2`, because `0x801C70F0` holds `PROT.DAT` from byte 0 with the header included; [`prot.md`](../../formats/prot.md)'s identically-shaped `toc[p+2]` uses the extraction index instead ([settled](../re-settled-threads.md#measurement--corpus)). |
...
| `800172C0` | **Field view-matrix builder** - turns the camera globals into the GTE's `R` and `TR`, two to five times per field frame from five sites, two of them in PROT 0901's `FUN_801F73E4` ([census](../../subsystems/cutscene.md#what-a-build-census-actually-measures)). Five calls: the light angles via `FUN_80026988` + `FUN_8005B648`; `Rot(pitch, yaw, roll)` from `0x8007B790`; `FUN_8005B3A8` folding the per-mode base matrix `_DAT_8007BF10` (a field state holds `24576 * I`); `FUN_8005B4B8` copying the live eye trio `0x800840B8/BC/C0` in as 32-bit words, then `FUN_8003D344` MVMVAing the focus through with `cv = TR`; and `FUN_8005B6A8` uploading the result. The staging descriptor is not in the chain. [details](../../subsystems/renderer.md#the-field-view-matrix-where-tr-comes-from). |
| `80026F50` | **Sibling view build for the non-field modes** (one caller, `0x80026DE0`). Same five-call shape as `FUN_800172C0` over the same globals, and three things separate them: it folds the ROM-constant base matrix at `0x80010B84` (`16384 * I`, a 4x scale) instead of the live one, it copies the eye trio's **low halfwords** sign-extended rather than the full words, and it runs no focus `MVMVA`. Zero entries across three field runs of 719 vsyncs each, which is what identified the field builder as the other one. |
...
| `0x801DDE24` / `0x80025034` (return addresses) | The two `FUN_80024EE4` push sites a screen-effect capture sees: `0x801DDE24` is `FUN_801DDC20`'s push (field overlay, `jal` at `0x801DDE1C`), `0x80025034` is `FUN_80025000`'s (SCUS, `jal` at `0x8002502C`). Across a `conc` -> `conc2` door the first carries the departure's two concurrent channels - OT bucket `2` an ambient `0x00003030` decaying, bucket `0` a white-out to `0xFFFFFF` - and the second the arrival, bucket `1`, `0xEEEEEE` falling to `0`: the first argument is a channel selector with two callers. |
...
| `80035CB8` / `80035DA0` / `80035E44` | Text-actor sub-handlers. Children of the per-frame text-actor tick (`FUN_80031D00`). Each counts a row via `FUN_80036044` and renders via `FUN_8003CC98`. `_DA0` resolves a magic-name string from `PTR_DAT_80075DB0` keyed by the `0x800754CC + idx*0xC` magic table; `_CB8` advances state at gp `+0x87c` / `+0x13c`. |
...
| `8003F348` | **Field-fog pool walk.** Loads the pool pointer from `_DAT_8007B7E0`, takes the record count from its `+0x02` halfword and strides `0x18` from `+0xA4`, running `FUN_8003F3FC` on every record whose `+0x05` byte is non-zero and summing each call's return into the draw-cost accumulator `gp[+0x990]` (zeroed at entry). Brackets the walk with the GTE control save/restore pair and the scratchpad `TR` load. Port `engine_core::fog_particles`. `see ghidra/scripts/funcs/8003f348.txt`. |
| `8003F3FC` | **Per-fog-record update and draw.** Advances one `0x18`-byte record (drift by the region's angle / spread / speed, life decay), clip-tests it against the walk-region box at `_DAT_1F800384..387` and the player at `_DAT_8007C364+0x14/+0x18`, then forms the packet's code word as `0x2E000000 \| colour` (`lui s2,0x2e00` at `0x8003F77C`) and hands it to `FUN_8003F86C`. The width jitter through `FUN_8003F838` is dead code - the caller has already seeded the value with the rate. It emits **textured quads, not line packets**: the `0x09000000` an earlier row read as a GP0 command is the ordering-table tag's word count. |
| `8003F838` | Fog width-jitter step - a thirteen-instruction LCG (`seed = seed * 12 + 2`, byte-swap) over scratchpad state at `_DAT_1F8002A8`. **Dead in retail**: its one caller overwrites the value it returns into with the rate before using it, so the jitter never reaches a packet. |
| `8003F86C` | **Field-fog sheet emitter.** Allocates `0x28` bytes (a ten-word packet) off the scratchpad primitive cursor `[0x1F800314]+0x8C`, projects the sheet's four corners with `RTPT` (`cop2 0x280030`), culls on the transformed X/Y bounds, copies the caller's four UV words into the packet's `+0x0C` / `+0x14` / `+0x1C` / `+0x24` slots, and links it with a tag whose top byte is `0x09` - nine body words, a `POLY_FT4`. Semi-transparent, texture page `0x27` (VRAM `(448, 0)`, 4bpp: page-x field `7`, page-y bit `0`, ABR `1` - an earlier row read the page as `(448, 256)`, which the page-y bit does not select), CLUT `0x7640`, OT slot `SZ2 >> 5`. Returns `1` on emit / `0` on cull. Saves and restores `s0..s7` through the scratchpad slots at `0x1F800000..0x1F80001C`. |
| `801D629C` | **Field-fog spawner** (field overlay, PROT 0897 file `0x7A84`). Maps the player's tile to a MAN section-4 fog-region record - `[enable][x0][z0][x1][z1][angle][spread][speed][unread][u16 flag]`, eleven bytes - and pops one of the eighty `0x18`-byte records from the pool at `_DAT_8007B7E0`, seeding it from the region's angle, spread and speed. It is the producer for `FUN_8003F348`'s walk, not a per-particle update. The dump filed under this VA from the 0896 image is a fragment of a different routine, with no prologue ([aliases](../overlay-va-aliases.md)). |
| `801D6058` | **Field ambient-effect emitter** (field overlay, PROT 0897). Its **first two instructions** load the ambient-particle master gate `_DAT_8007B854` (`lui v0,0x8008` / `lw v0,-0x47ac(v0)` at `0x801D6058`), so the whole body is script-gated: field-VM op `0x4C` nibble 3 sub-0 raises the word and sub-1 clears it. Spawned per scene as the `+0x08` handler of the one plain actor template at `0x801F271C`. The bare `801d6058.txt` dump is **not** this routine - it is the menu overlay's `FUN_801D603C` printed from an interior address. |
...
| `8005B0B8` | **`SquareRoot0`** - the PsyQ-shaped integer square root the camera composer and the field distance helpers use. Seeds GTE data register 30 with the argument (`mtc2 a0,0xf000`), reads the leading-zero count back off register 31, normalises the argument into `0x40..0xC0` and indexes the 192-entry mantissa table at `0x80078E84` (`lh t5,-0x717c(t5)` at `0x8005B11C`), then shifts by half the exponent. Result is `sqrt(x)` in q6. The earlier "shift-converter for texture / colour bit packing" reading named no operand of it. |
...
| `80046870` | **Incense window top-up** - adds `0x40` to `gp+0x2E8` (`_DAT_8007B600`) and caps it at `0x100`; the class-`0x82` arm of `FUN_800402F4` (`jal` at `0x800421A0`). The walk tick drains the window and the region encounter roll skips while it is non-zero - not a brightness or fade ramp. Port `engine-vm::battle_helpers::top_up_cooldown`, wired on both hosts. `see ghidra/scripts/funcs/80046870.txt`. |
...
| Address | Role |
|---|---|
| `80024CFC` | `play_anm_by_id(id, actor, ?)` - allocates an actor (via `FUN_80020DE0`), reads the per-record offset from `_DAT_8007B7C8 + (id*4) + 4` (the kingdom slot-5 CLUT-walk table installed by `FUN_8001F05C` case 6; parser `legaia_asset::clut_walk`), and stores `(table_base + record_offset)` in `actor[+0x4C]`. Writes `0xB` to `actor[+0x56]` (render mode) and `100` to `actor[+0x68]` (accumulator seed, `>=` any hold so the first copy fires at scene entry). The per-frame walk is `FUN_8001ADA4` case `0xB`; spawner = field-init `FUN_801D6704`, one actor per table entry. Its only caller, the field init's spawn loop, runs only while bit `0x20` of the `FUN_80020224` summary word is set. |
...
| `80036044` | Typewriter glyph count (not a pixel width). Same byte classification as the stride walker plus substitution dispatch on `(byte + 0x40) < 8` (catches `0xC0..0xC7`); the explicit cases `0xC1..0xC5` and `0xC7` add the length of the substituted string. The pixel measurer is `FUN_80035F04`. See [`dialog-font.md`](../../formats/dialog-font.md#line-width-and-wrapping). |
| `80036888` | Text renderer. Expands substitutions with `FUN_80036514` into the buffer at `0x800740EC`, then walks the result emitting one glyph sprite per byte at `widths[c] + DAT_800740E8 + 1` advance; no wrap, no clip. Third argument caps the glyph count drawn (typewriter reveal); returns the count drawn. |
...
### `8001ADA4` writes the GTE light matrix inline

The dispatcher's case-5 arm stages GTE control registers 8..12 - the light
matrix - directly, with `ctc2` at `0x8001B34C..0x8001B368`, rather than through
either of the SCUS control-word helpers. No routine has been found that reads
the block back on the frames that follow, so the write is either consumed by a
leaf whose setup is not visible as a call or is vestigial. Recorded as an
...
ride beside the yaw a reader taking the halfword alone sees.
...
Global TMD pointer table. Installed by `FUN_80026B4C` (asset-dispatcher case 2 per-TMD; `sw a0, 0(v1)` where `v1 = lui+addiu(0x8007C018) + idx*4` - Ghidra's static-xref misses the store because the intermediate `addu` defeats constant propagation). Counters: `DAT_8007B774` is the **next free** id and `DAT_8007BB38` is the **id just issued**.

The registrar stores the pointer at `DAT_8007B774 * 4`, publishes the same `n` through `gp[+0x820]` at `0x80026BBC`, and only then writes `n + 1` back to `DAT_8007B774` (`0x80026BCC`). So `DAT_8007BB38` is a last-index, not a count - it reads `DAT_8007B774 - 1` - and every walker adds the `1` back: `FUN_801D8280` bounds its loop with `addiu v0,v0,1` at `0x801D8320` and iterates `0..=DAT_8007BB38`. Each entry points to a TMD blob with magic `0x80000002`; `+0x8` is `group_count`, `+0xC..` is the `count × 0x1C-byte` group descriptors. Consumed by `FUN_80021B04` (actor allocator), `FUN_801D77F4` (overlay actor allocator + vertex copy), `FUN_801D8280` (table walker), `FUN_801F69D8` (world-map top-view tile dispatcher), `FUN_8001E890` (per-pack count override).
...
Overlay-resident actor allocator (alt to `FUN_80021B04`). Script-VM `4C D8` host hook (9-byte opcode). Takes `(vdf_idx: i16, tmd_idx: i16, kind: u16, variant: u16)`. Allocates actor slot via `FUN_80020DE0(0x8007068C, _DAT_8007C34C)`; resolves TMD from `DAT_8007C018[(i16)tmd_idx]` - an index the op's own arm has already offset by the scene-bank base `*(u16 *)0x8007B6F8` (`0x801E2DE0..0x801E2DE8`), so `tmd_idx` names the scene's `n`th model - and VDF body from `_DAT_8007B7DC + body_offsets[(i16)vdf_idx]`.
Two-pass vertex-pool build: sum `TMD_groups[record.idx].vertex_count * 8` into `_DAT_8007BA74`, malloc via `FUN_80017888`, then copy each referenced group's vertices into the pool. Populates `actor[+0x3C]=kind, [+0x3E]=variant, [+0x48]=TMD_ptr, [+0x4C]=VDF_body_ptr, [+0x90]=vertex_pool` (and zeros `+0x56/+0x5C/+0x68/+0x6E`).
...
The tail is ported as `World::spawn_morph_weight_actor` (the rest-pose
snapshot) and the pool envelope steps the weight on both hosts. The routine and
the apply pass walk the morph block with three record pitches - `0xC`
(`0x801D78D0..0x801D7900`), `n_vert * 8` (`0x801D792C..0x801D799C`) and
`n_vert * 0x60` - which agree only on a one-record block, the only shape the disc
ships ([`script-vm-menuctrl.md`](../../subsystems/script-vm-menuctrl.md#three-record-pitches-over-one-morph-block)).
