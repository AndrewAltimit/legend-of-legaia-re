...
| `80025000` | **SCUS fade-actor tick** - the pool actor's `+0x0C` handler, and the SCUS counterpart of `801DDC20`. Steps `FUN_80020C14` and, unless it returned `-1`, pushes the quad through `FUN_80024EE4(block[+0x22], block[+0x18], rgb)` - i.e. the id `FUN_80024E80` stamped into the template's last word, and the fade kind. Its address is the `+0x0C` tick word of two static actor templates (`0x8007067C`, `0x800706AC`). [Details ↓](#80020c14--80025000). `see ghidra/scripts/funcs/80025000.txt`. |
