...
| `80056798` | **BIOS `rand`** - `li t2,0xA0; jr t2; li t1,0x2F`, the `A(2Fh)` vector. Returns `(seed >> 16) & 0x7FFF`, the **high** half of the kernel LCG, which is why callers may test its low bits (`rand & 0xF`, `& 7`, `& 0x7F`). A port that hands those tests a raw 32-bit LCG state gets a low nibble of period 16; the fog spawner did (`bios_rand_shape` fixes that caller). About sixteen hundred `jal` sites in the dump corpus. |
