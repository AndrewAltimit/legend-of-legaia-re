...
| `801D8450` | **Side-band stream teardown** - the body of field-VM op `0x36` sub `3`. `FUN_800653C8(0x17)` then `(0x16)`, `FUN_8001FF58(6)` to release SEQ slot 6, then `_DAT_8007BA88 = 0` and `_DAT_8007BAFC = 0`. Ungated, and it *yields the frame* rather than falling through - the one arm of that sub-switch the request/acknowledge pair does not gate. See [`script-vm.md`](../../subsystems/script-vm.md#overlay-0897-command--submenu-support-functions). |
...
| `801D1EC4` | **Warp reposition animator + tile-crossing trigger handler** (field overlay). While the warp timer `_DAT_8007B6B0 > 0`, steps the warped actor toward `(_DAT_8007BDD0, _DAT_8007BDD4)`, re-sampling the floor (`FUN_80019278`); when it runs out, the landing arm re-rolls the encounter counter through `FUN_801DDF48` if it is `<= 0` (`0x801D1F6C`). When idle, compares the actor tile against its last-tile mirror `(_DAT_8007BDC8, _DAT_8007BDCC)` - not `0x8007BDD8`, the field clip base, which it sets to `2` after a trigger when `0x8007B6A8` is set (`0x801D21AC`) - runs the `cell & 0x600` filter, posts `FUN_8003BDE0` and arms a `0x26`-frame warp via `FUN_801D58F0`. See [`field-locomotion.md`](../../subsystems/field-locomotion.md#trigger-block-0x10000---four-kind-sub-tables). |
