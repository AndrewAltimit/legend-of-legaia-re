...
| Address | Role |
|---|---|
| `801CEC94` | **Slot-machine init** (PROT 0975). Loads the cabinet bundle `FUN_8003EB98(0x4B2, *(0x8007B85C), 1)` at `0x801CEE2C`, dispatches all three PROT 1200 descriptors through `FUN_80020224(0)` at `0x801CEE44` (the second is the cabinet's untextured TMD, bound into the model bank), and spawns the cabinet actor with `FUN_80020DE0(0x801D3618, *(0x8007C34C))` at `0x801CEEA8` after writing model slot 0 into `template+4`. The cabinet is bound geometry drawn by the shared TMD renderer, which is why no slot function emits it. |
...
| `801D03C4` | **Dance** 5-arm mode helper (PROT 0980, 500 B; `sltiu s0, 5` at `0x801D0444`), two `FUN_80058490` calls per arm. Nine in-image callers, five of them from the bring-up above. `overlay_dance_0980_801d03c4.txt`. |
| `801D0640` | **Dance** frame-delta leaf (PROT 0980, 272 B): scales by `DAT_1F800393` and hands off to the move-buffer consumer `FUN_800204F8` and `FUN_80024E08`. Frameless, so analysis never carved it out. `overlay_dance_0980_801d0640.txt`. |
...
| Address | Role |
|---|---|
| `801F6B24` | **Staged background loader** - the whole of PROT 0978 (`field_back_read`, dev module `other6`), which holds exactly **one** prologue and **one** `jr ra`, so the image is this single 2100-byte routine over `0x801F6B24..0x801F7358` plus a data tail. Two switches (`sltiu` bounds `0xC` at `0x801F6BBC` and `0x13` at `0x801F6EDC`) drive a staged read: nine `FUN_8003E8A8` TOC resolves, nine `FUN_8003E800` sector reads, eight `FUN_8003E9C0` and seven `FUN_8003E964` seeks, plus eight `FUN_800583C8` calls. Slot-B occupant at base `0x801F69D8`, so it VA-aliases PROT 0900 and the summon modules - read the qualified dump `overlay_field_back_read_0978_801f6b24.txt`. |
