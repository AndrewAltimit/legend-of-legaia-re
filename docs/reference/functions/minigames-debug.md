...
| `801D14B0` | **Step-size scaler** - a leaf: returns the argument unchanged while flag `DAT_801D1AB4` is set, else `arg/5` (`arg > 5`, via the `0x66666667` magic multiply), `1` (`arg < 3`), or `arg/2`. The **same 24-instruction routine is linked into PROT 0976 as `FUN_801D6710`** (the Baka tally drain), where only the gate global is relocated - `lui v0,0x801e; lw v0,-0x4100(v0)` reads `_DAT_801DBF00` there against this image's `_DAT_801D1AB4`, and the other 23 instructions match one for one. So the two addresses are one source routine in two overlays, not two behaviours; a byte compare at the two entries reports **zero** common bytes because the relocated pair is the first instruction. `see ghidra/scripts/funcs/overlay_0977_other_game_801d14b0.txt`. |
