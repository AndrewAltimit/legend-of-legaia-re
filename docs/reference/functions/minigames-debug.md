...
| `801D00F8` | **Arena backdrop**, `(brightness)` clamped into `0..=0xFF`. On `*(0x801D1AE0) == 0` it sets the OT depth `*(0x801D1AA8) = 0x3E8` and lays a 3x2 grid of record-2 quads through `FUN_801D08EC` at `(i<<7, j<<7)`, then the full-screen `POLY_G4` gradient `FUN_801D1610(0, 0, 0x140, 0xF0)`; otherwise it hand-writes two **`POLY_FT4`** packets (code `0x2C`; an earlier row said `POLY_GT4`) - the dome **panel still**, tpages `0x106` / `0x109`, `a0` broadcast into all three colour lanes as a fade, and the fork `*(0x801D1AE0)` raised by the arena init on **re-entry** only. Packet-only - filed `[prim_builder]` in the ignore list; geometry on [`ringside-still.md`](../../formats/ringside-still.md#what-draws-it). `overlay_0977_slotA_801d00f8.txt`. |
