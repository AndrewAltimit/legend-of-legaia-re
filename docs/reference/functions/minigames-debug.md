...
| `801D26CC` | **Fishing bite tick** (fishing overlay 0972). Runs the bite ladder and two per-frame map reads of the lure's own point: the `+0x8000` cell halfword's `0x4000` water gate (feeding the credit) and `FUN_801D7030`'s walk-grid probe (`jal` at `0x801D2E10`), whose hit drifts the lure instead. It is also the **only** writer of the persistent cast counter `_DAT_80084460`, reached through the save-block base rather than its own address - `t0 = 0x80084140`, `lw` / `addiu` / `sw 0x320(t0)` at `0x801D2954..0x801D296C` - on the hook arm that raises cue `0x204` and sets SM state `0x19`, so it steps once per hooked fish. See [`minigame-fishing.md`](../../subsystems/minigame-fishing.md). |
| `801D7030` | **Walkability-grid wall probe, high nibble** (fishing overlay 0972). `(x, z) -> bool` against `*(_DAT_1F8003EC) + 0x4000` - the same per-scene grid `FUN_801CFE4C` reads (see [`field-locomotion.md`](../../subsystems/field-locomotion.md)), but taking the byte's **high** nibble, with asymmetric coordinate ladders. Leaf, no frame. Port `engine-core::fishing_actors::walk_grid_overhead`. Its one consumer is the bite tick `FUN_801D26CC` (`jal` at `0x801D2E10`), where a hit **drifts the lure** rather than gating water: `0x801D2E34..0x801D2E58` step the 24.8 `x` accumulator `0x801D9174` by `frame_delta << 11`, signed by the lifetime cast counter's low bit. `overlay_fishing_801d7030.txt`. |
