...
| `801D7030` | **Walkability-grid wall probe, high nibble** (fishing overlay 0972). `(x, z) -> bool` against `*(_DAT_1F8003EC) + 0x4000` - the same per-scene grid `FUN_801CFE4C` reads (see [`field-locomotion.md`](../../subsystems/field-locomotion.md)), but taking the byte's **high** nibble rather than its low one. Leaf, no frame. `overlay_fishing_801d7030.txt`. |
| `801D765C` | **Tile distance between two tracked points** (fishing overlay 0972). No arguments; reads two `i16` pairs at `0x801D9184` (`+0` = x, `+4` = y) and `0x801D918C`, normalises the squared distance through `FUN_8005AF0C`, `>> 6` into grid tiles, clamps negatives to zero. `overlay_fishing_801d765c.txt`. |
