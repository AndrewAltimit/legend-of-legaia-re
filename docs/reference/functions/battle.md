...
| `801F3990` | **Cast audio-cue dispatcher** (PROT 0898 file `0x25178`). One `jal` disc-wide, at `0x801E3E04`, inside action-SM table slot `0x3D` - the Spirit / Item wait state, which an **item** use opens and a player Seru cast cannot ([details](../../subsystems/battle-action.md#the-one-caller-is-state-0x3d-and-it-is-an-item--spirit-state)). Reads `ctx[+0x13]` and the char-kind table `0x8007BD10`, dispatches on `actor[+0x1E8]`, plays the per-class cues via `FUN_8004FCC8` (enemy leg `0x20C..0x20E`, player leg `char_kind*0x10 + 0xF8..0xFC`; port `legaia_engine_vm::battle_cast_cue::cast_audio_cue`). Does **not** read the move-power table - that was a mis-based dump's address space ([move-power.md](../../formats/move-power.md#0x801f3990-is-a-real-function-and-not-a-consumer-of-this-table)). |
