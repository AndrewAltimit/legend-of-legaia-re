...
| `8004FE5C` | **Battle SFX-cue router.** `(id, category)`. Resolves a cue id into the 4-slot pending-SFX ring `DAT_8007B6D8` (drained by `FUN_80016B6C`): `< 0x48` enqueues `id-1` (element-tinted `id+0x281` for a non-party attacker at `id >= 0x1B`), `0x48..0x63` raw, `>= 0x64` the tinted `id+0x19C` - `*(0x801C9370[cat]+0x22C)+0x80` (an inferred "attacker's element") lands in byte `+4` = the **category** column of `bse.dat` record `[cue-0x200]` (`_DAT_8007B990` = `gp+0x678`). Dedupes vs `DAT_8007B724`. `id >= 0x100` from a **party** attacker instead starts the arts-voice CD-XA clip (`FUN_8003D53C`; clip `(id-0x100)>>3` remap 1/3/5 → 26/27/28, channel `&7`, duration table `0x800788B8`), gated on the tutorial byte + read-idle poll `FUN_8003DE7C(1)`. Ported as `legaia_engine_core::sfx_cue`. |
