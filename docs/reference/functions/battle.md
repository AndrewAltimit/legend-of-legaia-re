...
| Trampolines (six) | A capture-class module's `Tick` column below is usually a **trampoline**, not the choreography: `0x801F7A40` / `0x801F816C` / `0x801F7B28` / `0x801F92A4` / `0x801F8E60` / `0x801F7B1C` re-read `caster[+0x1DF]` and pick a body per spell id. Per-id map: [`cast-module.md`](../../subsystems/cast-module.md#the-trampolines-are-their-own-port-and-one-cell-holds-six-spells). |
