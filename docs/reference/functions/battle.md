...
| `801D5718` | **Screen-element "land" mover** `(dst_index, src_index)` on the `0x80076C10` table - third sibling of `801D5778` / `801D57E8`: copies `src+0x0A/+0x0C` into `dst`'s seat A *and* `dst+0x0A`, plus width and string. Called from `FUN_801D388C` at `0x801D4458`. The record is a two-seat pair ([`memory-map.md`](../memory-map.md#0x80076c10---one-table-three-names)); `FUN_801D5854` performs the record-41/42 shift inline at `0x801D5B08` / `0x801D5C58` (42 inherits 41's string, width and seat-B x; 41 takes `actor+0x1BC`, is measured by `FUN_80035F04`, and is re-seated at centre x 232, right clamp 304, park at 328 or beyond). |
...
| `801F69F4` | **Boss stage-module tick entry** (PROT 0968, the evolved-Cort stage). 7-word jump table at `0x801F69D8` on `ctx[+0x289]`; re-seeds `ctx[+0x6D6] = 0x100` per tick; called from SCUS `0x800564A0`. Phase 0 waits on camera word `0x800840BC` passing `0xC00`; a later arm spawns its own centred banner via `FUN_8003541C` (`0x801F7098`); the hand-back block clears the stage id `0x8007B64A` (`0x801F7120`) and writes flow `0x0B` at `0x801F713C` after the countdown on `0x801F73F8`. Phases 1..4 are bumped from SCUS `FUN_80056208` (`0x800562E8` / `0x8005640C`). |
