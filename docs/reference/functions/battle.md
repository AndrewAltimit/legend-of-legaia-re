...
| `801DBEC4` | **"Ra-Seru sealed" mark blit** (0898): a 64x16 quad at `(120, 96)`, CLUT `0x7700`. Third of the three marks, gated on the special-battle word `0x8007BAC0 & 0x200` - which the dome's own arena init **does** raise on the top seeded course (`0x321` at `0x801CEBC8`), so retail draws this mark there. The same VA in the field overlay 0897 is a mid-function label of `801DBE9C`, not an entry. Gate order and the whole chip path: [`minigame-muscle-dome.md`](../../subsystems/minigame-muscle-dome.md#what-makes-a-ra-seru-chip-render). |
