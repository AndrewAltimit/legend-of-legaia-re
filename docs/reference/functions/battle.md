...
| `801DBEC4` | **"Ra-Seru sealed" mark blit** (0898): a 64x16 quad at `(120, 96)`, CLUT `0x7700`. Third of the three marks, and the one gated on the special-battle word `0x8007BAC0 & 0x200` - which no Muscle Dome round ever raises. Gate order and the whole chip path: [`minigame-muscle-dome.md`](../../subsystems/minigame-muscle-dome.md#what-makes-a-ra-seru-chip-render). |
| `801D0088` | **Arena / special-battle init** (PROT 0977). Writes the **low byte** of `0x8007BAC0` - the `(course << 4) + round + 1` cell - and nothing else, so the dome cannot set the Item or magic restriction bits; those come only from SCUS battle init keyed on the first enemy monster id. `FUN_801D0748`, the match SM it feeds, is byte-identical in the dome and 0898 dumps. |
...
| `801F88EC` | **Element-change tick** - PROT 0964, action `0xAF` (and the range `0xB0..=0xB2` on its second arm, bounded by `slti 0xaf` / `slti 0xb3`). Redraws `rand() % 3` until it differs from the cycling counter `0x801C8FE4`, then writes the enemy record's `+0x1D` element and its `+0x1C` ME group from the in-image table `0x801F9B70`. The only routine on the disc that mutates a live enemy record's element. |
