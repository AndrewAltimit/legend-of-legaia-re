...
| `800243F0` | Per-frame BGM/asset poller. Resolves BGM IDs via the PROT-relative offset scheme, then runs the staged track-swap SM (stage counter `gp+0x744`, JT `0x800108C8`): teardown, async load, settle delay `gp+0x768`, install, re-attach. Sole setter of `_DAT_8007B750` bit 3 (`0x800246D0`) and the stall side of the sub-op 9 / `0xA` script handshake - see [`audio.md`](../../subsystems/audio.md#the-track-swap-handshake-fun_800243f0--op-0x35-sub-op-0xa). Also the side-band request resolver - [details ↓](#800243f0). `see ghidra/scripts/funcs/800243f0.txt`. |
...
| `80035BD0` | **SFX-cue overwrite (current slot).** `(cue_id: u16)`. Same 4-slot ring as `80035B50` but writes only at the current head - does NOT advance `gp+0x158`. Used to replace an in-flight cue; in `FUN_801d01b0` it plays the deny buzz `0x23` when a menu-open is refused under the `_DAT_1f800394 & 0x8000000` lock (the earlier "bonk overrides the pending step on a blocked move" reading is corrected - wall contact is silent). The 4-slot ring is then drained by the audio per-frame tick `FUN_80016B6C` (see the [cue router row](battle.md#battle-subsystem) for the same ring's battle-side producer). |
...
| `8004FCC8` | **Menu cue dispatch + voice trigger.** `(id)`. Splits on `sltiu v0,s0,0x100` at `0x8004FCD4`: below the threshold a UI cue goes into the 4-entry ring at `&DAT_8007B6D8`, above it a streamed voice goes to `FUN_8003D53C` behind two decline gates - the battle context's `+0x276` stage byte and the read-idle poll `FUN_8003DE7C(1)`. Ported with both gates as `engine_vm::battle_cast_cue::admit_voice_cue`. [details ↓](#8004fcc8) |
| `8003E104` | Monster-sound bank loader: `(monster_idx, slot, dst_buf)`. Reads `monster.snd` for the given monster, bounding the index against the bank count. Bank index at `0x801C8980`: `[u32 reserved][u32 count][u32 start_sector[count + 1]]`, with the offsets array at `0x801C8988` (4-byte stride) and monster `i` spanning `[tbl[i], tbl[i+1])`; the trailing word is the archive's own length, which is what makes the last span total. PROT 0895 stages the whole index (see [`memory-map.md`](../memory-map.md)). Those are **sector offsets relative to the bank base**, not absolute LBAs - the base is raw TOC `0x37D`'s start LBA (`*0x801C7EEC` -> `gp+0x8F0`). The build-mode gate is the `beq` at `0x8003E1FC`. Called twice from `FUN_800520F0` (slots 7 and 8). |
...
Its second streaming slot resolves the op-`0x36` sub-`1` side-band request `_DAT_8007BABC` at `0x800248B4..0x8002494C`, the arms running in sequence with later ones overwriting: `< 2000` -> raw index `*(0x8007BBE4) + 2` (slot `3`, slot `6` for `1000..=1999`), `2000..=2999` -> `*(0x8007BBE4) + id - 2000` in slot `3`, `>= 3000` -> `+ id - 3000` in slot `6`, and `0x1000` copies the request onto the acknowledge cell and loads nothing. `*(0x8007BBE4)` holds `1072`, CDNAME's raw `vab_01` index, in every catalogued state checked ([`sfx-table.md`](../../formats/sfx-table.md#the-side-band-bank-a-field-script-selects)).

### `8001FF58`

Indexes the 12-byte-stride resource table at `0x80091508` (record = `base +
id*0xC`; `+0x8` = VAB id, `+0xB` = loaded flag); when the flag is set it calls
`FUN_80068C80` on the handle with `sb zero,0xb` in the delay slot
(`0x8001FF78..0x8001FF90`). Teardown counterpart of the installer `FUN_8001E54C`
and of the scene-reset path `FUN_8001DCF8`. The loaded flag is also the cue
drainer's enable byte (`FUN_80016B6C`, `lb v0,0xb(v1)` / `beqz` at
`0x80016CE4..0x80016CEC`), so a cue whose category names a slot this routine
closed is silent, not rerouted. The callers that matter for the shared slot-2 /
slot-6 region are the field init `FUN_801D6704` (slots `2`, `7`, `8`, `11`),
battle mode init `FUN_8001DCF8` (`6`, `3`) and the side-band teardown
`FUN_801D8450` (`6`)
([`sfx-table.md`](../../formats/sfx-table.md#one-region-per-mode-slot-2-and-slot-6)).
An older reading had it releasing a SEQ slot.
`see ghidra/scripts/funcs/8001ff58.txt`.
