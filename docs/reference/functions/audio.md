...
| `801E2650` | **Battle flash-layer quad emitter.** The `FUN_801E2524` layer draw: `(offset, percent, semi, position)`. Emits **two** textured `POLY_FT4`s - **not** a full-screen wash: both sample the effect atlas at texpage `0x27` under CBA `0x7703` (the value-readout sheet), share the colour `percent * 256 / 100` in RGB and the code `0x2C \| semi << 1`, and slide in from opposite sides to a per-`position` seam. Travel is `min(ctx[+0x28C] - offset + 0x30, 0xF0) * 2`; the same term closes the shared band `0x90 - h` .. `0xB2 + h`, `h = (0x1E0 - travel) * 7 / 20`. `position` indexes 4 rows of `(bias, seam X, texel row)`, no default arm. Ported as `legaia_engine_vm::battle_action::flash_ramp::flash_quads`. `see ghidra/scripts/funcs/overlay_battle_action_801e2650.txt`. |
