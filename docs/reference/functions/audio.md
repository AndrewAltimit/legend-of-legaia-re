...
| `8003E104` | Monster-sound bank loader: `(monster_idx, slot, dst_buf)`. Reads `monster.snd` for the given monster, bounding the index against the bank count. Bank index at `0x801C8980`: `[u32 reserved][u32 count][u32 start_sector[count + 1]]`, staged by PROT 0895's `memcpy` of `0x400` at `0x801CEF74`; monster `i` spans `[tbl[i], tbl[i+1])`, and the trailing word being the archive's own length is what makes the last span total. Those are **sector offsets relative to the bank base**, not absolute LBAs - the base is raw TOC `0x37D`'s start LBA (`*0x801C7EEC` -> `gp+0x8F0`). The build-mode gate is the `beq` at `0x8003E1FC`. Called twice from `FUN_800520F0` (slots 7 and 8). |
