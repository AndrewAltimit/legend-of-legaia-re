...
| `801E2524` | **Battle Arts announcement banner** (read as a "screen-flash ramp" until its emitter's texels were decoded). Per-frame pass over the banner selector `ctx[+0x28B]` and the slide clock `ctx[+0x28C]`, emitting four ghost-trail layers through `FUN_801E2650`. Bands, clock walk, per-layer offsets and the cancel arm: [`subsystems/battle-action.md` § Arts announcement banner](../../subsystems/battle-action.md#arts-announcement-banner-fun_801e2524--fun_801e2650). Ported as `legaia_engine_vm::battle_action::step_flash_ramp`. `see ghidra/scripts/funcs/overlay_battle_action_801e2524.txt`. |
| `801E2650` | **Arts-banner quad emitter.** The `FUN_801E2524` layer draw: `(offset, percent, semi, position)`. Emits **two** textured `POLY_FT4`s - **not** a full-screen wash - both at texpage `0x27` under CBA `0x7703` (the value-readout sheet), sliding in from opposite sides to a per-`position` seam. Quad 2 is the sheet's shared `ARTS!!`; quad 1 is position-selected, so the four compose `NEW` / `HYPER` / `MIRACLE` / `SUPER` `ARTS!!`. Travel term, band arithmetic and the position table: [`subsystems/battle-action.md` § Arts announcement banner](../../subsystems/battle-action.md#arts-announcement-banner-fun_801e2524--fun_801e2650). Ported as `legaia_engine_vm::battle_action::flash_ramp::flash_quads`. `see ghidra/scripts/funcs/overlay_battle_action_801e2650.txt`. |
