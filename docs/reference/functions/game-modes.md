...
| `801E5154` | **Two-child link updater** (0897, frameless, 484 B). `(actor)`. Reads the two child-actor pointers at `+0x90` and `+0x94`; if either child's flag word `+0x10` already carries the done bit `0x8`, it sets its own `+0x10 \|= 8` and returns, otherwise it compares the children's `+0x14` positions. The consumer of the completion bit the clip-timer leaves above raise - a tether/beam actor between two others. `overlay_field_0897_801e5154.txt`. |
