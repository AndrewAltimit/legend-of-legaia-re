...
| `801D820C` | **Fourth clip-timer leaf** of the family above (PROT 0897, 76 B, frameless). Multiplies the actor's `+0x54` state halfword by the frame-delta scalar `DAT_1F800393`, accumulates into `+0x78`, and when the accumulator reaches `0x1000` clamps it to `0xFFF` and ORs the same retire bit `0x8` into the flag word `+0x10`. The `0x1000` ceiling is what distinguishes it: the other three clamp at a per-actor duration, this one at a fixed 12-bit fraction. `overlay_field_0897_801d820c.txt`. |
| `801E59B0` | **Field 2-D rotate helper** (PROT 0897, 88 B, frameless). `(angle, vec2)` -> `(vec2[0] * t1[angle] + vec2[1] * t2[(angle + 0x400) & 0xFFF]) >> 12`, off the two halfword tables at `0x8008-0x47E4` and `0x8008-0x4808`; the `+0x400` bias is the quarter-turn that makes the second table the cosine of the first. `overlay_field_0897_801e59b0.txt`. |
...
| `801DDA90` / `801DDB44` | **Field box-quad emitter arms** (PROT 0897) - two slices of one loop, not two entries: each fills a `POLY_F4`-shaped halfword block at `a1[+0x8..+0x16]` and `j`s to the shared tail at `0x801DDBC8`, which links the packet through `FUN_8003D2C4` and repeats while an 8-iteration counter holds. Ghidra's body for the second arm is a disjoint address set, which is why the attribution sweep classes both as `gapped`. `overlay_field_0897_801dda90.txt`. The byte worklist ranks the second arm 88 bytes into that body, and Ghidra minted an entry there over a body starting at the earlier address; the dump's header carries the address its instruction stream actually starts at, so no function is asserted at the later one. |
...
| `801D2784` (PROT 0979) | **Battle-intro transition tail** - 148 bytes past the driver above, all of it addressing the overlay's own `0x801E****` globals (`-0x4128`, `-0x4124`, `-0x4110`, `-0x411C`) plus one `jal 0x801D6710`. Like the two rows above it runs to the last word of its image, so it carries no `jr ra` and the gap census scored it as a stray run rather than as a body. `overlay_field_battle_intro_0979_801d2784.txt`. |
