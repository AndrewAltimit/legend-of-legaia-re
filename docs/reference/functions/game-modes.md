...
| `80025044` / `8002504C` | Two `jr ra; nop` leaves used as the null tick slot of an actor template. `_044` is referenced from three static SCUS templates (`0x800706F4`, `0x8007070C`, `0x80076814`), `_04C` once from the field overlay 0897's template table at `0x801F270C`. A template that ticks nothing. `see ghidra/scripts/funcs/80025044.txt`. |
| `80025054` | Actor-template tick (template word `0x8007061C`): clears actor flag bit `0x2` via `FUN_80025344`, writes the heading helper `FUN_80019278`'s result to `actor[+0x16]`, then builds a fixed `0x18`-byte parameter block on the stack (`0xF0`, `0xF0`, `0xF0`, `1`, `0`, `0x40`, `0x20`, `0xA`, `0x10`, `1`, `0x14`) and hands it to the move-buffer setup `FUN_800204F8`. `see ghidra/scripts/funcs/80025054.txt`. |
