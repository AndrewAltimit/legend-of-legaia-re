...
| `8002612C` | Mode 16 (READ INIT) handler - a frame, `jal 0x801CE9C0`, and the epilogue. It calls the overlay loader `FUN_8003EBE4` **not at all**, and `0x801CE9C0` is an entry point in no image: the only bytes that cover it are the debug-menu overlay's, at slot-A base `0x801CE818` + `0x1A8`, mid-way through `FUN_801CE97C`'s global-clear block. A retail-stripped dev path - see [`boot.md`](../../subsystems/boot.md#three-init-handlers-stage-nothing). Ported as `legaia_engine_core::mode::mode_init_bare`. |
