...
| `0x8007BD60` | u8 | Region **battle-setup** byte, `region[+8] & 0x1F`, written by the region re-read `FUN_801D9E1C(player, 0)` that op `0x3E` and the step roll both run. Unmodelled in the port. |
| `0x8007B9D8` | u32 (also read `lbu`) | Per-mode **frame-step floor** the adaptive frame step `DAT_1F800393` cannot drop below. The cold-boot `opdeene` capture holds `3`; the port installs `2` for field scenes (open). |
...
| `0x8007BDD8` / `0x8007B6AC` | u32 / u32 | The field clip **base** `FUN_801D1EC4` stores and the clip **override** op `4C CE` writes; `FUN_801E58A8` picks `base + override - 1` when the override is non-zero, `leader + 1` when the base is `99`. |
...
| `0x801F696C` | Battle overlay: "**a Super fired this build**" latch | Set to `1` by the Super trigger arm (`0x801EF5A8`/`0x801EF5B4`), by the tail-replace `FUN_801EF9E4` (`0x801EFBD4`) and at `0x801F0518`; read at `0x801E3840` (a reader the port does not model yet); what makes Miracle-before-Super ordering observable. See [`battle-action.md`](../subsystems/battle-action.md). |
