...
| `0x8007B604` | u8 | World-map encounter countdown - an **unsigned byte**; the zero / disabled path leaves it at zero (the port once wrote `0xFF` there). |
| `0x80073F14` | i8[8] | Compass axis-sign table read by motion-VM ops `0x37` / `0x41`'s eight-direction walk. |
| `0x80077498` | placement record 91 | HUD element `0x5B`, the battle message bar (payload pointer `0x800774AC`) that carries the steal / thief-return captions; slides from `(16, 236)` to `(16, 194)`. |
| `0x80077A38` / `0x80077A4C` / `0x80077A64` / `0x80077A70` | caption templates | Steal "took" / "recovered" / steal-result / "recovered all" templates composed by `FUN_8004AD80`'s death-spoils arm, with tails at `0x8007B690` / `0x8007B694` / `0x8007B698`. |
...
| `0x80085758` | u8[] | **Fourth flag bank** - bitfield accessed via SET / CLEAR / TEST `(idx >> 3, 0x80 >> (idx & 7))` (`FUN_8003CE08`/`_CE34`/`_CE64`). The field-VM opcode encoding spans `idx = 0x000..=0xFFF` plus `0x8000..=0x8FFF` (extended-prefix opcodes `0xD0..=0xFF`), so it is **not** a fixed 256-bit array. The earlier `0x80086D70` was a double-count of the `0x1618` save displacement onto `0x80085758` (which itself already `= 0x80084140 + 0x1618`); see [`subsystems/script-vm.md`](../subsystems/script-vm.md). Flag `0x6DE` (`0x80085833` bit `0x02`) is a live [position predicate](../subsystems/script-vm.md#a-system-flag-can-be-a-live-position-test-not-progress), not a latch. |
| `0x80077998` | u8[5] per character | **Rot limb object ranges** for the battle per-actor draw `FUN_80048A08`, one five-byte row per roster character, indexed `0x8007BD10[seat] - 1` (`addiu t1,t1,0x7998` at `0x80048C48`, row stride formed as `(id - 1) * 5`). For a party seat whose battle actor's `+0x16E` carries Rot bit `0x08` the draw dims mesh objects `row[0]..=row[1]`, bit `0x10` dims `row[2]..=row[3]`, bit `0x20` dims every object from `row[4]` up. The port reads the table from `SCUS_942.54` in `World::install_menu_text` (`RotLimbTable::from_scus`) and resolves it per actor in `World::battle_limb_dim_plan`; see [`renderer.md`](../subsystems/renderer.md#rotted-limbs-draw-dark). |
...
| `0x8007BD0D` | u8 | Formation monster slot 1 (the per-seat formation cells run `0x8007BD09 + seat`, seat 3 = `0x8007BD0C`). The battle draw tick `FUN_800480D8` reads it as `gp+0x9F5` - zero means a lone monster - not a debug byte. |
| `0x8007BDA8` | u8 | Outdoor-stage flag (13 stages); also the battle tint pass `FUN_8004A908`'s invert gate, which turns the distance fade toward a brighter copy on outdoor stages. |
...
| battle actor `+0x227` | u8 | Read by the victory drop roll: a seat whose actor has it non-zero cannot supply the drop item (`0x8004F53C`). Written by the capture path; the "capture counter" name is inference. |
...
| `0x8007BCA8` | u32 | Fog **live count** (`gp+0x990`), recounted by the render walk every frame: zeroed at `0x8003F398`, bumped per live record at `0x8003F3C8`. The spawner compares the value the last walk left, so one frame can overshoot the cap by what one frame spawns. |
| `0x8007BCB0` | u32 | Fog **cap** the spawner tests the live count against: `0x48` when `MAN[1] & 4` or the `0x8007B6A8` byte is set, `0x18` otherwise (`FUN_8003AEB0`, `0x8003B6BC..0x8003B6E8`). Four scene MANs raise it - `map01` / `map02` / `map03` and `opurud`; it is not a debug switch. |
| `0x8007BD62` | u8 | Arts-shout **last channel** (`gp+0xA4A`) - one byte for the whole party. `FUN_8004C140` re-rolls a draw that equals it and stores the pick (`0x8004C3E8` / `0x8004C400`). |
| `0x8007BD60` | u8 | Region **battle-stage variant**, `region[+8] & 0x1F`, written by the region re-read `FUN_801D9E1C` that op `0x3E` and the step roll both run; battle init `FUN_800513F0` loads the backdrop from entry `0x80084540 + variant` (raw TOC `+5`). The same re-read stores `_DAT_8007B64B` (keep backdrop object 1), the Door of Light / Wind gates in `0x1F800394` and the world-map return triple `0x80084624` / `0x80084628` / `0x8008462C` ([`script-vm.md`](../subsystems/script-vm.md#the-region-battle-setup)). Port: `region_encounter::region_battle_setup`. |
| `0x8007B9D8` | u32 (also read `lbu`) | Per-mode **frame-step floor** the adaptive frame step `DAT_1F800393` cannot drop below (`FUN_80016B6C`, `0x80017178..0x80017198`). Field init `FUN_801D6704` writes `2` (`0x801D6990`); a scene's own prescript raises it through move-VM ext sub-op `0x2F` (`0x801D45E4`, `opdeene`'s `3`); name entry `FUN_801F03F0` holds `1` while open. Every writer is tabulated in `engine-core::world::frame_step_floor`. |
...
| `0x8007B5FC` | u32 | Encounter step counter - one word across doors, not reset per scene. Written from five sites, every one a `FUN_801DDF48` roll: SCUS `0x8003AC90` (adds half a roll while below 487), `FUN_801D1EC4` at `0x801D1F6C` (when `<= 0`), op `0x3E` at `0x801E076C`, `4C EC` at `0x801E34F8`, and the copy inlined in `FUN_801D9E1C`; read and stored as a word. | `No Random Battles` writes `0x0377`. |
| `0x8007B6A8` | u8 | Save-anywhere allow flag: `MAN[1] & 1`, stored by the scene installer `FUN_8003AEB0` (`sb` at `0x8003AF54`); the same bit raises the fog cap `0x8007BCB0` to `0x48` (every kingdom overworld). | `Save Anywhere (Press Select+X)` (writes a halfword). |
...
|---|---|---|
| `0x8008444C` / `0x80084458` / `0x8008445C` | Fishing | Persistent **point record**: the points pool (capped) at `0x8008444C` and two record words, `0x80084458` the best catch the HUD row reads ([`minigame-fishing.md`](../subsystems/minigame-fishing.md)). |
| `0x80084450` | Fishing | Persistent **equipped lure** row (`0..=2`, item `0x9D` + row), written by the tackle picker. |
| `0x80084454` | Fishing | Persistent **rod** stat (`0..=2`), the divisor row of the reel tug. |
| `0x8008446C` | Fishing | Persistent **purchased** bitmask of the point exchange: bit `row + venue * 8`. |
...
| `0x801D1A7C` | Muscle Dome | The still's **level** - broadcast into all three colour bytes of each quad, the call skipped at zero (`beqz a0, 0x801D00B8`). Six hub arms own it: the re-entry init zeroes it at `0x801CECD0`; arm `0x0A` raises it `4 dt` to `0x80`, `0x0B` lowers it `2 dt` to `0x40` while tally lane 0 sits at its clamp, `0x0C` and `0x15` hold, `0x14` raises it again on the latch-`1` arm, and `0x16` drains it `2 dt` to `0` at `0x801D002C..0x801D0040` ([`ringside-still.md`](../formats/ringside-still.md#on-a-natural-re-entry)). |
...
| `0x8007BAFC` | **Field-bank latch** (`gp+0x7E4`). The field init `FUN_801D6704` loads PROT 0876 into VAB slot 6 and sets it only while it is clear (`0x801D6FF4..0x801D7048`); battle mode init `FUN_8001DCF8` (`0x8001DFC0`, closing slots 6 and 3, only when the mode word `0x8007B83C` reads `0x14` - a minigame's call with `0x18` skips it), the minigame warp `FUN_80025980` (`0x800259A4`) and the side-band teardown `FUN_801D8450` clear it ([`sfx-table.md`](../formats/sfx-table.md#one-region-per-mode-slot-2-and-slot-6)). |
| `0x8007BABC` | Op-`0x36` sub-`1` **side-band bank request**, resolved by `FUN_800243F0`'s second streaming slot into a slot-3 / slot-6 `vab_01` bank; `0x1000` parks. |
| `0x8007BBE4` | Word holding the raw TOC index of the `vab_01` block (`1072`, CDNAME's `#define vab_01`, in every catalogued state checked); the side-band resolver adds the request's offset to it. |
| `0x8007B9B8` | One-shot latch gating the field init's slot-10 load (raw `0x428` then `0x422`) together with track `0x814` at `0x8007BAC8` (`0x801D71A0..0x801D7274`). The load sets it itself, in the delay slot at `0x801D71CC`; SCUS `FUN_800243F0` reads it at `0x800243F4`; its only clearer is the DEBUG MODE overlay's init (PROT 0971, `0x801CE9B4`). Unmodelled. |
...
| `0x8007C348` | u32 - Actor allocator free-stack **top index** (`0x8E` when full); the stack itself is `+0x28 + 4*i`, 143 entries. Seven list sentinels follow at `+0x04..+0x24` (written by `FUN_8001E1B4` at `0x8001E338..0x8001E364`), with the player slot at `+0x1C` (written by field MAIN_INIT at `0x801D6D7C`). |
...
| `0x801F351C` | u32 | One-shot gate flag for the world-map POLY_FT4 batch emitter. `FUN_801D8258` sets it to `1`; `FUN_801D7EA0` clears it (the "0897 sibling `FUN_801C9688`" was that same routine printed `0xE818` low) after one emission. Lives in the persistent `0x801F0000+` region so survives overlay swaps. |
...
| `0x801F6990` | Battle overlay: the arts queue-builder's 16-word per-token side array | `1` = an accepted art's starter (build loop), `4` = a Super tail-replace starter; read by the marked-starter reorder pass `0x801EF8A0..0x801EF968`, and by SCUS `FUN_8004AD80` at `0x8004B804`, whose store at `0x8004B80C` raises the Arts banner selector `ctx[+0x28B]` over the `3` written just before it. |
...
| `0x801E4404` / `0x801E442C` / `0x801E44B8` / `0x801E4518` | Menu overlay (PROT 0899): the **options tables** and the casino prize table | The options display layout (ten `[u16 row_id, u16 advance]`), its shared string-pointer table, the eight-byte row-node list (zero-terminated; `lw` / `bnez` at `0x801D2A0C`), and the prize-exchange table (`0x60`-byte blocks of eight-byte rows). Each is formed by its consumer's `lui`/`addiu` pair ([`field-menu.md`](../subsystems/field-menu.md#options-screen)). The same VA `0x801E4404` in PROT 0897 is unrelated data ([`overlay-va-aliases.md`](overlay-va-aliases.md)). |
...
| `0x801F35C0` | Field overlay: tile-board **cell buffer pointer** | `DAT_801F35C0 = FUN_80017888(0, width * height)` at `0x801EF3E8..0x801EF3F4` - exactly one byte per cell and nothing pads it. Freed with the tile-actor table `DAT_801F35BC` by the walk SM's teardown state `0xE` (`FUN_80017B94`, `0x801EFE78` / `0x801EFE88`). See [`tile-board.md`](../subsystems/tile-board.md). |
...
| `0x801D0D4C` | STR / MDEC overlay (PROT 0970): the one-shot init flag | Head of the **initialised** data segment, distinct from the uninitialised region above - the reason the entry has two separate runs. |
| `0x801D0D58` / `0x801D0DDC` | STR / MDEC overlay (PROT 0970): the two **MDEC command packets** | `0x801D0D58` = header `0x40000001` then the luma / chroma quant matrices at `0x801D0D5C` / `0x801D0D9C`, which `FUN_801CFCDC` copies from its caller; `0x801D0DDC` = header `0x60000000` then a static IDCT matrix. Both go out through `FUN_801CFFDC(pkt, 0x20)`. |
| `0x801D0E60..0x801D0E9B` | STR / MDEC overlay (PROT 0970): the **hardware-register pointer block** | Fifteen words, loaded by `FUN_801CFFDC` and its siblings: DMA channels 0-2 (`0x1F801080..0x1F8010B8`), MDEC0 / MDEC1 (`0x1F801820` / `0x1F801824` at `0x801D0E90` / `0x801D0E94`) and DPCR (`0x1F8010F0` at `0x801D0E98`). A pointer block, not a strided table. |
...
| `0x801F21B4` / `0x801F2214` / `0x801F2254` | Field overlay (PROT 0897): the **three probe tables** | Not one twelve-row table: the actor-collision probes at `0x801F21B4` (six rows, formed at `0x801CFE74` / `0x801D5A70`), the leading-edge wall probes at `0x801F2214` (four rows, `0x801CFEE8` / `0x801CFFC0` / `0x801D009C`) and the interact facing compass at `0x801F2254` (eight `±64` points, `0x801D0834`); the last ends at the `lw`/`sw` scalar `0x801F2274`. Together the head of the overlay's **data segment** (file `0x2399C..0x25000`), which is not one table. `legaia_asset::field_probe_tables`. |
