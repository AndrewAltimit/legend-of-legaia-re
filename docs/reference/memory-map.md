...
| `0x80085958` | u8[] | **Item inventory** array (= SC `+0x1818`), 2-byte stride `(id, count)`; the `Have 99 Items` cheat targets the count bytes (`0x80085958..0x800859E8` = the 72-slot general-item page). Stacks cap at 99. Accessors (`FUN_80042310`/`_42EE0`/`_42F4C`/`_423E0`/`_43048`) all scan/write within the active window `gp[+0x2D2]..gp[+0x2D4]` and bound the slot index on `gp[+0x2D4]` — there is no unchecked slot-index write path. | `Have 99 Items` and `Item Modifier`. |
