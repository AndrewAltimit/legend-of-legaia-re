...
| `0x8007B9D4` | ptr - **Character-pack (`player.lzs`) base pointer**, `gp+0x6BC` (`gp = 0x8007B318`). Written by the loader `FUN_8001ED60`, read by `FUN_8001E890`'s decode arms and by the unclamped model-pack registrar at `0x8001EAFC`. It names the same heap block in 97 of 98 catalogued states and is **garbage in all 61 battle states**, because the battle load reuses the block. |
...
| `0x801D46CC` | records | Dance-overlay **sprite table**, 20-byte stride. Record 0 is the count-in banner - half-extents `0xA0` x `0x20`, texel seat `(0x48, 0x90)`, CBA `0x7D0A` - seated by `FUN_801D2F38` about its centre, with the slide arms passing `a1 = 0x77` (`0x801D2EB8` / `0x801D2EE8`) and the hold arm `0x78` (`0x801D2F00`); the record's `+0x0F` blend byte is `1` for the halves and `0` for the hold. |
| `0x801E46B4` | u16 | Shop **quantity** under edit. Stepped by the pad handlers `FUN_801DB7F4` / `FUN_801DBD94` in `+1` / `-1` / `+10` / `-10`, clamped, with the bound `min(gold / price, 99, 99 - held)` - so the quantity is a value with a stepper, not a cursor into a row list ([`shop.md`](../subsystems/shop.md)). |
...
| `0x1F80037D` | u8 | Second per-frame scratch mirror, reloaded beside `0x1F800393` by the mode-INIT core state reset. The cast band's per-arm countdown drain multiplies the two: each arm takes the **product** with `0x1F800393`, twice that byte, or once it, so a drain measured on one arm is not the band's rule ([`cast-module.md`](../subsystems/cast-module.md#frame-gating-measured)). |
