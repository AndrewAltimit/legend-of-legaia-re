...
| `0x801D46CC` | records | Dance-overlay **HUD widget table**, 34 records x 20 bytes (`addiu v0,v0,0x46cc` at `0x801D2F74`, index `(a2 & 0x3FF) * 20`). Record 0 is the count-in banner - CBA `0x7D0A` at `+0x06`, texel seat `(0x48, 0x90)` at `+0x08`, half-extents `0xA0` / `0x20` at `+0x0A` - and `FUN_801D2F38` draws it **centred** on the seat, so the record covers 320 x 64. The count-in's callers pass the seat `0x77` for the slide halves (`0x801D2EB8` / `0x801D2EE8`) and `0x78` for the hold (`0x801D2F00`), and patch the `+0x0F` blend byte to `1` and `0` respectively. Layout in [`minigame-dance.md`](../subsystems/minigame-dance.md#hud-widget-table-dat_801d46cc--emitter-geometry). |
