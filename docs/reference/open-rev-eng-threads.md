...
| Spine flag `0x142` (dolk-dungeon-clear) writer | open (capture-gated) | Spine-writer #2 of 3. Flag `0x142` clear routes `map01`'s dungeon entrance to `dolk` (pre-boss), set routes to `dolk2` (post-boss); the branch mechanism + engine port are resolved (see the `dolk`→`dolk2` destination row) but the *writer* that sets `0x142` at the dolk-clear beat is in no loaded program - readers only. Closes with a fresh PCSX-Redux write-watchpoint capture at the dolk-clear story beat. Runbook `docs/tooling/spine-flag-writers-capture.md`. | `project_re_wave_5_plan.md` |
| Spine flag `0x482` (Drake mist-wall) writer | open (capture-gated) | Spine-writer #3 of 3. Flag `0x482` gates `map01`'s mist walls (`P2[34..36]` C1); it reads `0` across the whole save library incl. both pre-victory Zeto captures, and `rikuroa` sets it nowhere, so the writer is unobserved. Closes with a fresh PCSX-Redux write-watchpoint capture at the post-mist story event. Runbook `docs/tooling/spine-flag-writers-capture.md`; see the `Gate-flag setters outside the MAN field VM` row for the `0x482` census. | `project_re_wave_5_plan.md` |
...
| Gate-flag setters outside the MAN field VM (`549`, `0x482`) | `549` mechanism resolved; `0x482` capture-gated | `549` (`0x225`, town01 opening one-shot) is written by `FUN_80038158` **op-`7`**, a per-actor motion/bytecode VM (dispatch `FUN_8003BC08` on actor `+0x10 & 0x80`) that sets `DAT_80085758` inline from its own script bytecode - a different bytecode than the MAN field VM, so `--system-flag-census` cannot see it; pinned by a bank RAM-diff over the town01 opening + the dump. `0x482` (mist walls) reads `0` across the whole save library incl. both pre-victory Zeto captures, and `rikuroa` sets it nowhere - needs a fresh post-beat capture. Engine leaves both gated (faithful). See [world-map.md](../subsystems/world-map.md) + [functions.md](functions.md). | `project_re_wave_5_plan.md` |
