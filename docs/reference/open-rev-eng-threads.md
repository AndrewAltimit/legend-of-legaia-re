...
| Thread | Status | What would close it |
|---|---|---|
| `_DAT_8007B910` carries two incompatible roles | resolved (moved to re-settled) | It is the **live audio level**; the "brightness" label had nothing behind it. `FUN_80062004` is `SsSeqSetVol` via `FUN_80061EDC`, so its second argument is a volume - the discriminator this row named. Grade `disassembly`. See [`re-settled-threads.md`](re-settled-threads.md#_dat_8007b910-is-the-live-audio-level-not-screen-brightness). |
...
| Phantom-VA sweep of the PROT 0897 imports | resolved (moved to re-settled) | The word-level sweep (`resolve-phantom-va.py`) closed all three residues - grade `disassembly`. The `0x801E5000` boundary band resolves per dump (the two open addresses are 0897 own-content *data*, decided 13/13 and 14/14 words); `0x8020D05C` is 0898 rodata at `0x801F6874`, not a function; and the `overlay_0896_*` family is **two** imports (untagged `0x801C0000` with the three-band law, tagged `base=0x801C5818` printing 0896's own bytes), each byte-decided with zero exceptions. See [`overlay-va-aliases.md`](overlay-va-aliases.md#the-byte-level-sweep) and [`re-settled-threads.md`](re-settled-threads.md#phantom-va-sweep-of-the-prot-0897-imports). |
