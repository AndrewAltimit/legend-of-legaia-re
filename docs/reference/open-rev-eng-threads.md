...
**The actor-collision probe is decoded + modelled.** `FUN_801cfc40` (bits `1`/`4`) walks the active-actor table `DAT_801c93c8`, box-testing the three `DAT_801f21b4` probe points (disc-pinned: 64/63 ahead, ±32 lateral — wider than the wall edge) against each actor: a static entity anchors at its MAN object record (`tile*128 + sub*16`) with the `0x40+0x10` half-extent; a moving actor uses its live position with caller extents. The locomotion gates each 2-unit step on the actor bits and the wall bit together, so NPCs block exactly like walls. Engine: `World::field_actor_dir_blocked` ports the static-entity arm over the MAN-derived NPC anchors, gated by `World::solid_field_npcs` / `play-window --solid-npcs`.
**Residual (open):** the moving-actor arm (bit `1`, flags `0x40020000`), the touch side-effects (`FUN_801d5b5c` event post on the mutual `+0x98` partner link; face-the-NPC turn), and the third facing-indexed probe table `DAT_801f2254` (one pair per 8-direction facing, extents `0x20`) are unmodelled; a live player-vs-NPC press capture would pin the engine's 142-unit head-on standoff byte-exactly.
