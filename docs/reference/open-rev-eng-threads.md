...
| First boss (Zeto) trigger → Battle | formation + engine port RESOLVED; trigger writer capture-gated | Zeto (id `0x4B`, in `rikuroa`) has **no** on-disc formation record; it uses the **battle-id path** (`DAT_8007b7fc = 0x4B` → `FUN_8005567c` lone-monster cell, byte-exact with both `zeto_*_mid_cast` captures). Port: `World::install_boss_encounter` + a `rikuroa` first-visit latch on `0x1BE`, victory-latched. **Open:** the writer is not statically formed anywhere in the PROT corpus (see the `0x142` row) - runtime-computed; needs a Mt. Rikuroa write-watchpoint. Spine-writer #1 of 3; runbook `docs/tooling/spine-flag-writers-capture.md`. See [`encounter.md`](../formats/encounter.md#scripted-battle-id-path-fun_8005567c). | `project_re_wave_7_plan.md` |
| Spine flag `0x142` (dolk-dungeon-clear) writer | open (capture-gated; ALL static channels exhausted corpus-wide) | Spine-writer #2 of 3. Routes `map01`'s dungeon entrance (`dolk` clear / `dolk2` set; mechanism + port resolved). No writer by ANY static channel: readers only, motion-VM census negative, corpus-wide sweep clean (all-programs Ghidra + raw-byte scan of all 1235 PROT entries, `find_spine_flag_writers.py`), and the op-0x49 window census (209 sites; the only disc window family tops out 3 BELOW `0x142`) has it nowhere - runtime-computed. `chapter1_hub_breadth_oracle` also pins NO MAN SET site disc-wide (sole non-test site: `dolk P1[26]` Clear). Closes with a write-watchpoint at the dolk-clear beat (runbook `spine-flag-writers-capture.md`). | `project_re_wave_8_plan.md` |
| Spine flag `0x482` (Drake mist-wall) writer | open (capture-gated; ALL static channels exhausted corpus-wide) | Spine-writer #3 of 3. Flag `0x482` gates `map01`'s mist walls (`P2[34..36]` C1); it reads `0` across the whole save library incl. both pre-victory Zeto captures, `rikuroa` sets it nowhere, the motion-VM census is negative, and the corpus-wide static sweep + framed op-0x49 window census (see the `0x142` row; `0x482` has zero near-misses) are clean. Closes with a fresh PCSX-Redux write-watchpoint capture at the post-mist story event (the one leg needing fresh forward play past Zeto - no card slot brackets it). Runbook `docs/tooling/spine-flag-writers-capture.md`. | `project_re_wave_8_plan.md` |
...
| dolk2/rikuroa MAN source - REOPENED (the "v12-embedded MAN" was an over-read) | open | Their own `base+3` bundles are the MAN-less count=4 form `[1,2,6,0x14]`; the "embedded MAN at 0x1000" inside their SceneV12Table entries is the extended-footprint over-read onto the NEXT scene's `base+3` bundle (byte-identical to suimon/geremi's - see [scene-v12-table.md](../formats/scene-v12-table.md) § over-read). Where retail sources their partition scripts is unknown; the engine's `BundleSource::V12Embedded` follows the naive block and needs the raw-law fix (`Scene::field_map_index` already uses it). | `project_re_wave_8_plan.md` |
...
| Overworld story-conditional destination (`dolk`→`dolk2`) | resolved (mechanism + engine port) | Beyond the record-level C1/C2 gate, an entrance record can switch its `0x3F` target by an in-record op-`0x70` `SysFlag.Test`. `map01`'s dungeon entrance (`P2[1]`/`P2[2]`) branches on flag `0x142`: clear → `dolk` (pre-boss), set → `dolk2` (post-boss), same trigger + arrival tile. `overworld_portal_sites` decodes the conditional `0x3F` pair (`ConditionalDest`); the seeder resolves via `World::system_flag_test` (`chapter1_boss_spine_oracle` Part D). **Falsifies** "dolk2 is reached from a dungeon interior" (no interior lists dolk2). **Open:** who *sets* `0x142` (dolk-clear writer) - capture-gated. See [world-map.md](../subsystems/world-map.md). | `project_re_wave_5_plan.md` |
| Gate-flag setters outside the MAN field VM (`549`, `0x482`) | both open (capture-gated; `549` op-7 carrier falsified) | `549` (`0x225`, town01 opening one-shot): the bank RAM-diff pin stands, but `--motion-flag-census` finds **no** stream carrying `549` - the "written by op-7 from its own script bytecode" reading is falsified as to the carrier; the setter is a direct code path (`FUN_8003CE08`-class, overlay-resident). `0x482` reads `0` across the whole save library, `rikuroa` sets it nowhere, and both bytecode censuses are negative - needs a fresh post-beat capture. Engine leaves both gated (faithful). See [world-map.md](../subsystems/world-map.md) + [motion-vm.md](../subsystems/motion-vm.md). | `project_re_wave_7_plan.md` |
