...
**Collision derivation — decomp-pinned; X validated live; the Z bias is the capture-gated remnant.** `FUN_801cfe4c` is fully decoded (overlay `0897` @ `0x801CE818` + the on-disc bias table `DAT_801f2214`): it tests **three leading-edge footprint probes** (~47 units ahead, ±16 lateral) and derives each sub-cell as `zc = (z>>6)+2`, `xc = ((x+0x3f)>>6)−1`. The engine's [`World::field_tile_is_wall`] uses a single candidate-centre point with floor indexing. Settled: the **quadrant-mask formula is byte-identical** (the "inverted X parity" worry is **false**); the **X derivation is aligned** (retail's `ceil−1` equals the floor except at exact 64-multiples, an odd-parity position 2-unit stepping never reaches); the **Z derivation differs by one row** (the `+2`) for every sample.
**X press validated byte-exact (wall-press capture):** the `rimelm_wall_press_left` library save rests at `(1838, 4528)` against the full-height wall column at grid col 13 — the leading-edge probe `x−47 = 1791` reads the column's last wall sub-cell, one step shallower reads clear, and the capture's live grid is byte-identical to the engine-built one. But a full-height column blocks BOTH derivations (they only diverge by a Z row), and the floor tiers match too, so the Z bias is **not** discriminated by an `X−` press. Disc-gated: `engine-shell/tests/field_collision_discriminator.rs`.
**Still missing:** a **`Z+` (down) wall press** capture — there retail probes the wall's near edge row where the engine floor-read lands one row short on a clear cell, so even a thick wall separates the two (a `Z−` press needs a 1-row-thin wall). Do **not** realign the derivation blind. Code-pinned by `world.rs::tests::retail_collision_subcell_indexing_differs_from_engine_by_a_z_tile`.
