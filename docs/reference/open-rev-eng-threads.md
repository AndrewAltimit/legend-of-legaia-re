...
| Spine flag `0x482` (Drake mist-wall) writer | open (capture target; the "other7-pool writers" reading is falsified) | Every census site for `0x482` is a desynced-walker SJIS alias, pinned non-clean by the decode-coherence flag ([script-vm.md](../subsystems/script-vm.md) § decode-coherence). Motion/op-0x49 censuses: no site; polls show it only in bulk-load ticks. Writer = presumed a **direct code path**; next = P7 write-watch across the post-Zeto Drake-revival beat (`chapter2_garmel_pre_zeto`). (549's identical verdict fell to the `4C 0xE_` width fix; `0x482` stays all-noise under the fixed widths - the census anchor is the living check.) Reader solid (`map01` P2[34..36] C1 bands). `other7` = a dev-leftover koin3 MAN revision ([cdname.md](../formats/cdname.md#numbering-space)). |
...
Writer-less gates worth a capture: `0x50A` (koin1 toggle) and `0x5D6` (koin4) — no clean script SET disc-wide even under the fixed `4C 0xE_` widths (the trap that hid 549's writer); a reader-watch burst or a further width audit would settle them.
