...
| Writer of the Rim Elm opening flag (`549`) | partial - reader resolved, writer still open | `549` (`0x225`) is the Rim Elm opening one-shot. Its reader is the C1/C2 record-gate evaluator `FUN_8003BDE0` (the same header-gate mechanism as `0x1BE`): it is read as `P2[3]` C1 and `P2[4]` C2 across `town01`/`town0b`/`town0c`/`gameover_data`, never as an inline `0x70` test, which is why the opcode census reported no test sites (anchor `flag_549_reader_is_the_rim_elm_p2_gate`; engine mirror `World::p2_gate_flag_set`). The writer is still open: no census stream carries `549` and no field-VM `SET` exists, so the setter is a direct code path (`FUN_8003CE08`-class, overlay-resident). A targeted capture at the opening beat would close it. |
...
Two reader-only flags first exposed the pattern. `0x1BE` (Jeremi's arrival at `geremi`) is a self-latch: `geremi P2[0]` both sets it and lists it as its own C1 gate (anchor `geremi_p2_0_is_the_0x1be_self_latch`). `549`/`0x225` (the Rim Elm opening) is read the same way across the Rim Elm variants, though its writer is still open — it keeps a standalone row above.
...
- **`taiku` / `doman` / `rayman`** — self-latch pairs plus a linear `0x201` → `0x1FB` → `0x200` → `0x1FC` chain in `rayman`; `rayman2` is the same MAN with a shared C1 on the low flag `0x7`, a variant discriminator. Anchor `chapter2_dungeon_gate_families`.
...
- **`ropeway` / `ropeway2` / `jiji`** — the only spokes the capture corpus walked organically, so their play order is confirmed. `ropeway2` hosts a four-bit switch puzzle (`0x3FF`–`0x402`); its payoff records `P2[31..=34]` are gated via C2 on all four switches plus the `0x359` commit, an internal consumer the inline census had earlier mistaken for an external one. Anchor `chapter2_ropeway_jiji_gate_families`.
- **`dohaty` / `retock` / `retockin` / `stone`** — `dohaty` opens with a six-record `0xF` first-visit group; `retock`'s progression depends cross-scene on `balden`'s `0x1D5` and runs a second `0x357` → `0x502` chain; `retockin` is the `0x7`-gated interior variant, sharing `0x502`/`0x357` with `retock`; `stone` is a single one-shot. Anchor `chapter2_dohaty_retock_stone_gate_families`.
...
**Nivora Ravine (`nilboa`).** An entry group sharing `0x456`, a `0x47x` puzzle cluster, and a cross-scene successor gated on `0x370`; `nilboa2` is the `0xF`-gated variant carrier. Anchor `nilboa_nivora_ravine_gate_family`.
