...
| Spine flag `0x142` (dolk-dungeon-clear) writer | open (capture-gated; ALL static channels exhausted corpus-wide) | Spine-writer #2 of 3. Flag `0x142` routes `map01`'s dungeon entrance (`dolk` clear / `dolk2` set; mechanism + port resolved, see the `dolk`→`dolk2` row). No writer exists by ANY static channel: readers only in all loaded programs, motion-VM census negative, and the corpus-wide sweep (all-programs Ghidra + raw-byte scan of all 1235 PROT entries: setter jals, materializers, ori-formed, derived-base, data words - `find_spine_flag_writers.py`) is clean - the write is runtime-computed. Closes with a write-watchpoint capture at the dolk-clear beat. Runbook `docs/tooling/spine-flag-writers-capture.md`. | `project_re_wave_7_plan.md` |
| Spine flag `0x482` (Drake mist-wall) writer | open (capture-gated; ALL static channels exhausted corpus-wide) | Spine-writer #3 of 3. Flag `0x482` gates `map01`'s mist walls (`P2[34..36]` C1); it reads `0` across the whole save library incl. both pre-victory Zeto captures, `rikuroa` sets it nowhere, the motion-VM census is negative, and the corpus-wide static sweep (see the `0x142` row) is clean. Closes with a fresh PCSX-Redux write-watchpoint capture at the post-mist story event. Runbook `docs/tooling/spine-flag-writers-capture.md`. | `project_re_wave_7_plan.md` |
