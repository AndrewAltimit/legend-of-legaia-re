...
| First boss trigger -> Battle | rikuroa boss CORRECTED to Caruban (engine ported, `0x142`-gated); Zeto re-scoped to `jou` | Mt. Rikuroa's scripted boss is **Caruban** (id 73 = `0x49`, `FUN_8005567c` lone-monster band), gated by `0x142` - the rikuroa MAN's post-victory record P2[50] C1 one-shot + in-record SET (carrier PROT 0157; brackets `rikuroa_pre/post_caruban`). Engine: `SCRIPTED_SCENE_BOSSES = (rikuroa, 73, 0x142)`; the victory latch flips dolk->dolk2 organically. The old "Zeto in rikuroa, gated 0x1BE" is falsified: the Zeto battle capture's scene is `jou`, and `0x1BE` is geremi's arrival one-shot. **Open:** the `jou`-side Zeto staging record + gate, and the `DAT_8007B7FC` writer op. | `project_re_wave_8_plan.md` |
...
| Enemy-ally charm win-by-ally softlock (`FUN_801E7320`) | root cause CONFIRMED (static); fix open | Multi-enemy fight: a charmed ally that KOs the last hostile hard-locks. The `0x380` retarget `FUN_801E7320` has an unbounded reroll at `0x801E7370` - `do { reroll } while (monster[a0].HP == 0)` over the monster band; when the ally is the last living monster every other slot is dead so it never terminates. The `0x801E6638` victory widen isn't enough (it fixes the state-`0x5A` count, which runs *after* the action; the spin is inside the retarget at ActionSeed `0x0C`). Fix needs an overlay-hosted bounded-retarget detour (SCUS gap full). Probe `autorun_charm_win_softlock.lua`; `enemy_ally.rs` "## Known limitation". | `project_enemy_ally_charm.md` |
