...
| First boss (Zeto) trigger → Battle | formation + engine port RESOLVED; trigger writer capture-gated | Zeto (id `0x4B`, in `rikuroa`) has **no** on-disc formation record; it uses the **battle-id path** (`DAT_8007b7fc = 0x4B` → `FUN_8005567c` lone-monster cell `[0x4B,0,0,0]`, byte-exact with both `zeto_*_mid_cast` captures). Port: `World::install_boss_encounter` + a `rikuroa` first-visit latch on flag `0x1BE`, victory-latched in `apply_battle_loot` (oracle Part C). **Open:** the `DAT_8007b7fc` writer is in no loaded program (readers only); needs a Mt. Rikuroa write-watchpoint. Spine-writer #1 of 3; runbook `docs/tooling/spine-flag-writers-capture.md`. See [`encounter.md`](../formats/encounter.md#scripted-battle-id-path-fun_8005567c). | `project_re_wave_5_plan.md` |
| Spine flag `0x142` (dolk-dungeon-clear) writer | open (capture-gated; both bytecode censuses negative) | Spine-writer #2 of 3. Flag `0x142` routes `map01`'s dungeon entrance (`dolk` clear / `dolk2` set; mechanism + port resolved, see the `dolk`→`dolk2` row) but the *writer* is in no loaded program - readers only - and the motion-VM census (`--motion-flag-census`) is ALSO negative, so it is a direct code path, not scene bytecode. Closes with a PCSX-Redux write-watchpoint capture at the dolk-clear beat. Runbook `docs/tooling/spine-flag-writers-capture.md`. | `project_re_wave_5_plan.md` |
| Spine flag `0x482` (Drake mist-wall) writer | open (capture-gated; both disc-bytecode censuses negative) | Spine-writer #3 of 3. Flag `0x482` gates `map01`'s mist walls (`P2[34..36]` C1); it reads `0` across the whole save library incl. both pre-victory Zeto captures, `rikuroa` sets it nowhere, and the motion-VM census finds no op-7/op-8 site either - a direct code path. Closes with a fresh PCSX-Redux write-watchpoint capture at the post-mist story event. Runbook `docs/tooling/spine-flag-writers-capture.md`; see the `Gate-flag setters outside the MAN field VM` row for the `0x482` census. | `project_re_wave_5_plan.md` |
...
| Post-FMV "per-FMV event record" (teleport + flags) | falsified (as a table) | The debug-menu STR path observably restores a scene and the beats' flags change, but the FMV dispatch record is playback-only (`legaia_asset::fmv_dispatch` - no scene/position/flag fields), the dev-menu renderer `FUN_801EAD98` is display-only, and no dumped code writes `_DAT_8007BA78` outside the three known field-VM/title sites. Whatever applies the post-STR teleport+flags is un-dumped code (the scene-restore label list at `0x801CAE40`-adjacent `0x801CE8AC` is the restore half); do not re-walk the "per-FMV event table" shape. | `project_re_wave_7_plan.md` |
| Spawned-record player-channel (`0xF8`) ExecMove/HaltAcquire handshake | open (engine gap) | A door-cutscene P2 record that drives the PLAYER channel (`A2 F8 06` ExecMove + `C3 F8 ...` HaltAcquire) never completes in the engine: `field_channels::resolve_target` returns `None` for the special `0xF8` target, so no completion bit is signalled and the timeline spins until the frame cap force-completes it WITHOUT executing the trailing `0x3F` - blocking driven hops like `jou`→`jouina` (pinned by `chapter1_hub_depth_oracle.rs`; the hop is decode-asserted + direct-load verified instead). Closes with a player-channel completion model in the timeline stepper. | `project_re_wave_7_plan.md` |
