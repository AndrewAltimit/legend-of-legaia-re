...
| Monster stat-record archive source | open | The monster-record **format** is pinned from `FUN_80054CB0` (name ptr `@0x00`, XP/drop `@0x04`, HP `@0x0C`, MP `@0x10`, stat u16s `@0x0E/0x12/0x14/0x16/0x18/0x1A`, magic count `@0x4A`, spell-ptr array `@0x4C`); the loader `FUN_800542C8` streams per-monster `0x14000` LZS blocks at offset `(id-1)*0x14000` and applies offset→pointer fixups. But the **source bytes** aren't located: retail builds `data\battle\<8-byte name of PROT 869 monster_data>` (the name comes from the per-entry filename table `DAT_8007B31C + idx*8`) and opens it via the `break 0x103` host-PC trap (`FUN_800608F0`) — a dev-host artifact with no matching ISO file on the retail disc; and PROT.DAT entry 869 @ `0x047E7000` is not a valid `(id-1)*0x14000` archive (slot 0 LZS-decodes to all-zeros — the false-positive trap). Closing it needs a runtime watchpoint on the monster-record load during a live battle (capture the CD-DMA source LBA, same technique as the title-overlay / collision-grid source pins), since the archive is likely an unindexed disc region. Encounter formations already resolve monster-ids (`town01`/`town0c` = {4,7,10,61,62,63,79}); only the stat blocks are missing. | `project_monster_stat_archive.md` |
