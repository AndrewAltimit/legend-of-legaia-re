...
| `0x19` fallback approach drive - what idles it in the caught park | open (mechanism + fix settled; this is the last sub-question) | Healthy fallback melee approaches during `0x19` (~19 units/vsync, Move-clip playback side, `autorun_gaza2_summon_displacement.lua` capture); the caught park had anim bytes idle (`0/0`) and zero movement from the first poll. Close it by catching another park with an extended hunter recording `+0x1DA`/`+0x1D9`/the strike queue at park time, or by pinning the driver that consumes the staged clip and what clears it (reaction override / summon-staging aftermath are the candidates). |
