...
| Equipment stat-bonus table — `+0`/`+4` stat labels + UI slot model | partial (attack/def/mask/slot-category pinned) | The equipment stat-bonus table (`DAT_80074F68`, 8-byte stride, indexed by the item property `+1` byte) is decoded from `FUN_801CF650` and parsed as `legaia_asset::equip_stats` — see [`equipment-table.md`](../formats/equipment-table.md). `+1` attack / `+2`/`+3` def-up/down / `+6` equip mask / `+7` slot category + Ra-Seru bit are byte-exact vs gamedata. **Open:** (a) the `+0` (head-gear) and `+4` (footwear) bonus bytes are the agility/evasion pair but the exact stat-order (which is `acc` vs `eva`) isn't confirmed from in-repo ground truth — the equip-screen accumulator order or a savestate stat-diff would close it; (b) the `+7` byte only distinguishes 4 slot categories (weapon/body/head/footwear), so it can't drive the engine's 8-slot model (helmet vs ring vs accessory all read as "head") — the helmet/accessory disambiguation source is unpinned. | `project_equipment_table.md` |
