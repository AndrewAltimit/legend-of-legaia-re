...
| Thread | Status | What would close it | Memory |
|---|---|---|---|
| Town/field free-movement locomotion | open | Pin the function that turns the d-pad into player free movement in normal towns (position write to `_DAT_8007c364 + 0x14/0x18`). The `801db81c..801dbf9c` cluster is now **ruled out**: a Ghidra re-decomp (force-disassemble the `jal 0x8003ce9c` holes via `fix_field_locomotion_flow.py`, then re-create functions at real prologues) shows it is the **field camera system**, not movement — `FUN_801db510` = camera follow + smooth-scroll (camera origin `_DAT_80089118/120`, default `= -player[0x14/0x18]`), `FUN_801dbc20` = camera-mode config from a script operand, `FUN_801dbec4` = arrival handler (tile = `(player_pos - 0x40) >> 7`, region lookup, snap camera + facing `+0x16` via `func_0x80019278`). These only *read* `player+0x14/0x18`. The held-pad (`_DAT_8007b850`) movement code in overlay 0897 lives in the field VM (`801de840`) and tile-board walk SM (`801ef2b0`); the prior "`s8` position-write" arithmetic was a decompiler hallucination from the data holes. Remaining candidates for the integrator that *writes* `player+0x14/0x18`: the actor / motion VM (`FUN_8003774C` / `FUN_801D6628`) reading the held pad, or a per-actor update inside the field VM. A runtime write-watchpoint on `_DAT_8007c364 + 0x14` during town walking would pin it directly. | `project_tile_board_grid.md` |
