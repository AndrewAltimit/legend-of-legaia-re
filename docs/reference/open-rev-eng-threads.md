...
| Item-effect table — class/targeting + literal amounts | resolved (descriptor table AND amounts are static disc data) | Class/tier/targeting/usability is the static `DAT_800752C0` table (`FUN_8003043c` / `FUN_80030628`). The restore *amounts* are **also static** — NOT overlay-resident: the apply handler is the **static** `FUN_800402F4` (jump table `0x80014FA0` by class, one fn for field + battle via `game_mode == 0x15`); its HP/MP heal arms read a tier-indexed `u16` table at `0x8007655C` (HP `[200,800,9999]`) / `0x80076564` (MP `[50,200,20]`), deficit-clamped. Tier `3+` HP is character-relative (`0x80084140` Seru-heal tables); revive is `max_hp*0.4 + rand%(max_hp/8)`. Parser `legaia_asset::item_effect` (`heal_amounts` / `restore_amount` → `RestoreAmount`); disc-gated `item_effect_real` pins it and `item_catalog_disc` byte-confirms the curated engine figures. Usability + all-party flags wired (`ItemCatalog::apply_effect_flags`, disc-gated `item_effect_flags_disc`). | `project_item_effect_table.md` |
| Equipment stat-bonus table — slot model | **slot model resolved**; accessory/Goods system is the distinct open thread | The equipment stat-bonus table (`DAT_80074F68`, 8-byte stride, indexed by the item property `+1` byte) is decoded from `FUN_801CF650` and parsed as `legaia_asset::equip_stats` — see [`equipment-table.md`](../formats/equipment-table.md). All five `+0..+4` stat targets are **pinned** by `FUN_801CF5D0` (which pre-loads each accumulator from a named char-record halfword before the equipment bytes are summed): `+0`=INT, `+1`=ATK, `+2`=UDF, `+3`=LDF, `+4`=SPD. **The earlier "agility/evasion pair" reading of `+0`/`+4` is falsified** — equipment never touches AGL; `+0` is INT (head gear), `+4` is SPD (footwear). `+1`/`+2`/`+3` magnitudes + `+6` mask + `+7` slot category + Ra-Seru bit remain byte-exact vs gamedata. **WIRED:** `DiscEquipInfo` gates `EquipSession`'s per-character item list — a Vahn-only weapon is hidden from Noa's picker (disc-gated `disc_equip_restrictions_gate_equip_session_item_list`). **Slot model RESOLVED (the "+7 can't disambiguate" framing was false):** the four `+7` categories *are* Legaia's four armour/weapon slots — a by-name cross-check against gamedata is exact for body (20=20), head (15=15), footwear (16=16); the disc "Head" bucket is precisely the 15 gamedata **helmets** (seals/clips/crowns/bands), NOT a helmet+accessory mix, and **none of the 77 accessories ("Goods") appear in this table at all**. So `+7` was never meant to classify accessories. **Still open (reframed, distinct thread):** where the accessory/Goods records live + how they're equipped — a separate table/system, not a `+7` disambiguation. Pinned by disc-gated `legaia-gamedata::equip_slots_vs_disc`. | `project_equipment_table.md` |
...
**Collision derivation — decomp-pinned, one capture-gated remnant.** `FUN_801cfe4c` is fully decoded (overlay `0897` @ `0x801CE818` + the on-disc bias table `DAT_801f2214`): it tests **three leading-edge footprint probes** (~47 units ahead, ±16 lateral) and derives each sub-cell as `zc = (z>>6)+2`, `xc = ((x+0x3f)>>6)−1`. The engine's [`World::field_tile_is_wall`] uses a single candidate-centre point with floor indexing. Two things are settled: the **quadrant-mask formula is byte-identical** between the two (the "inverted X parity" worry is **false**), but the **sub-cell index derivation differs** — retail indexes one tile further in Z (the `+2`) and the opposite X parity (`ceil−1` vs floor) for the same world point. **Open (capture-gated):** whether that offset reads the wrong grid cell in a live scene depends on how the disc grid is authored relative to the world-tile origin; town01 walks acceptably and the disc test only checks coarse stop-at-a-wall, so it is not yet a proven player-visible bug. Settle with a town01 capture of the live player position vs the `+0x4000` grid byte that blocks it. Do **not** realign the derivation blind. Code-pinned by `world.rs::tests::retail_collision_subcell_indexing_differs_from_engine_by_a_z_tile`.
...
| Options/menu overlay PROT entry | resolved + RAM-verified (**PROT 0899** @ `0x801CE818`) | The real options/pause/inventory-equipment-status menu overlay is **PROT 0899**, not 0896 — `FUN_801CF650`'s instruction signature byte-matches PROT 0899 file `0xe38` (`0x801CE818 + 0xe38 = 0x801CF650`), 101/139 captured menu-dump functions align as prologues, jal-recovery agrees (30 votes). **RAM-verified clean copy:** the `0x15e8c`-byte `.text`+`.rodata` prefix is byte-identical across six menu-open saves (equipment/status/options x field `map01` + town `town01`; scenarios `menu_*_field` / `menu_*_town`). VA-alias sibling of the field overlay (0897) in slot A — the menu overlay REPLACES the field overlay at `0x801CE818` (field VM at `0x801DE840` is not a prologue while `FUN_801CF650` is). The earlier "0896 = menu" label is falsified. | `project_static_overlay_pipeline.md` |
...
| Slot-A scene-overlay family beyond field/battle/menu | resolved (in the static map) | The rest of the slot-A (`0x801CE818`) VA-alias family is now pinned from the disc: **0970 cutscene_str** = the STR/MDEC FMV overlay (modes 26/27; decisive `MV*.STR` movie paths + `MDEC_in_sync`/`MDEC_out_sync` decoder strings — the long-capture-blocked F-FMV overlay, now Ghidra-importable straight from the disc), and the **minigame** overlays **0972 fishing / 0973 slot_machine / 0976 baka_fighter / 0980 dance**, each cross-checked by a documented minigame function landing on a prologue at the base (`anchor_va`). The minigame entries over-read each other (one minigame's code duplicated across consecutive entries at `base + N×0x800`), so jal-recovery can latch a phantom base; the canonical entry recovers `0x801CE818` (slot machine excepted — `0x4000` over-read prefix → `0x801CA818`). Found via `asset overlay scan` + the leading dev string. | `project_static_overlay_pipeline.md` |
...
| Slot-B overlay cluster (`0900..0969`) per-entry identity | mostly resolved | The slot-B buffer (link base `0x801F69D8`) timeshares the summon / dance / minigame blobs of the `0900..0969` cluster; static extraction at the link base is the clean path (a capture catches a mix). Each base is cross-checked by the fraction of internal absolute self-pointers that resolve in-file (`static_overlay::pointer_resolution`; test asserts ≥70 %). **Pinned in the map:** 0900 (summon render), 0905 (Gimard stager), 0902 (GAME OVER), 0907/0924/0927 (Disco King dance songs), 0957 (summon-effect string table — `Puera`/`Damage`/`Recover`; **NOT a dance song**, correcting `overlay-ptr-table`). **Resolved over-broad-range thread:** "summon `0905..=0915`" is the loader's *arithmetic* range, not a stager list — **0907 is the dance song "Hell's Music", not a summon** (it only parsed as a scene-graph via over-read bleed into neighbours' `FUN_80021B04` calls; `summon_overlay::NON_SUMMON_IN_STAGER_RANGE`; block-sweep test asserts its dance identity). **Still open:** the per-summon spell-id → stager-entry assignment for the binary stagers (needs a capture; over-read defeats a static spawn census). | `project_static_overlay_pipeline.md` |
