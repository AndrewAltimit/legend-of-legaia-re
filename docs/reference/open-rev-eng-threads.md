...
1. **The *"It was the Seru."* caption's data source.** The crawl-1 capture pins WHERE it shows - a centered line over the villager-tableau shot, between the creation crawl's last page and the Seru-history crawl's first - but it is confirmed NOT inline in the `opdeene` MAN (the body carries exactly the two crawl blocks' 22 pages and no other text segment), so it is an external (`4C E1`-style) balloon: likely a MES id or a channel-script emission. Closing it = finding the bytes the balloon spawner (`FUN_8003C764` path) is handed during that beat.
