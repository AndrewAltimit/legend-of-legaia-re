...
| Field collision-map source | open | `FUN_801cfe4c` reads a per-scene walkability grid at `*(_DAT_1f8003ec) + 0x4000` (one byte per 128-unit tile, high nibble = 4 sub-cell wall bits). Pin where the scene loader populates `_DAT_1f8003ec` / the `+0x4000` grid — which field-pack slot or MAN section carries the on-disc walkability bytes. A write-watchpoint on the map base during scene load closes it. | `project_field_locomotion_integrator.md` |
