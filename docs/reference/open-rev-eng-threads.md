...
**Residual (open):** NPC motion (the engine's NPCs stand at their anchors, so live position == anchor), the prop walk-touch **event** semantics engine-side (props block but the engine has no prop event scripts to fire), and the interaction-end teardown (who clears the player's `0x80000` engaged flag and restores the actor facing from `+0x5A`).
