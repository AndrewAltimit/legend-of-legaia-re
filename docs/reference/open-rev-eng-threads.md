...
| Chapter-2 opener flag `0x1BE` (Jeremi/`geremi` arrival) | resolved (writer decoded; consumer engine-side) | `0x1BE` (446) is SET unconditionally at the head of `geremi` `P2[0]`, the Jeremi-arrival cutscene record (`51 BE`, then the `0x527..0x52E` narration-scratch clears; embedded dialog "…monsters in the Mist!" / "it's a Genesis Tree!"), and CLEARED in `geremi` `P1[1]`. It has **zero Test sites disc-wide** in field-VM space (`man-scripts --system-flag-census`) - a write-only progress marker read by engine/overlay code, matching the poll (flips once on `geremi` entry, tick 28605). Corrects the old "`0x1BE` = rikuroa Zeto gate" (a CDNAME-frame-skew artefact). **Open:** the engine-side reader of bit `0x1BE`. | `project_re_wave_8_plan.md` |
