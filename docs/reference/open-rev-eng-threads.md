...
| Tetsu 4-option spar menu mechanism | partial - mechanism CRACKED, engine wiring open | **Live-cracked** (`autorun_tetsu_confirm.lua` replays `s5_tetsu_battle` to the menu): a **dialog-SM inline-picker menu**, not the field VM (`FUN_801DE840` never fires while it is up; the actor parks at script PC `0x155`). The **cursor is the inline-picker cursor `*(0x801C6EA4)+0x0C`**; **confirming index 2** (the 3rd entry) with CROSS starts the spar (mode `0x03 -> 0x09 -> 0x15`, live). So the cursor/dispatch ARE the inline-picker path the engine models (`step_inline_dialogue`); the open sub-question is **where town01's 4 option entries live** (`scan_pickers` finds none in its MAN). Engine wiring = present the menu + arm the carrier only on index 2. | - |
