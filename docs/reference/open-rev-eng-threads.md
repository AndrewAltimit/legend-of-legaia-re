...
Render wiring therefore reuses the summon parser + move VM. The `model_sel` additive base `gp[0x754]` — only *read* in the corpus — is now **live-captured = 3 in battle**: a PCSX-Redux exec-bp on `FUN_80021B04` during a battle move-FX spawn (probe `autorun_summon_model_base`) confirmed the full `FUN_801e09f8 → FUN_80050ed4 → FUN_80021B04` chain (`ra = 0x80050F08`, `a3 = 0x1000`, prototype table `0x801F6324` + effect-list id `0x22` live in registers) and read `gp+0x754` (global `0x8007BA6C`) = 3, so a battle move-FX mesh is `DAT_8007C018[model_sel + 3]` — placing `model_sel` in the inferred `DAT_8007C018[3..=32]` window. The engine surfaces every record field as a resolved `MoveFx` (`engine-core::move_power::MovePowerCatalog::fx_for_move_id`).
