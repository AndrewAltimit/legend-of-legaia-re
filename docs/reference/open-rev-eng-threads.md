...
**Residual (open):** the static prop arm (bit `4` — no engine prop-collision list), NPC motion (the engine's NPCs stand at their anchors, so live position == anchor), the touch side-effects (`FUN_801d5b5c` event post on the `+0x98` link; face-the-NPC turn), and the third facing-indexed probe table `DAT_801f2254` (one pair per 8-direction facing, extents `0x20`).
