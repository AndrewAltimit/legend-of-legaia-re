...
| Monster mesh + texture pool | mesh resolved; pool open | The monster's 3D battle model is a [Legaia TMD](../formats/tmd.md) embedded in each PROT 867 archive block at the offset in stat record `+0x04` (installed at battle-actor `+0x230`; the `0x1C`-stride records `FUN_80049858`/`FUN_800495C8` walk are its object table). **186/194 slots parse cleanly.** Parser `legaia_asset::monster_archive::mesh`; CLI `--obj` export; WASM `LegaiaViewer::monster_mesh_*` accessors drive the enemy-table site page's per-row WebGL viewer (untextured / directional-lit). **Open:** the texture/CLUT pool at record `+0x08` — internal layout (CLUT count, 4bpp page extents) and runtime VRAM placement (the on-disc CBA/TSB are nominal defaults the loader relocates per battle slot, same wall as the [battle-data pack](../formats/battle-data-pack.md) descriptor). Next step: byte-match the decoded pool against a captured battle VRAM from a **catalogued** save (`saves/library/<backup_fingerprint>.sstate`, e.g. `rim_elm_gimard_victory` / `rim_elm_queen_bee_battle`), not a live mc slot. | `project_monster_mesh_source.md` |
