...
Render wiring therefore reuses the summon parser + move VM; the lone residual is `gp[0x754]`, the `model_sel` additive base — only *read* in the corpus (the same scalar the summon thread waits on), so one read-watch pins both. The engine already surfaces every record field as a resolved `MoveFx` (`engine-core::move_power::MovePowerCatalog::fx_for_move_id`).
