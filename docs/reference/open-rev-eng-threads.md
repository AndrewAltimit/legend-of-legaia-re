...
| Random-encounter trigger path | open | The script-VM install opcodes describe **scripted** encounters. Random encounters gated by the rate counter at `_DAT_8007B5F8` may bypass `actor[+0x94]` entirely — `0085_map01`'s 46 indexed scripts contain zero install opcodes despite map01 having random encounters in play. Open: find the function that turns a successful rate roll into a populated formation cell. | `project_encounter_record_format.md` |
