...
| Gate-flag setters outside the MAN field VM (`549`) | `549` open (op-7 carrier falsified); `0x482` moved to its own resolved row | `549` (`0x225`, town01 opening one-shot): the bank RAM-diff pin stands, but `--motion-flag-census` finds **no** stream carrying `549` and the carrier-complete field-VM census has no SET either - the setter is a direct code path (`FUN_8003CE08`-class, overlay-resident); a targeted capture at the opening beat is the closer. `0x482`'s writers are now pinned as `other7`-pool script bytes (see the spine `0x482` row). | `project_re_wave_7_plan.md` |
