...
| Item-effect table — class/targeting vs. literal amounts | partial (descriptor table pinned; amounts overlay-resident) | The effect *class + tier + targeting + field/battle usability* is the static `DAT_800752C0` table (id → subtype → `[class, tier, flags, 'A']`), decoded from `FUN_8003043c` / `FUN_80030628` and parsed as `legaia_asset::item_effect` — see [`item-effect-table.md`](../formats/item-effect-table.md). What is **not** on disc is the literal restore *amount*: `(class, tier) → 200/800/...` is a `switch` in the overlay-resident item-use *apply* handler (the field-menu "Use" path in the menu overlay + the battle-action item path), which is not in the dumped corpus. **Closes when:** that apply handler is dumped (add it to `dump_funcs.py` / a per-overlay dumper) and the `(class,tier)`-keyed amounts are read from its immediates or a per-overlay data table. Until then the engine keeps curated amounts. The usability + all-party (`0x20`) flags are **wired**: `ItemCatalog::apply_effect_flags` installs them and the item-use session fans party-wide heals (Bloom/Fruit) across living allies (disc-gated `item_effect_flags_disc`). | `project_item_effect_table.md` |
