...
| First boss (Zeto) trigger → Battle | formation + engine port RESOLVED; trigger writer capture-gated | Zeto (id `0x4B`, in `rikuroa`) has **no** on-disc formation record; it uses the **battle-id path** (`DAT_8007b7fc = 0x4B` → `FUN_8005567c` lone-monster cell). Port: `World::install_boss_encounter` + a `rikuroa` first-visit latch on `0x1BE`, victory-latched. **Open:** the writer hasn't fired in any firehose capture; with the segment-pool carrier identified (see the `0x142` row), census the pools for a battle-id staging op FIRST - capture only if negative. Spine-writer #1 of 3; runbook `docs/tooling/spine-flag-writers-capture.md`. | `project_re_wave_7_plan.md` |
| Spine flag `0x142` (dolk-dungeon-clear) writer | writer PINNED (live capture + disc carrier); pool-census port open | Spine-writer #2 of 3, CAUGHT: firehose-caught at the **rikuroa post-Caruban beat** ("dolk clear" was a mislabel), `ra 0x801E3598` = the field-VM dispatcher's own `0x5x` SET arm. "Runtime-computed" is falsified: it is script bytes `51 42` in the scene's **segment-pool blob** (`[u16 size][u16 0x0300]` carrier; rikuroa = PROT 0157); no earlier census walked this carrier ([script-vm.md](../subsystems/script-vm.md#a-second-script-byte-carrier-the-standalone-segment-pool-blob)). Save-state bracket catalogued (`rikuroa_pre/post_caruban`). Open: pool-record flag-census walker + an e2e oracle that pool execution flips dolk→dolk2 organically. | `project_re_wave_8_plan.md` |
| Spine flag `0x482` (Drake mist-wall) writer | open (capture-gated; ALL static channels exhausted corpus-wide) | Spine-writer #3 of 3. Flag `0x482` gates `map01`'s mist walls (`P2[34..36]` C1); it reads `0` across the whole save library incl. both pre-victory Zeto captures, `rikuroa` sets it nowhere, the motion-VM census is negative, and the corpus-wide static sweep + framed op-0x49 window census (see the `0x142` row; `0x482` has zero near-misses) are clean. With the segment-pool carrier identified (see the `0x142` row), census the pools for a `54 82` SET FIRST (`74 82` TESTs already surface in `0122_rikuroa2`'s pool); the write-watchpoint capture at the post-mist event stays the fallback. Runbook `docs/tooling/spine-flag-writers-capture.md`. | `project_re_wave_8_plan.md` |
