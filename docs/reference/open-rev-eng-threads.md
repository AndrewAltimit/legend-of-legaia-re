...
| Thread | Status | What would close it |
|---|---|---|
| Enemy magic-band cast attribution (Xain "Bloody Horns"/"Terio Punch" non-elemental) | mostly resolved | The element law is pinned (per-slot; slot 7 = cast-body record `0x801C9358+0x1D`, hardcoded `li a1,7` roll calls in the summon overlays 0902..0934; live-confirmed for a player cast - [battle-formulas.md](../subsystems/battle-formulas.md), [summon-readef.md](../formats/summon-readef.md#actor-record-slot-last-streamed-slot-of-a-group)). Open leg: which attacker slot an **enemy** magic-band cast (BH `0x5C`, TP `0x5D`, Astral Wave `0x6A`) passes. Close by capturing an enemy magic cast at `FUN_801DD0AC` (`LEGAIA_POKE_MAGIC_ID` knob on the element/wait probes) and reading `a1`; 7 would settle why BH/TP match no elemental guard. |
| Gaza 2 magic softlock (community-reported: endless camera orbit after a player magic cast) | open | The orbit is the unconditional idle azimuth sweep (`FUN_801D0748` stepping `_DAT_8007B792`), so the symptom is an action-SM stall in the magic band (`FUN_801E295C` `ctx[7]` `0x28..0x2E` waits: `actor+0x1FA`, `+0x1D9` vs `ctx+0x24C`/`actor+0x21B`, `ctx+0x24D`, `ctx+0x249`). The producers of those done-signals are undumped. Close with (1) `autorun_magic_wait_producers.lua` runs over healthy casts to name each signal's writer PCs, then (2) a capture of the wedge itself on a Gaza 2 save (none in the library yet) - watch `ctx[7]` + the six signals + the yaw per vsync while replaying magic casts. |
