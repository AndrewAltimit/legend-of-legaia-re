...
| `0x19` fallback approach drive - what idles it in the caught park | open (mechanism + fix settled; this is the last sub-question) | Healthy fallback melee approaches during `0x19` (~19 units/vsync, Move-clip playback side); the caught park had anim bytes idle (`0/0`), frozen from the first poll. Out-of-reach is the norm (4/4 healthy melees), so this drive failure is the softlock's sole anomaly. Sharpest untested candidate: **summon-staging aftermath** (the boss is staged to `(0,-2048)` and back; the healthy capture contains zero summon-then-melee sequences). Test: a directed capture recording `+0x1DA`/`+0x1D9`, stalling after summons so the boss melees next; or pin the driver that consumes the staged clip and what clears it. |
