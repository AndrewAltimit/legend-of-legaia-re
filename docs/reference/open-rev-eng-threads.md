...
| Character-record HP/MP/AP pair order (`+0x104..0x110`) is `(max, cur)` - `legaia_save::HpMpSp` labels swapped | resolved (relabel pending) | Three independent sources agree `+0x104/+0x108/+0x10C` = effective maxima, `+0x106/+0x10A/+0x10E` = currents - the reverse of the `HpMpSp` / cheat-classifier labels: (1) walk-regen `FUN_801D0B90` bumps `+0x106`, clamping at `+0x104`; (2) aggregator `FUN_80042558` rewrites `+0x104` per frame from base stats + %-passives (a per-frame recompute cannot be current HP); (3) GameShark "Infinite HP" codes write `+0x106` at every character stride - they pin the *current*. Fresh-save fixtures mask the swap (cur == max at seed). Pending: relabel `HpMpSp` + `cheats::classify` + the `card.rs` round-trip test; re-audit HUD/battle-init consumers. |
...
| First boss trigger -> Battle | partial (both bosses pinned; one vestigial-flag question open) | Mt. Rikuroa's boss is Caruban (id 73 = `0x49`), gated by `0x142` (`SCRIPTED_SCENE_BOSSES`). Zeto is resolved (live firehose from `chapter2_garmel_pre_zeto`): id `0x4B` (byte-confirmed `Zeto`, PROT 867) fights in `garmel` via the `FUN_801DA51C` `actor[+0x94]` record path (formation `[0x4B,0,0,0]`); the `0x8007b7fc` write-watch stayed silent all fight, so it is not the battle-id path, superseding the old `jou` scoping. Open: does any retail battle write `DAT_8007b7fc` (reads `0` everywhere, so it may be vestigial)? Full detail + Caruban re-verify note in [encounter.md](../formats/encounter.md#scripted-battle-id-path-fun_8005567c). |
...
| Spine flag `0x482` (Drake mist-wall) writer | open (capture target; the "other7-pool writers" reading is falsified) | Every census site for `0x482` is a desynced-walker alias of SJIS text; hand-disasm-verified per class, pinned non-clean by the census decode-coherence flag (see [script-vm.md](../subsystems/script-vm.md) § decode-coherence + `man_variant_carrier_census_disc.rs`). Motion/op-0x49 censuses: no site; polls show it only in bulk-load ticks. Writer = a **direct code path** (like 549); next = P7 write-watch across the post-Zeto Drake-revival beat (`chapter2_garmel_pre_zeto`). Reader solid (`map01` P2[34..36] C1 bands). Bonus: `other7` = a dev-leftover earlier koin3 MAN revision (no loader in captured code; see [cdname.md](../formats/cdname.md#numbering-space)). |
