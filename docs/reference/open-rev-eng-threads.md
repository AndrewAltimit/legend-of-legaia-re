...
| First boss trigger -> Battle | mostly resolved (mechanism pinned for both bosses; one vestigial-flag question open) | The scripted-battle arm is the field-VM op `3E FF <formation_row>` ([battle.md](../subsystems/battle.md#scripted-battle-entry-3e-ff-row)). Zeto: garmel `P2[12]` ends `3E FF 09` -> row 9 = lone `0x4B` (firehose-confirmed; `0x8007b7fc` stayed silent, so not the battle-id path); the engine enters it organically (`organic_zeto_encounter_disc.rs`). Caruban: rikuroa stager `P1[3]` ends `3E FF 11` -> row 17 = lone `0x49` - the **same record path**, statically pinned; converting the engine's rikuroa arm from `SCRIPTED_SCENE_BOSSES` to organic `P1[3]` execution is the remaining stand-in. Open: does any retail battle write `DAT_8007b7fc` (reads `0` everywhere; may be vestigial)? |
...
**Nivora Ravine (`nilboa`).** An entry group sharing `0x456`, a `0x47x` puzzle cluster, and a cross-scene successor gated on `0x370`; `nilboa2` is the `0xF`-gated variant carrier. `0x456`'s writer is pinned: `nilboa P2[11]` both SETs and CLEARs it (`Set 0x455` + `Set 0x456` at `+0x37..+0x39`, inside a `CC .. C3` per-actor run). `0x370` stays **writer-less**: its only census SET candidate (`doman` variant `P1[15]`) is the ASCII `Sp` of "Space Bomb" dialogue — a capture watch is still what settles the writer, same as the koin gates. It is not obviously reader-less, though: the doman-variant `P1[3..9]` `73 70` Test sites decode clean and carry no text-alias marker under the census prose classifier — candidate genuine readers worth a record disasm. Anchor `nilboa_nivora_ravine_gate_family`.
