...
| Battle face-stamp issuing site | narrowed (mechanism pinned) | The ~220-byte facial-texel overwrite in the party texture bands is a one-shot battle-init VRAM→VRAM copy of an alternate face frame authored in the same band (upload rects tile with no overlap; zero `MoveImage` traffic in steady-state battle). Same primitive family as move-VM op `0x40`, but no authored op pattern targets the band in the player files — the issuing site is overlay-resident or runtime-built. Close: `autorun_battle_moveimage_trace.lua` across a battle ENTRY (a PCSX overworld state that rolls an encounter, or an exec-bp through the field→battle transition). See `battle-data-pack.md` § Open questions. | `project_battle_face_stamp.md` |
