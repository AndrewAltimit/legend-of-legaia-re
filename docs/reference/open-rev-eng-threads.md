...
| Chapter-2 (Sebucus) progression gate spine | resolved (gate chain + engine-wired) | Mined from the ch2 poll, pinned via C1/C2 record gates (`FUN_8003BDE0`): **teien** `P2[1]`→`0x1C8`→`P2[2]`→`0x1C9`→`P2[5]`→`0x332`; **tower** `P2[2]` (needs `0x1C9`) latches tower-clear `0x1C7`; **geremi** `P2[1]` requires `0x1C7`; **balden** `P2[19]`/`P2[18]` self-latch `0x5B3`; **map02** `P2[9]` = overworld mirror. Low flags `0x8C..0x8F` = tower/teien-LOCAL switch state (no gate). **Engine-wired** with no chapter code (no `SCRIPTED_SCENE_BOSSES` analog: cutscene beats latch via `SysFlag.Set`, unlike chapter-1's post-battle latch): oracle `chapter2_sebucus_spine_oracle`; C1/C2 pinned in `chapter2_sebucus_gate_spine`. | `project_re_wave_8_plan.md` |
