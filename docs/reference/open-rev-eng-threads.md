...
| Equipment stat-bonus table — UI slot model | partial (all `+0..+4` stat targets + mask + slot-category pinned) | The equipment stat-bonus table (`DAT_80074F68`, 8-byte stride, indexed by the item property `+1` byte) is decoded from `FUN_801CF650` and parsed as `legaia_asset::equip_stats` — see [`equipment-table.md`](../formats/equipment-table.md). All five `+0..+4` stat targets are now **pinned** by `FUN_801CF5D0` (which pre-loads each accumulator from a named char-record halfword before the equipment bytes are summed): `+0`=INT, `+1`=ATK, `+2`=UDF, `+3`=LDF, `+4`=SPD. **The earlier "agility/evasion pair" reading of `+0`/`+4` is falsified** — equipment never touches AGL; `+0` is INT (head gear), `+4` is SPD (footwear). `+1`/`+2`/`+3` magnitudes + `+6` mask + `+7` slot category + Ra-Seru bit remain byte-exact vs gamedata. **WIRED:** `DiscEquipInfo` gates `EquipSession`'s per-character item list — a Vahn-only weapon is hidden from Noa's picker (disc-gated `disc_equip_restrictions_gate_equip_session_item_list`). **Open:** the `+7` byte only distinguishes 4 slot categories (weapon/body/head/footwear), so it can't drive the engine's 8-slot model (helmet vs ring vs accessory all read as "head"); the disambiguation source is unpinned, so those slots stay mask-gated only. | `project_equipment_table.md` |
