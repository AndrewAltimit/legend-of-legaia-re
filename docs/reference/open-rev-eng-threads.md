...
| PROT 0874 section-0 outer producer | pinned | The per-scene field initializer `FUN_801D6704` drives the pool fill: `FUN_80020118` (resets `DAT_8007B6F8`=0, loads the party/character meshes via `FUN_8001E890` → `DAT_8007C018[0..4]`) then `FUN_80020224` (walks the scene's main field file - streamed into `_DAT_8007b85c` by `FUN_8001eef0` - dispatching every descriptor through `FUN_8001F05C`; the type-`0x02` packs install into `DAT_8007C018[5..]`). The char-mesh path is `FUN_8001E890` (the PROT 874 §0 producer), the scene-pack path is `FUN_80020224`; both feed `FUN_80026B4C`. |
| Drake uncapped cluster-A totals | resolved | The `LEGAIA_PC_CAP=50000` re-run landed: no PC saturates the cap, so the per-kingdom totals are exact and the cross-kingdom delta table is closed - see [world-map-overlay](../formats/world-map-overlay.md) (kind 18 = the workhorse; Sebucus dispatches ~20.6k instances). |
| Slot-4 freeze flag `_DAT_8007B824` | resolved (static; vestigial) | The corpus has exactly one store to `0x8007B824`: the `FUN_8001F05C` case-2 freeze sub-path (`sw` at PC `0x8001F2F8`, `8001f05c.txt:505`), gated `param_3 == 1`. Every static + overlay caller passes `param_3 != 1` (`FUN_80020224` forwards `s6`, `FUN_8002541C` passes `0`, `overlay_baka_fighter_801d4c50` passes `0`; no dumped `FUN_80020224` caller passes `1`), so the flag stays at its BSS-init zero and the "persistent slots" semantic is vestigial (matches the `world-map-overlay.md` analysis; a static-overlay sweep of any not-yet-dumped overlay would be the only way to find a `param_3==1` caller, but none is expected). Not capture-blocked. |
...
| Thread | Status | What would close it |
|---|---|---|
| Encounter record carrier | resolved (no array) | Decompile of `FUN_801DE840` shows install handlers all use `pbVar43 = param_1 + param_2` (current opcode pointer in the field-VM script bytecode); each scripted encounter is its own dispatcher-op site (cases `0x37`/`0x41`, `0x38`, `0x43`, `0x47`, `0x4C`), with monster count/ids inline as the trailing operand bytes. No separate encounter-record table on disc. See `docs/formats/encounter.md` writer table. |
...
| Seru-magic summon visual (e.g. Tail Fire) | **player visual RESOLVED + WIRED** | [details ↓](#seru-magic-summon-visual-eg-tail-fire) |
...
| Battle face-stamp issuing site | RESOLVED | The facial-texel overwrite is the per-frame **facial animator `FUN_8004C7B4`** (called from the render-node update with the clip's frame cursor; Terra skipped): action-entry facial tracks at `+0x8C` (eyes) / `+0x98` (mouth) select frames from static per-character SCUS tables, stamped by `MoveImage` from band-parked frame strips every frame. Pinned live across a battle entry (`karisto_sol_pre_encounter` + the MoveImage trace probe); the earlier "one-shot at init" reading traced a summon mid-cast window where the animator is paused. Residue: the sibling pass `FUN_8004CCD4`'s trigger states. See `battle-data-pack.md` § Facial animation tracks. |
| Per-spell magic power / multiplier | **mechanism RESOLVED + roll PORTED** | [details ↓](#per-spell-magic-power--multiplier) |
...
| Battle party mesh pack `other5` = **PROT 1204** (battle form; Baka Fighter reuses it) | resolved (empirical) | [details ↓](#battle-party-mesh-pack-other5--prot-1204-battle-form-baka-fighter-reuses-it) |
...
| First boss trigger -> Battle | Caruban = rikuroa boss (`0x142`-gated); Zeto = `garmel` via **record path** | Mt. Rikuroa's boss is **Caruban** (id 73 = `0x49`), gated by `0x142` (`SCRIPTED_SCENE_BOSSES`). **Zeto RESOLVED** (live firehose from `chapter2_garmel_pre_zeto`): id `0x4B` (byte-confirmed `Zeto`, PROT 867) fights in `garmel` via the `FUN_801DA51C` `actor[+0x94]` record path (formation `[0x4B,0,0,0]`); the `0x8007b7fc` write-watch stayed **silent** all fight → NOT the battle-id path, supersedes the old `jou` scoping. **Open:** does any retail battle write `DAT_8007b7fc` (reads `0` everywhere → maybe vestigial)? Full detail + Caruban re-verify note in [encounter.md](../formats/encounter.md#scripted-battle-id-path-fun_8005567c). |
| Spine flag `0x142` (Caruban beat / dolk-dolk2 switch) writer | RESOLVED (disc writers + engine port + oracle) | Spine-writer #2 of 3, CLOSED: plain field-VM `51 42` SETs in the rikuroa **streaming variant MAN** (PROT 0157: P1[10..12] + post-victory P2[50], C1 = `0x142` itself - the self-latching one-shot), re-asserted by dolk2's carrier P1[0..1], cleared by dolk P1[26]. Firehose-caught live (`ra 0x801E3598`); resident heap byte-matches the carrier. The old corpus-negative stood because no census walked the streaming carriers. Census + pins: `man_variant_carrier_census_disc.rs`; engine victory-latch port in `SCRIPTED_SCENE_BOSSES` ([world-map.md](../subsystems/world-map.md)). |
| Spine flag `0x482` (Drake mist-wall) writer | script writers FOUND (static); executing beat open | Spine-writer #3 of 3: SETs are script bytes in the `other7` block's P1[15]/P1[39] (op `0x54`), clears in the `edbalden`/`eddoman` epilogue **variant carriers**. The corpus-negative dissolved with the carrier-complete retail-frame census (`man_variant_carrier_census_disc.rs`). **Open:** which story beat executes the `other7` pool records (an "other"-block shared script pool, not a per-town scene) - needed before the engine opens the walls organically. |
...
| Drake Castle deep interiors (`jouinc`/`jouind`) depth decode | open | The castle chain is driven to `jouinc` (`chapter1_hub_breadth_oracle` part F); `jouinc` (`[43,18,60]`) lists `{jouinb, jouind}` and is otherwise undecoded; `jouind` is untouched. Next depth push: gate census + beat chains for both. |
| `scene_destinations` P1-table scan misses P2-only door names | open (engine hygiene) | The partition-1 destination-table scan under-reports doors carried only by P2 records (jouinb→jouina return door; reverse of the jou `P2[5]` blind spot). The P2 walker + strict portal-site join see them. Fold the P2 pass into `scene_destinations` (the breadth oracle pins the asymmetry and will trip when fixed). |
...
| Chapter-2 opener flag `0x1BE` (Jeremi/`geremi` arrival) | resolved (writer + reader) | `0x1BE` (446) is SET at the head of `geremi` `P2[0]` (the Jeremi-arrival cutscene; `51 BE`, dialog "…monsters in the Mist!"), cleared in `P1[1]`. **Reader** = the record-gate evaluator `FUN_8003BDE0`: `P2[0]`'s C1 one-shot list = `[0x1BE]` - the record that sets it also gates on it, a self-latch (same shape as rikuroa `P2[50]`/`0x142`). A reader-watch caught it live on both geremi entries (`pc 0x8003BF78`, caller `0x801D218C` = field scene-init). The census showed "zero Test sites" because it walks inline `0x70` TESTs, NOT the record-HEADER C1/C2 gate lists (`partition2_record_gates`). Anchor `geremi_p2_0_is_the_0x1be_self_latch`; probe `autorun_flag_reader_watch.lua`. |
| Chapter-2 (Sebucus) progression gate spine | resolved (gate chain + engine-wired) | Pinned via C1/C2 record gates (`FUN_8003BDE0`): **teien** `P2[1]`→`0x1C8`→`P2[2]`→`0x1C9`→`P2[5]`→`0x332`; **tower** `P2[2]` (needs `0x1C9`) latches tower-clear `0x1C7`; **geremi** `P2[1]` requires `0x1C7`; **balden** `P2[19]`/`P2[18]` self-latch `0x5B3`; **map02** `P2[9]` = overworld mirror. Low flags `0x8C..0x8F` = tower/teien-LOCAL switch state (no gate). **Engine-wired**, no chapter code (cutscene beats latch via `SysFlag.Set`, no `SCRIPTED_SCENE_BOSSES` analog): oracle `chapter2_sebucus_spine_oracle`; C1/C2 pinned in `chapter2_sebucus_gate_spine`; **hub-sweep** `chapter2_sebucus_hub_sweep_disc` drives the arc through real `map02` over live `0x3F` hops, organic latch. |
| Chapter-2 dungeon gate families (`taiku`/`doman`/`rayman`) | resolved (disc-static; poll order owed) | From C1/C2 record headers (`FUN_8003BDE0`), not the inline census. **taiku**: self-latch pairs `0x390` (`P2[8]`/`P2[9]`) + `0x38F` (`P2[10]`/`P2[15]`); `0x505..0x519` = LOCAL puzzle switches. **rayman**: `0x1EC` arc-reached fan-out off entry `0x1EB`, plus chain `0x201`→`0x1FB`→`0x200`→`0x1FC`; **rayman2** = same MAN + a shared C1 on low flag `0x7` (variant discriminator). **doman**: `P2[4]` `0x3FB` + `P2[3]` cross-scene `0x6E7`; `taiku2` = 1-record sub-room. **CAVEAT:** the poll never organically walked these - flags surface only via a bulk save-load + map01 mass-clear, so play ORDER awaits a live capture. Anchor `chapter2_dungeon_gate_families`. |
| Chapter-2 `balden2`/`station` gate families + cross-scene links | resolved (disc-static) | **balden** = an arc around `0x1D5` (entry `P2[0]` blocks on `0x1D4`/`0x1D2`; a `0x346` cross-scene successor; the `0x5B3` self-latch `P2[19]`/`P2[18]`). The `0x5B3` beat is the ONE chapter-2 dungeon gate the poll caught ORGANICALLY (`flagset` `scene=balden` @tick 89729); rest disc-static. **balden2** = a sibling carrier: diff body (~53 B) but an IDENTICAL 20-record gate family - no `0x7`-style discriminator (variant picked by streaming slot, not a flag). **Cross-scene:** `balden P2[15]` C1=`[0x3FD,0x359]` gates on the `ropeway2` switch group; `station P2[24]` + `station3 P2[2]` both C2=`[0x38F]` = taiku's area flag. Anchor `chapter2_balden_station_gate_families`. |
| `ropeway2` switch-group flags `0x3FF..0x402` (+`0x359`) | resolved (mechanism + consumer) | A 4-bit switch group in `ropeway2`'s variant MAN (PROT 0339). `P1[10]` group-control if/else (dev "End flag setting"): commit branch SETs `0x3FF/0x400/0x401/0x402` (+clears `0x359`), reset branch CLEARs `0x3FD`+group. Per-switch records own one bit (`P1[44]`/`P1[50]`/`P1[53]`); `0x359` sequences. **Consumer resolved (INTERNAL):** `P2[31..=34]` each C1=`[0x359]` C2=`[0x3FF,0x400,0x401,0x402]` - payoff records spawn only when all four switches set AND `0x359` still clear, then `0x359` commits + latches them shut - a header C2 gate, invisible to the inline census (the earlier "external consumer" note is retracted). Poll order confirmed. Anchor `chapter2_ropeway_jiji_gate_families`. |
| Chapter-2 `ropeway`/`jiji` gate families | resolved (disc-static + POLL-confirmed) | The only chapter-2 dungeons the poll walked ORGANICALLY - every flag below was an in-scene `flagset`, not a bulk-load artifact, so both structure AND play order are confirmed. **ropeway** (sparse): one-shot entry gates `0x1D6` (`P2[5]`/`P2[6]`; the ropeway-reached flag, contiguous with balden's `0x1D5`), `0x321` (`P2[18]`), `0x514` (`P2[30]`, adjacent to taiku's `0x505..0x519`). **ropeway2** shares `0x1D6`/`0x321`, adds a `0x5A8` cross-scene successor (`P2[23]`), and hosts the switch-group consumer (see the row above). **jiji** (small): one-shot pairs `0x305` (`P2[2]`), `0x306` (`P2[3]`/`P2[4]`), `0x3BD` (`P2[6]`/`P2[7]`). Anchor `chapter2_ropeway_jiji_gate_families`. |
| Chapter-2 `dohaty`/`retock`/`stone` gate families | resolved (disc-static) | The remaining `map02`-hub spokes (none poll-walked, so structure only). **dohaty**: a six-record `0xF` first-visit group (`P2[3..=8]`, low-flag variant gate like retockin's `0x7`), a `0x344` one-shot (`P2[10]`), a `0x63D` self-latch pair (`P2[14]`/`P2[15]`). **retock** (rich): progression depends CROSS-SCENE on balden's `0x1D5` - the `P2[13/14/15]` chain (`0x337`/`0x339`/`0x33A`) C2-gates on `0x1D5`, `P2[28]` needs it; a 2nd chain `0x357`→`0x502` (`P2[33]`→`P2[31/32]`). **retockin** = the `0x7`-gated interior variant (~30 records carry `0x7`), SHARING `0x502`/`0x357` with retock + a `0x205` self-latch. **stone** is trivial (one `0x590` one-shot). Anchor `chapter2_dohaty_retock_stone_gate_families`. |
| Chapter-2 `map02` hub gate family | resolved (disc-static) | The Sebucus hub is deliberately sparse - a router (33 partition-1 door/NPC scripts) whose partition-2 layer holds only TWO gated records, both **overworld-mirror one-shots** of a dungeon-arc completion (C1 = the mirror flag the record sets, C2 = the dungeon prerequisite): `P2[9]` C1=`[0x332]` C2=`[0x1C9]` = the teien-arc mirror (also in `chapter2_sebucus_gate_spine`); `P2[10]` C1=`[0x357]` C2=`[0x3AD]` = the retock-area mirror, its `0x357` the SAME flag retock/retockin gate on (`retock P2[33]`, `retockin P2[42]`). Every other chapter-2 progression gate lives in a spoke dungeon's MAN, not the hub. Anchor `chapter2_map02_hub_gate_family`. |
| `town0c`/`town0b`/`town0d` = Rim Elm town-state variants (NOT ch2 spokes) | resolved (reframed) | The chapter-2 poll's `town0c` visit is a Rim Elm (`town01`) REVISIT, not a Sebucus spoke; its "scene" sibling is a non-entity (the poll CSV column header, not a CDNAME map). `town01`/`town0b`/`town0c` share the Rim Elm opening-chain block at `P2[3..=11]` byte-identically (the `0x225`=549 opening one-shot → `0x226` → `0x227` + the `0x231`/`0x232`/`0x233`/`0x141` sub-chains - the 549-reader family; see the `549` row). They are story-state renditions keyed by that chain (no organic set in the ch2 poll = a latched revisit). `town0d` is the fully `0x7`-gated later variant (low-flag discriminator, cf. rayman2/retockin/dohaty). Anchor `town0c_is_a_rim_elm_state_variant_not_a_ch2_spoke`. |
| `nilboa` (Nivora Ravine) gate family | resolved (disc-static) | The first scene of the next region beyond Sebucus (`nilboa` = ニルボア/NIRUBOA = Nivora Ravine, music M111). A first-visit **entry group** `P2[0..=3]` each C1-shares `0x456` with a per-record switch (`0xD`/`0x50C..0x50E`), `P2[4..=6]` the `0xD` successors; a `0x47x` puzzle cluster (`0x475`/`0x476` at `P2[14]`, `P2[18]`/`P2[19]` C2-gate on `0x475`) + a `0x45x` cluster; `P2[32]` C1=`[0x47E]` C2=`[0x370]` cross-scene. Same `0xF` low-flag variant gate as the Sebucus dungeons; `nilboa2` (variant carrier, entry 648) is the fully `0xF`-gated trimmed rendition. NB the chapter-3 hub is `map03` (dests `taiku`/`station3`/`korb3`/`bubu1`/`son`/`deroa`) - no "jankeria" scene exists. Anchor `nilboa_nivora_ravine_gate_family`. |
| `map03` (chapter-3 / Karisto) hub region gate families | resolved (disc-static) | `map03` doors route to `taiku`/`station3`/`korb3`/`bubu1`/`son`/`deroa` (taiku + station3 already mined). **map03** is an even purer router than `map02`: ZERO gated `P2` records. **bubu1** (Buma) has NO field MAN (streamed/special). **bubu2** = a small `0x608`/`0x3D3`/`0x609` requires-all chain. **son** = sparse one-shots (routes back to `taiku`). **deroa** = a `0x46D..0x46F` group + `0x3E1` gate (routes to underground `chitei2`). **korb3** (Karisto castle) is RICH: a nine-record **collection group** `P2[5..=13]`, each C1=`[0x403]` (the "all done" latch) + C2 a distinct `0x43E..=0x45C`, plus a `0x41x` requires-all cluster (`P2[2]`); routes to `korb2`/`kor`. Anchor `map03_karisto_region_gate_families`. |
| Gate-flag setters outside the MAN field VM (`549`) | READER resolved (record-gate); WRITER still open (capture-only) | `549` (`0x225`, Rim Elm opening one-shot). **Reader = the C1/C2 record-gate evaluator `FUN_8003BDE0`** (same as `0x1BE`): read as `P2[3]` C1 (opening one-shot) + `P2[4]` C2 (requires-all) across `town01`/`town0b`/`town0c`/`gameover_data`; never an inline `0x70` TEST, which is why the opcode census showed zero test sites (anchor `flag_549_reader_is_the_rim_elm_p2_gate`; engine mirror `World::p2_gate_flag_set`). **WRITER still open:** no census stream carries `549` and no field-VM SET exists - the setter is a direct code path (`FUN_8003CE08`-class, overlay-resident); a targeted capture at the opening beat is the closer. |
...
| Thread | Status | What would close it |
|---|---|---|
| Dialog font extraction | done - kept for reference | Earlier "blocked on runtime trace" framing was wrong; tile-page lives at VRAM `(896, 0)..(960, 256)`, extracted by `legaia-font::font-extract` from any in-game save state. Listed here only so the older "open" framing doesn't get re-opened. |
