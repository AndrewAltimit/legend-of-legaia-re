...
| Chapter-2 (Sebucus) progression gate spine | resolved (gate chain from the ch2 poll) | Mined from the chapter-2 poll flag beats + pinned via C1/C2 record gates (`partition2_record_gates`, `FUN_8003BDE0`): the **teien** beat sequence `P2[1]` (C1=`[0x1C9]`, sets `0x1C8`) → `P2[2]` (C1=`[0x1C9]` C2=`[0x1C8]`, sets `0x1C9`) → `P2[5]` (C1=`[0x332]` C2=`[0x1C9]`, sets `0x332`); **tower** `P2[2]` (C1=`[0x1C7]` C2=`[0x1C9]`: needs the teien arc, sets tower-clear `0x1C7`); **geremi** `P2[1]` (C2=`[0x1C7]`: requires the tower); **balden** `P2[19]` C1=`[0x5B3]` self-latch / `P2[18]` C2=`[0x5B3]` successor; **map02** `P2[9]` (C1=`[0x332]` C2=`[0x1C9]`) = overworld mirror. Low flags `0x8C..0x8F` = tower/teien-LOCAL switch state (inline actor tests, no gate). Anchor `chapter2_sebucus_gate_spine`; feeds the B-spine chapter-2 engine arc. | `project_re_wave_8_plan.md` |
...
| Gate-flag setters outside the MAN field VM (`549`) | READER resolved (record-gate); WRITER still open (capture-only) | `549` (`0x225`, Rim Elm opening one-shot). **Reader = the C1/C2 record-gate evaluator `FUN_8003BDE0`** (same as `0x1BE`): read disc-wide as `P2[3]` C1 (opening one-shot) + `P2[4]` C2 (post-naming beat requires-all) across `town01`/`town0b`/`town0c`/`gameover_data`; NEVER an inline `0x70` TEST, which is why the opcode census showed zero test sites (`partition2_record_gates`; anchor `flag_549_reader_is_the_rim_elm_p2_gate`; engine mirror `World::p2_gate_flag_set`). **WRITER still open:** `--motion-flag-census` finds no stream carrying `549` and the field-VM census has no SET - the setter is a direct code path (`FUN_8003CE08`-class, overlay-resident); a targeted capture at the opening beat is the closer. | `project_re_wave_7_plan.md` |
