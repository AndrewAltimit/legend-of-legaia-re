...
A handful of entries lead with an ASCII title string before the pointer table. Three are **Disco
King dance-minigame** song-data overlays — `0907_xxx_dat.BIN` "Hell's Music", `0924_xxx_dat.BIN`
"Ultimate Rave", `0927_xxx_dat.BIN` "Dark Eclipse" (each now in the static overlay map, slot-B
base `0x801F69D8`). **`0957_xxx_dat.BIN` is NOT a dance song** (an earlier reading grouped it
here): its head is a summon string table — `Dies` / `Puera` / `Both` / `Damage` / `Recover` (the
summon `Puera` + effect/target labels) — followed by an absolute-pointer table and code. It is
the slot-B `summon_effect_table` overlay. Note `0907` also falls inside the summon loader's
arithmetic range (extraction `903..=913`, the spell-`0x85` slot under the corrected `param +
0x37F` index math) — that range is over-broad; `0907` is the dance song, not a summon.
