...
| [Item-effect descriptor table](item-effect-table.md) | Static `SCUS_942.54` table (`DAT_800752C0`, 130 records, 4-byte stride), indexed item id → subtype → `[class, tier, flags, passive_index]`. Effect class + tier + all-party/field/battle usability flags; literal restore amounts are overlay-resident, not here. |
| [Equipment stat-bonus table](equipment-table.md) | Static `SCUS_942.54` table (`DAT_80074F68`, 8-byte stride), indexed equippable item id → property `+1` byte. Per-equip attack/def bonuses + equip-character mask + slot type + Ra-Seru flag. |
| [Accessory passive-effect table](accessory-passive-table.md) | Accessory ("Goods") passives: 64-slot index space - descriptor `+3` byte (`0x41` = none) / equip `+5` byte (sentinel-only in retail) → bit in the per-character ability bitfield `char+0xF4` (`FUN_80042558`). Name/description/scope table at `0x8007625C`; stat-% magnitudes inline in the aggregator. |
...
| [Player battle files](battle-data-pack.md) | `data\battle\PLAYER1..4` (extraction 863..866 = retail `battle_data` block). Header + LZS `record[0]` (battle-palette chain) + 12-byte `[id, offset, size]` descriptor table + per-slot LZS streams decoding to `[header + Legaia TMD + texture pool]`. |
