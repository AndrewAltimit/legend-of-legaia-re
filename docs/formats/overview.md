...
| [Summon overlay record table](summon-overlay.md) | The per-summon move-VM scene-graph part list embedded as inline data in each Seru-summon code overlay (PROT 905..915; Gimard Tail Fire = 905). Byte-pinned for PROT 905: 19 records of `0x58` bytes at file offset `0x180C`, each `[i16 model_sel][u16 flags][0x54-byte move-VM bytecode]`. |
