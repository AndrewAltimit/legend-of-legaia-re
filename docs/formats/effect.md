...
2. **2D sprite billboards.** The `efect.dat` sprite atlas (entry 873) drives the per-frame billboard emit in `FUN_801E0088` pass 2. The atlas entry layout is confirmed from that consumer: `u8 u, u8 v, u8 w, u8 h, u16 tpage, u8 clut, u8 unk` - the pass-2 code reads `tpage` from atlas byte 4 (`*(puVar3+0xe)`) and `clut` from byte 6 (`*(puVar3+0x16)`), and builds the sprite UV rectangle from `(u, v)..(u+w-1, v+h-1)`. The atlas's `tpage = 0x7680` decodes to VRAM **page (0,0), 8bpp** - a *different* region than `etim` (which textures the 3D models at `fb_x≥320`). It is **not** `etim`, nor the PROT 870 flame atlas (`fb_x≥320`), nor cluster entries 872 / 875 (none byte-match the page-`(0,0)` region in a live battle VRAM). What it *does* overlap is the **live battle framebuffer**: in every battle capture the GPU's `display_fb` / draw-clip sits at `(0,4)`/`(0,244)` (two 320×224 buffers), so page (0,0) — VRAM x `0..64` halfwords, y `0..256` — coincides with a framebuffer, and the bytes there are the rendered scene, not a tidy texture page. So a billboard sampling `0x7680` either reads back the framebuffer (a frame-buffer-effect) or the static atlas's `0x7680` is a default that the spawn path relocates at runtime. Distinguishing these needs a capture of a **2D-billboard effect actually firing** (the `etim`-textured 3D-mesh summons in the corpus don't use the billboard path), which the savestate library doesn't yet have — so this sub-thread is **capture-blocked**, not resolvable from the current captures.
...
- **2D billboard page-(0,0) texel source (capture-blocked).** The 3D-effect-model textures are pinned (`etim.dat`, pixel-verified), as is the PROT 870 flame atlas (battle VRAM `(320,0)`/`(384,0)`/`(448,0)`, byte-verified). Separately, the `efect.dat` 2D sprite atlas samples VRAM **page (0,0), 8bpp** (atlas `tpage = 0x7680`, confirmed via `FUN_801E0088` pass 2). Page (0,0) (x `0..64` halfwords, y `0..256`) **coincides with the live battle framebuffer** (`display_fb`/draw-clip at `(0,4)`/`(0,244)` in every battle capture), and its bytes are the rendered scene — not `etim`, not PROT 870, not entries 872/875. So either the billboards read back the framebuffer or `0x7680` is relocated per spawn; distinguishing the two needs a capture of a 2D-billboard effect actually firing (the corpus only has `etim`-textured 3D-mesh summons). Until such a capture lands, trace the `LoadImage` / spawn site that sets up page (0,0) at the moment a billboard effect fires.
