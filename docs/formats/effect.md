...
1. **3D effect models.** `etim.dat` (entry 874 §2) holds the textures (4bpp TIMs, pages at `fb_x≥320`, `fb_y=256+`, CLUTs in rows `473..478`) for the 3D effect *models* in `etmd.dat` (entry 874 §0). The `etmd` model primitives reference exactly those `etim` CLUT rows. This is confirmed **pixel-exact** against a live battle VRAM dump captured mid-cast (Gimard's *Tail Fire* - a 3D flame mesh): five of the seven `etim` TIM pixel blocks (the fire tiles at `fb(832,256)`, `(852,256)`, `(872,256)`, `(880,384)`, `(880,448)`) byte-match VRAM at their stated targets, and the CLUT rows match. (The two larger `64×256` blocks at `fb(320,256)`/`(384,256)` hold background / character textures in that capture, so they aren't this effect's tiles.) The engine uploads `etim` into the scene VRAM at scene entry (`seed_effect_vram_from_befect_data` in `engine-core`), making these texels resident for effect-model rendering.
   ...
   **Render path (which model is the flame).** Walking the live GPU primitive pool from the same mid-cast capture (decoded with `legaia_mednafen::prim_pool`, filtered to on-screen prims sampling the `etim` page/CLUT) isolates the flame as a tight cluster of ~15 visible **Gouraud-textured** primitives (`POLY_GT3` / `POLY_GT4`) in a ~40×50px screen region, all sampling page `(832,256)` 4bpp + CLUT row 478 (`cba=0x778E`). Everything else on that page is at CLUT row 479 (character textures) - the fire is row 478 specifically. The source mesh is the **smallest of the five `etmd` models** (the 5th, 2 objects / 18 verts / 25 prims): its textured groups are mode `0x3D` (`POLY_GT4`) / `0x2D` (`POLY_FT4`) with `tsb=0x001D@(832,256)` 4bpp + `cba=0x778E@(224,478)` and UVs in `(192..239, 192..239)` - byte-for-byte the live prims' page/CLUT; its untextured groups (modes `0x29`/`0x39`/`0x21`/`0x31`) are the inner Gouraud glow core. The other four `etmd` models are large (200-400 prims) - bigger spell / summon effects. The engine renders this as a textured `legaia_tmd` mesh through the standard VRAM-mesh pipeline (`engine-core::scene::ETMD_TAIL_FIRE_MODEL_INDEX`). The retail effect-id → `etmd`-model selection (driven by the move/art VM) is not yet decoded, so the engine's production spawn path doesn't yet pick a model on its own.
