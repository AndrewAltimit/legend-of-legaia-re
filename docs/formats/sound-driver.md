...
So `bse.dat` leads with `[u16 tag @0][u16 @2 = even byte-offset of the record table][records …]`, and the `>>1 … *2` is a round-to-even of that offset, not a division of the data. The earlier "`+2` is a record-count divisor" reading was imprecise, and so is reading the split as **two sections**: `@2` is `4` in both retail carriers, so the "second section" is simply the record table and there is nothing before it but the header. What follows the table in entry 888 belongs to a different file - see [`bse-dat.md`](bse-dat.md#the-bytes-past-the-table-are-not-this-banks). The pointer indexes the battle-loaded bank, **not** the per-scene `.dpk`.
...
Empirically confirmed against the disc: PROT `0877` (`sound_data2`) walks cleanly as `[type 0: 0x2820][type 1: 0x1BA90][type 2: terminator]` and the type-`2` marker lands inside the file - the same shape `FUN_8001FE70` expects.

`FUN_8001FA88`'s hardcoded `0x37A` load is **not** the `.dpk` and does not tie the filename to this block, though it does sit in it: raw TOC `0x37A` is extraction 888, which is `bse.dat`, and the load is on the **retail** branch. The `.dpk` is the routine's other load, `param_1 + 5`. See [`bse-dat.md`](bse-dat.md#which-entry-it-is-and-when-it-loads).

The difference from the DATA_FIELD walker is **only the terminator**: `FUN_8001FE70` ends on a type-`0x02` chunk (whose `size` is non-zero), whereas `FUN_8002541C` ends on a zero-`size` header - so the DATA_FIELD parser mis-reads a `.dpk`. The walker is `legaia_asset::parse_streaming_with(buf, max, StreamTerminator::TypeTwo)`; the disc-gated `sound_pack_stream_real` test pins the clean type-2 walk of PROT `0877`.
