...
The arm is **data-driven**: rather than blindly raising the bit on entry, `SceneHost::enter_field_scene` walks `opdeene`'s own MAN cutscene-timeline partition for the real `GFLAG_SET 26` write and arms only when it is present ([`World::arm_prologue_handoff_from_man`], built on [`man_field_scripts::walk_partition_gflag_sites`](../../crates/engine-core/src/man_field_scripts.rs)). So the engine confirms the arming op exists in the disc bytecode and sets exactly the bit the executed op would, and a cutscene scene that never issues that write can never produce a false hand-off. The engine doesn't yet tick the partition-2 timeline records frame-by-frame (the camera + actor `MoveTo`s that precede the flag write); that — and the field-VM op that auto-opens the name-entry overlay — are the remaining prologue tail. The disc-gated test `opdeene_prologue_arm.rs` pins the `GFLAG_SET 26` at the partition-2 record-18 offset `0xA5E` and asserts `town01` carries no such arm.
