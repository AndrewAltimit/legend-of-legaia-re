...
**The NEW GAME opening sequence lives in these sub-modes, not in the field/town scene.** Confirming NEW GAME from the menu (`Phase14`) advances `Phase14 → Phase07 (0x801DE134) → Phase15 (0x801DE260) → … → Phase06`, and only `Phase06` writes master-mode 2 (field). The intervening sub-modes are the opening: `Phase15` runs a per-frame text accumulator (`state[+0x7F]` added into a scroll/timer counter) and text-render helpers (`FUN_801E3EE0` / `FUN_801E36C4`) — the scrolling opening **narration** (full-screen text, no scene) — and the **Vahn name-entry** screen is a sibling sub-mode in the same chain before `Phase06`. The new-game data-init (`FUN_80034A6C`, gold/flags/stats) and the opening scene (`town01`) run only after `Phase06`; the narration + name-entry are therefore *not* in any field scene or in the data seed. This chain (`Phase07 → Phase06`) is not captured in any save state (none ever stepped through NEW GAME), so its handlers are statically present but otherwise undocumented.
