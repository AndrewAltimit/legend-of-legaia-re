...
So the C7-REVERB blocker dissolves: there is no per-cue reverb-enable source to trace. To match retail the live engine selects `ReverbMode::StudioC` once at SPU init and routes voices into reverb by default. (Output depth — `vLIN`/`vROUT`, set separately by `SpuSetReverbDepth` — is the one piece not fixed by the preset; the EON mask's exact membership varies per frame with which voices happen to be sounding.)
