...
| `Renderer::set_occlusion_fade` (`--no-occlusion-fade` disables, `D` in `play-window`) | **on** in `play-window` | Camera-occlusion fade: field geometry between the camera and the player dissolves to a screen-door dither in a circle around the character, so the player always reads through walls / roofs / cliffs ([renderer](renderer.md#camera-occlusion-fade-see-through-walls-opt-in-enhancement)). Presentation-only; replays force it off. |
