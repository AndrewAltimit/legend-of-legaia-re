...
- **Composite world / actor system** — `crates/engine-core/src/world.rs`. A `World` struct that owns the actor table, battle ctx, effect pool, and RNG state, and implements every per-VM `Host` trait by routing through it. `World::tick` drives whichever VMs are live for the current `SceneMode` (`Title` / `Field` / `Battle` / `Cutscene`). This is the integration layer engines reuse so they don't have to maintain four parallel VM-state tables.
