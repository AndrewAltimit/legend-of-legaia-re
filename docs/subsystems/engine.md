...
| `World::npcs.animate` (`--no-live-npcs` disables) | **on** in `play-window` + browser play page | Villagers patrol their MAN-authored `0x4C 0x51` routes through the motion VM ([motion-vm](motion-vm.md#field-npc-walking)); off parks them at their placement anchors. An interaction prologue's own walk runs regardless. |
...
| Follow-camera user knobs: drag orbit / drag tilt / wheel zoom (`Camera::manual_orbit` / `manual_tilt` / `manual_zoom`) | identity (retail shot) | The player steers the retail follow camera about the character - swing, tip, pull in or back - on both hosts through one engine vocabulary ([`camera::follow_knobs`](../../crates/engine-core/src/camera.rs)); the character keeps the frame's centre. **Locked while a cutscene owns the camera**: the setters drop the gesture when a timeline is live, so a scripted shot stays where the script put it and never snaps on return. Orbit feeds the movement compass; tilt and zoom are pure framing. Double-click resets all three. |
