...
1. **Overlay helpers not yet captured.** `0x4C nE sub-3/7/8` need `func_0x8003C83C` (actor-table walker) and `_DAT_801C6EA4`-relative camera writes that live in an overlay slot the current capture pipeline doesn't surface. Same story for `nD sub-0/4/5` (16-element u16 OR/AND mask via `func_0x80058104`).
2. **Halt-acquire entanglement.** `0x4C n5 sub-1..=4` (dialog query family) and `n6 sub-0x61` (16-byte halt-acquire) interleave with the STATE_RESUME tristate machinery. They're tractable but porting them requires a small refactor of the existing `field_halt_acquire_*` hook pair, deferred to a follow-up round.
3. **Actor-allocator family.** `0x4C n8 sub-0/1/3/6/0xB/0xD` all call into `func_0x80020DE0` and friends — the actor pool layout is captured in `actor-vm.md` but the high-half allocator has its own conventions that aren't yet matched in the engine port.
...
Three small leaf helpers in the dispatcher's call graph are pure arithmetic — no globals, no overlay calls — so they get clean-room ports in [`crates/engine-vm/src/field_helpers.rs`](../../crates/engine-vm/src/field_helpers.rs) instead of host hooks. The dispatcher arms call into them directly.
...
| `load_u16_le`           | `FUN_8003CE9C`    | `ghidra/scripts/funcs/8003ce9c.txt`      | `0x4C nC sub-5/6`, `nD sub-1` |
| `load_u24_le`           | `FUN_8003CEB8`    | `ghidra/scripts/funcs/8003ceb8.txt`      | `0x4C nE sub-5` (XP add)      |
...
The Rust ports are exhaustively tested (31 tests covering escape sequences, terminator placement, bit ordering, search bounds, and LE byte assembly across short / full-width / over-long buffers). Tests live alongside the ports in `field_helpers.rs`.
