...
| C | 0xC0..0xCF | Small per-actor / per-scene writes (slot table, sub-tile broadcast, sound trigger, `field_74` XOR). Sub-1 is a 1-byte trigger-flag record-array reset: walks `_DAT_80073ED8[..count]` (stride `0xB`), tests each record's 16-bit index via [`party_flag_test`](#helper-functions), writes the inverted bit to `record[0]`; PC always += 2. Sub-0xF is a position broadcast: 4-byte `[4C, 0xCF, b1, b2]` resolves each byte to either the actor's world coord (`0xFF`), the tile-center conversion `b * 0x80 + 0x40` (non-zero), or 0; advances by 4. Sub-9 is a 2-byte global-pair compare gate: PC += 2 unless `_DAT_8007BAB8 != _DAT_8007BA9C`, then halts. |
...
| `party_flag_test`       | `FUN_8003CE64`    | `ghidra/scripts/funcs/8003ce64.txt`      | `0x4C nC sub-1/5/6`    |
...
The Rust ports are exhaustively tested (19 tests covering escape sequences, terminator placement, bit ordering, and search bounds). Tests live alongside the ports in `field_helpers.rs`.
