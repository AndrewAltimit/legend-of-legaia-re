...
**Remaining parity gap - geometric summon animation + spawn driver.** The model geometry is now retail-accurate and the static flame renders with the correct baked row-478 CLUT. What's missing is the **animation**, which is **geometric**, not palette: a capture of two animation-distinct Tail Fire frames shows the entire CLUT band (VRAM rows 470..499) **byte-identical** while the framebuffer differs ~21%, so no per-frame CLUT/CBA cycling occurs (this **falsifies** the earlier "fire flicker = CLUT cycling" reading). The motion comes from the PROT 905 summon overlay **spawning part-actors** (decompiled — see [`battle-action.md`](battle-action.md#inside-a-summon-overlay-prot-905-decoded)): the overlay stages parts via the SCUS part-stager `FUN_80021B04` (`FUN_801F16A0` spawns 8 flame parts in a `rand()`-seeded loop, plus `FUN_801F36A0`/`FUN_801F4DD0`) and the actor system ticks them. **FALSIFIED:** there is **no** move VM here — PROT 905 has zero `jal 0x80023070`, and `FUN_801F69D8` is a *different* overlay aliasing the same load address; the earlier "move-VM scene-graph tweened by `FUN_80023070`/`0x801D829C`" reading is wrong. (The overlay's 3 `LoadImage` CLUT uploads target VRAM row `481+s5`, the character-CLUT region, conditionally - not the flame's row 478, and that region is also byte-identical across the two frames.) So the production spawn is a **per-summon code overlay** (Gimard → PROT 905; dispatched by the battle SM, see [`subsystems/battle-action.md`](battle-action.md#seru-magic-summon-overlay-dispatch)); wiring faithful flame animation means recovering the part records (read at runtime `0x801f8314..0x801f862c`, beyond the extracted file) and porting the actor-based spawn, not animating a CLUT.

This is distinct from the 2D billboard path here: `World::active_effect_sprites` builds billboards from the `efect.dat` atlas, whose `tpage = 0x7680` samples VRAM **page (0,0), 8bpp** (confirmed via `FUN_801E0088` pass 2). That `0x7680` is the atlas entry's **CLUT**, not its tpage — the `+4`/`+6` fields are CLUT (u16) / tpage (byte), the reverse of an earlier reading (the emit at `~0x801E0980` writes `atlas[4..5]` into the primitive's CLUT field and `atlas[6]` into its tpage field). `0x7680` decodes as CBA fb `(0,474)`, an effect-CLUT row, *not* page `(0,0)`. Confirmed from a melee hit-spark battle capture: no prim samples page (0,0)/8bpp/`0x7680`, and the spark draws as textured quads sampling the loaded effect pages (PROT 870 flame atlas `(320,0)`/`(448,0)`, effect-band CLUTs). The engine's `SpriteAtlasEntry` now reads the fields in the correct order, so `active_effect_sprites` yields the real effect page + CLUT and the billboards sample the resident PROT 870 / `etim` texels. The faithful per-frame token cadence (`FUN_801E0088` pass 1 state algebra) is also still inlined-only; the render loops each child's anim batch uniformly over the effect lifetime as a stand-in.
