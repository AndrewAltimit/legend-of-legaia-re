...
**Remaining parity gap - geometric summon animation + spawn driver.** The model geometry is now retail-accurate and the static flame renders with the correct baked row-478 CLUT. What's missing is the **animation**, which is **geometric**, not palette: a capture of two animation-distinct Tail Fire frames shows the entire CLUT band (VRAM rows 470..499) **byte-identical** while the framebuffer differs ~21%, so no per-frame CLUT/CBA cycling occurs (this **falsifies** the earlier "fire flicker = CLUT cycling" reading). The motion comes from the PROT 905 summon overlay's **move-VM scene-graph** - `FUN_801F69D8` stages ~22 part-actors via `FUN_80021B04` and tweens them via `FUN_80023070` / `0x801D829C`. (The overlay's 3 `LoadImage` CLUT uploads target VRAM row `481+s5`, the character-CLUT region, conditionally - not the flame's row 478, and that region is also byte-identical across the two frames.) So the production spawn is a **per-summon code overlay** (Gimard → PROT 905; dispatched by the battle SM, see [`subsystems/battle-action.md`](battle-action.md#seru-magic-summon-overlay-dispatch)); wiring faithful flame animation means porting that move-VM scene-graph (large), not animating a CLUT.
