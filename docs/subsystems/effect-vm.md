...
This is distinct from the 2D billboard path here: `World::active_effect_sprites` builds billboards from the `efect.dat` atlas, whose `tpage = 0x7680` samples VRAM **page (0,0), 8bpp** (confirmed via `FUN_801E0088` pass 2). That page-(0,0) source is *not* `etim`, *not* the PROT 870 flame atlas (both at `fb_x≥320`), and is **capture-blocked**: page (0,0) coincides with the live battle framebuffer (`display_fb` at `(0,4)`/`(0,244)` in every battle capture), so resolving whether the billboards read back the framebuffer or relocate `0x7680` per spawn needs a capture of a 2D-billboard effect actually firing - which the savestate corpus lacks (it only has `etim`-textured 3D-mesh summons). So the 2D billboards still sample empty/framebuffer VRAM (the shader discards zero texels → tinted-outline fallback) until such a capture lands. The faithful per-frame token cadence (`FUN_801E0088` pass 1 state algebra) is also still inlined-only; the render loops each child's anim batch uniformly over the effect lifetime as a stand-in.
