...
**Remaining parity gap - flame flicker + summon spawn driver.** The model geometry is now retail-accurate, but the fire flicker is **CLUT/palette cycling** the PROT 0905 summon overlay uploads each frame (`LoadImage`), which the engine doesn't yet animate. And the production spawn is a **per-summon code overlay** (Gimard → PROT 905; dispatched by the battle SM, see [`subsystems/battle-action.md`](battle-action.md#seru-magic-summon-overlay-dispatch)), not a move/art-VM opcode - so the model is resident and render-correct, but the overlay-driven spawn + animated CLUT remain to be wired.

This is distinct from the 2D billboard path here: `World::active_effect_sprites` builds billboards from the `efect.dat` atlas, whose `tpage = 0x7680` samples VRAM **page (0,0), 8bpp** (confirmed via `FUN_801E0088` pass 2). That `0x7680` is an **authoring placeholder the runtime relocates**, not the sampled page — pinned from a melee hit-spark battle capture: no on-screen prim samples page (0,0) / 8bpp / `0x7680`, and the white hit-spark draws as textured quads sampling the loaded effect-texture pages (the PROT 870 flame atlas at `(320,0)`/`(448,0)`, effect-band CLUTs - pages present only in the impact frame). So the engine still samples the literal `0x7680` (empty/framebuffer → tinted-outline fallback) only because the **per-spawn relocation rule** (atlas entry → loaded effect page) isn't decoded yet; the texel *source* is the effect pages already resident, not a missing page-(0,0) upload. The faithful per-frame token cadence (`FUN_801E0088` pass 1 state algebra) is also still inlined-only; the render loops each child's anim batch uniformly over the effect lifetime as a stand-in.
