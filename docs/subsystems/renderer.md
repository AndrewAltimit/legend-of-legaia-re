...
- **6 placements of one mesh** (pack 74 / obj 347) - dropped by the `Field` pre-pass, whose band exclusion + `upload_all_tims: true` never fill CLUT row 510: all four of its prims sample the **same** texture page `(960, 256)` + CLUT `(64, 510)`.
  ...
  `play-window` already restores the atlas via `legaia_asset::interior_page`; the standalone `SceneResources` pre-pass (and with it the web-viewer full-map path and the VRAM oracles) still builds without it, which is what the placement test pins.

So the colour-block follow-up is done; the remaining town-render item is uploading the system-UI bundle in the pre-pass itself (a VRAM-coverage question - the disc source and semantics are pinned).
The placement test pins the current split (untextured props build a non-empty `ColorMesh`; the missing-CLUT mesh stays empty until the pre-pass grows the bundle upload) as a regression guard.
