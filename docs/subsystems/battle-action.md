...
- **Super Arts are gated on input model, not the matcher.** `SuperMatcher`'s `find` patterns match a queue of *chained named-art* constants (`0x19 <art> ...`), which the saved-chain model never builds - a chain carries only raw directions, not the resolved art-constant queue. So Super detection in the live submenu is blocked on a chained-named-art input model the simplified live loop doesn't have yet (the matcher itself is ported and exercised by `resolve_action_queue`'s tail pass and `battle.rs`'s `commit_turn`).
