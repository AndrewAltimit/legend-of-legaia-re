...
- **Monster reward fields (EXP / gold / drop).** All six stat halfwords are now named (see [actor stat block mapping](#actor-stat-block--monster-record-mapping)), but the rewards are *not* in the record at `+0x04` - that word is the attack-effect / animation data pointer (staged to actor `+0x230`, walked by `FUN_80049858` / `FUN_800495C8`). The victory-spoils path credits party gold at `0x8008459C` and formats the banner at actor `+0xA9` (`"Gained N Experience and M G."`), but that logic is in the un-dumped battle-action FSM (`FUN_801E295C`); pinning the reward record offsets (somewhere in the unexplored `+0x1C..+0x49` head region) needs that function dumped. Drop / steal items are curated in `legaia-gamedata`.
