...
|---|---|---|---|
| PROT 944 | Guilty Cross `0x37` -> **bypass** (dispatcher `+0x1510` sends `0x37` to the `+0x2C` tick; playtest-confirmed - an Ebony Jewel makes no difference); Curse All `0x53` -> `+0xA98` tick with **no damage-wrapper call**; no monster record carries `0x53` and no curated source lists a caster - apparently unused content (cf. the dummied Freeze Thunder `0x2C`) | Cort (humanoid phases) | per-spell (see cells) |
