...
The four town01 NPC TMDs at field intersections sample CLUT row y=479 slots x=128..240 (`CBA = 0x77C8..0x77CF`). An earlier hypothesis was that those palettes lived inside `battle_data` (PROT 865..869) and would land in VRAM via a boot pre-load - the [byte-match corpus](../formats/battle-data-pack.md#whats-not-in-the-pack-the-npc-palettes-at-row-479) refutes that. The actual source is the matching `scene_tmd_stream` entries in *town01's own CDNAME block*, wrapped in type-0x01 chunk headers that `FUN_8001FE70` dispatches during battle init (see [`npc-palette.md`](../formats/npc-palette.md)). Retail field saves carry row 479 = zero because retail field-mode rendering never has those CLUTs resident either.
