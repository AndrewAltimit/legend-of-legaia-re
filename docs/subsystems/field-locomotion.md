...
**Engine model (clean-room).** [`World::field_tile_is_wall`] / [`World::advance_with_collision`] use a **single candidate-centre** point test with **retail's exact sub-cell derivation** (`zc = (z>>6)+2`, `xc = ((x+0x3f)>>6)−1`, quad `(zc&1)<<1|(xc&1)`), stepped incrementally. The derivation is pinned by two cheat-free Rim Elm wall-press captures (scenarios `rimelm_wall_press_left` / `rimelm_wall_press_down`; disc-gated `engine-shell/tests/field_collision_discriminator.rs`):
...
- **Open (fidelity): the three-probe leading-edge footprint.** Retail blocks when any of three body-edge points (~47 ahead, ±16 lateral) hits a wall; the engine tests one candidate centre — a standoff/feel difference (retail rests ~47 units off the wall plane where a centre-point test walks up to it). The indexing itself is now byte-aligned with retail.

Capture note: both wall-press captures park in the **`town0c`** Rim Elm variant whose live session still holds the **`town01`** field buffer — the variant switch does not reload the `.MAP` (the live grid at `*(_DAT_1f8003ec)+0x4000` byte-matches town01's base + paints, located via the scratchpad pointer, `SaveState::scratch_ram`). PROT block 21 (`town0c`) carries its own, different `.MAP`-shaped entry (PROT 0028); which map a *cold* town0c entry loads is unverified.
