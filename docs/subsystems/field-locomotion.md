...
- **The collision grid has exactly one writer — there is no separate base-collision blob or "field-pack projection".** An exhaustive search of every function dump for stores to `_DAT_1f8003ec + … + 0x4000` finds only the field-VM `0x4C` nibble-7 op handler in `FUN_801de840` (the four mutations `| 0xf0`, `& ~(mask<<4)`, `| mask<<4`, `& 0xf` at `_DAT_1f8003ec + col + row*0x80 + 0x4000`). Every other reference is a *read*: the locomotion check `FUN_801cfe4c`, the Y/elevation lookups `FUN_80019278` and `FUN_8003a55c`, etc. Neither the field-pack loader `FUN_8001f7c0` (which only memsets/loads `+0x12000` and `+0x12800`) nor the scene-entry init `FUN_8003aeb0` (which writes the `+0x8000` object map, the `+0x48` height LUT, and the `DAT_80085758` low flag bytes, but never `+0x4000`) projects anything into the grid. So the "walls are runtime-projected from the field-pack preamble" hypothesis is **falsified**: all collision, base and conditional alike, is authored as nibble-7 paint ops in the per-scene field-VM scripts.
- **Therefore the only path to walls is running the scene's field-VM scripts**, which is the **scene-setup execution model** (correct per-scene script entry point + ordering of the records/contexts run at scene load + the right flag state). Caveats found while testing real scripts: (1) `map03`/`map01`/`map02` are world-map-class scenes (not field-pack-formatted; see [`pcsx-redux-automation.md`](../tooling/pcsx-redux-automation.md)); `map03`'s MAN holds only four nibble-7 ops, all behind `0x73`/`0x76` system-flag TEST jumps and *clear* in a real save taken inside `map03` — i.e. story-conditional collision deltas over an otherwise-open walkable field, so an empty base grid for `map03` is plausibly correct. (2) The flag bank that gates them seeds from a save (base `0x80085758`, SC offset `0x1618`; see [`script-vm.md`](script-vm.md#default-case-extension-opcodes---the-fourth-flag-bank)) but that alone paints nothing. (3) Wall-heavy scenes are the towns: `town01` carries 46 nibble-7 ops (in its richest event-script record), but that record halts on the first non-opcode byte when run linearly from its frame-divider, confirming it is not run linearly from offset 0 — the entry point + record/context ordering is the unsolved piece.
