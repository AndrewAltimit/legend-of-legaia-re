...
- **The sub-cell INDEX derivation differs.** Retail computes `zc = (z>>6)+2` and `xc = ((x+0x3f)>>6)−1`; the engine floors both (`z>>6`, `x>>6`). The `+2` applies to *every* sample (not just a look-ahead), so for the same world point retail indexes **one tile further in Z**; in X the `ceil−1` equals the floor everywhere **except exact 64-multiples** (where retail still reads the previous sub-cell). A half-tile-centred player at `(320, 448)` (tile-2/tile-3 centre) maps to retail `(col 2, row 4, quad 2)` but engine `(col 2, row 3, quad 3)`.
- **X alignment proven; standoff validated live (wall-press capture).** A Rim Elm capture with the player holding `X−` pressed against a wall (scenario `rimelm_wall_press_left`) rests at `(1838, 4528)` against the full-height wall column at grid col 13: the leading-edge probe `x−47 = 1791` reads the column's **last** wall sub-cell, one 2-unit step shallower (probe 1793) reads clear — so the 47-unit standoff is step-exact and the X derivation reads the *correct* blocking sub-cell.
  The exact-64 probe (1792) is where retail and floor genuinely diverge (retail wall / floor clear), but the odd resting parity that would expose it is unreachable under 2-unit stepping. The capture's live grid is byte-identical to the engine's (`enter_field_live` base `.MAP` copy + prescript paints). Disc-gated: `engine-shell/tests/field_collision_discriminator.rs`.
- **Open (capture-gated): the Z `+1 row` bias.** An `X−` press against a full-height wall column cannot separate the two derivations (both read the same all-quads wall byte one row apart, and the floor tiers match). The clean discriminator is a **`Z+` (down) wall press**: retail's probe (`z+47`, then the `+2` bias) reads the wall's **near edge row**, where the engine's floor-read lands one row short on a clear cell — so even a thick wall separates the two. (A `Z−` press only discriminates against a 1-row-thin wall: against a thick one the engine reads the wall's interior and both block.) Do **not** realign the derivation before that capture lands.
- **Open (fidelity): the three-probe leading-edge footprint.** retail blocks when any of three body-edge points (~47 ahead, ±16 lateral) hits a wall; the engine tests one candidate centre. This is a standoff/feel difference layered on top of the indexing question — live-confirmed by the wall-press capture: retail rests 46 units off the wall plane where the engine's centre-point test would let the player walk to it.
