...
After the step it also dispatches the **touch side-effects**: result bit `4` posts the touched entity's event (`FUN_801d5b5c` on the `+0x98` partner); with no bit `4` it runs one more facing-indexed probe (a third table `DAT_801f2254`, one pair per 8-direction facing, extents `0x20`) whose bit-`1` hit posts the touch event and turns the player toward the touched actor (`func_0x80019b28` angle into player `+0x26`).
...
- **The actor-collision arm is modelled too.** `World::field_actor_dir_blocked` ports `FUN_801cfc40`'s static-entity arm: the three `DAT_801f21b4` probes box-tested against the MAN-derived NPC anchors (`World::field_npc_positions`) with the 80-unit half-extent, gated behind `World::solid_field_npcs` / `play-window --solid-npcs` — NPCs become solid, resting 142 units short of an anchor head-on (the 64-unit probe + the strict 80-unit box on 2-unit step parity; unit-pinned in `world.rs::tests`). Not modelled: the moving-actor arm (bit `1`), the touch side-effects (event post + face-the-NPC), and the `DAT_801f2254` facing probe.
