...
- **The base collision grid is DMA-loaded from disc; nibble-7 ops are conditional deltas on top.** A runtime Write-watchpoint on the live grid region (`_DAT_1f8003ec + 0x4000`) during a Drake-Castle → Drake-world-map transition caught exactly one writer: the routine at **`0x8005DA50`** (caller `0x8005C2D4`), a **CD-DMA (channel 3) transfer** — `v1` holds the DMA3 CHCR `0x1F8010B8`, `a0 = _DAT_1f8003ec + 0x4000` (the grid base) is the destination, `a1 = 0x10200` is the byte size (covers the `+0x4000` collision grid, the `+0x8000` object map, and into the `+0x12000` field-pack), and `a2` is a CD source LBA. The grid jumped from 2093 to 6805 wall tiles across the transition while only **6** nibble-7 tile-writes fired — proof the bulk arrives as a disc block, not from script paints. So the base walkable collision is **disc data, streamed into the field buffer at scene load**, and the field-VM `0x4C` nibble-7 ops (`| 0xf0`, `& ~(mask<<4)`, `| mask<<4`, `& 0xf` at `_DAT_1f8003ec + col + row*0x80 + 0x4000`, the sole *CPU-store* writer) are conditional modifications layered on afterward. (An earlier draft of this note claimed nibble-7 was the *only* writer and the base was script-authored; that was wrong — it came from a static search for CPU stores to `+0x4000`, which cannot see the DMA-load path. Captured via [`autorun_collision_grid_writers.lua`](../tooling/pcsx-redux-automation.md); `0x8005DA50` is the same CD-DMA copy routine that loads the title overlay raw from disc.)
- **For the engine, base collision is therefore a load step, not a script step**: locate the scene's field-buffer disc block (the `+0x4000` collision sub-region of the ~`0x10200`-byte CD-DMA payload at the `a2` LBA) and load it directly. The nibble-7 ops still matter for story-conditional changes — they ride the scene's field-VM scripts, which run multi-context at load: the same transition fired `FUN_8003aeb0` (scene-entry init) → `FUN_8003ab2c` (MAN system-script prologue runner) and 17 distinct script contexts (two `0xFB` system channels + per-actor `0x06..0x14`), and the 6 nibble-7 paints came from the `0xFB` system context via `FUN_8003ab2c` — validating that path as the conditional-delta painter. `0x8005DA50` / `0x8005C2D4` are not yet dumped in Ghidra (CD-DMA loader); dumping them pins how `a2` (the LBA) is derived from the scene bundle.
- **Collision-grid zero-init / base-load site.** Resolved to the CD-DMA load above (the grid is overwritten wholesale per scene, not incrementally cleared). The scene-boot allocator that sets `_DAT_1f8003ec` and the `0x8005DA50` CD-DMA loader are the load-bearing pair; ruled-out earlier candidates were `FUN_8001f7c0` (loads `+0x12000`/`+0x12800` only), `FUN_8003a024` (100-byte control block), `FUN_800513f0` (battle work buffer).
