...
- **Reaching the nibble-7 paints (the dominant open thread).** Painting walls is not as simple as "run the scene's prescript". Two findings from running real scene scripts through the ported field VM: (1) The nibble-7 paints sit behind `0x73`/`0x76` system-flag TEST conditional jumps. In `map03` the four paints in the MAN scene-entry script (partition[1][0]) are **story-conditional collision deltas** — gated by flags `0x6C2`/`0x6D1`/`0x778` that are *clear* even in a real memory-card save taken inside `map03`. So `map03`'s base walkable grid is **not** these MAN paints; it comes from another source (most likely the field-pack region projection, still unlifted). (2) The flag bank that gates them can be seeded from a save (base `0x80085758`, SC offset `0x1618`; see [`script-vm.md`](script-vm.md#default-case-extension-opcodes---the-fourth-flag-bank)) but seeding alone does not produce walls — the larger gap is the **scene-setup execution model**: the correct per-scene script entry point and the ordering of the records/contexts that run at scene load. Naive linear execution of a record reaches none of the paints (e.g. `town01`'s 46-paint record halts on the first non-opcode byte when run from its frame-divider, confirming that record is not run linearly from offset 0).
