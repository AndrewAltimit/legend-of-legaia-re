...
- **Camera params.** The op-`0x45` events flow to the `Camera` controller, but only the camera-target X/Z params are pinned; eye / distance / pitch and the snap-vs-interpolate commit (`FUN_801DE084`) are not yet disambiguated, so the camera **path** is approximate until a clean field-overlay re-dump (and, ideally, a mid-cutscene save capture) lands. The native `play-window` field renderer still uses its orbit camera; routing the cutscene camera into the window view is the remaining visual wiring.
