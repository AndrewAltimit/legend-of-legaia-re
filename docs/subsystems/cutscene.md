...
- **Camera Configure op `0x45`.** The CONFIGURE sub-path (`op0 & 0xC0 == 0`) reads a big-endian 10-bit field mask `(op0<<8)|op1`; bit `(9−i)` selects param `i`, each a signed-16 LE word written into the camera staging struct at `0x801C6EA8 + 0x02 + i*4`, followed by the commit `FUN_801DE084(struct, apply_trigger)`. The commit (`overlay_cutscene_dialogue_801de084.txt`) maps every param to a camera global:
  ...
  |---|---|---|---|
  | 0 | `+0x02` | `_DAT_8007b790` | rotation / zoom-state component |
  | 1 | `+0x06` | `_DAT_8007b792` | **yaw** (heading) |
  | 2 | `+0x0a` | `_DAT_8007b794` | azimuth (zeroed in the field-camera build path) |
  ...
  **The "eye distance" is not a missing scalar - retail has no explicit eye-distance parameter.** The per-frame camera setup (`FUN_801DB8EC` → `FUN_801DAB90`, recovered from live overlay RAM since the headless Ghidra overlay dump truncates them) builds the view from a *parameter struct* at `0x801F3580`, not a focus + distance: `+0x1a/+0x1e/+0x22` = focus (anchor `+0x14/+0x16/+0x18`, X/Z negated), `+0x02` = param 0 (the "rot / zoom" word), `+0x06` = yaw (param 1), `+0x0e/+0x12/+0x16` = the shake/offset trio (params 3-5), `+0x26` = H (param 9, also pushed to GTE control word 26 via `func_0x8003d254`). `FUN_801DB8EC` sets the focus globals (mode byte `_DAT_8007B607>>4 == 5` → tile-derived `-(tile<<7)-0x40`; otherwise anchor-follow `-(anchor+0x14/+0x18)`) and the H register; `FUN_801DAB90` is mode-keyed on `_DAT_8007B607>>4` (e.g. mode 4 auto-faces the anchor, computing yaw from a `FUN_80019B28` atan2 of the camera→anchor delta) and samples terrain height via `FUN_80019278`. So the camera position is implicit in **(focus, yaw, param 0, H)** + a mode-dependent GTE rotation/translation build, not an orbit radius. Confirmed against the `new_game_cutscene_intro_a` save state: focus `(8640, 0, 10304)` (mode byte `0x10` = anchor-follow), yaw `-2967`, param 0 `180`, H `792`. A negative finding: reading the GTE rotation matrix + translation straight from a save-state frame does **not** recover the camera - the matrix is the last-rendered object's composed transform (row norms ≈ 6.0, a per-object scale), not a unit camera-view rotation. Fully closing the engine's framing means decoding the mode-keyed GTE build that consumes the `0x801F3580` struct (param 0 → pitch/zoom); the snap-vs-interpolate timing of the commit is likewise un-pinned.
...
- **Camera params.** The op-`0x45` events flow to the `Camera` controller and the host writes the param set to `World::camera_state`. The native `play-window` renders the cutscene with [`window::cutscene_camera_mvp`](../../crates/engine-render/src/window.rs) whenever a cutscene timeline is installed, decoding the pinned params (see the op-`0x45` table above): it frames the **focus** `(-param6, param7, -param8)` — sign-corrected back to world space from the negated GTE-translation globals — rotated by the **yaw** (param 1, PSX `4096` = full turn) with the **FOV** derived from param 9 (the GTE H register), via `SceneHost`'s `cutscene_view`. The eye **distance** is the one approximation left: retail has no explicit eye-distance param (the camera is parametrized as focus + yaw + param 0 "rot/zoom" + H — see the op-`0x45` table above), so the engine uses a scene-sized orbit radius rather than the (still-undecoded) mode-keyed GTE rotation/translation build. The shot re-targets each time the timeline executes a new Camera Configure op; rather than cutting, `play-window` eases the rendered `(focus, yaw, FOV)` toward each new beat through [`window::CutsceneCameraInterp`](../../crates/engine-render/src/window.rs) (per-frame ease, yaw along the shortest arc; reset to snap when the timeline first installs), so beats blend the way retail's GTE camera does.
