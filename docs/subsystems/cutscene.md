...
Still open: actually **executing** this timeline in-engine — spawning record 18 as a field-VM context (entry PC `0x10`, ext-target `0xF8` → the camera/lead actor) so the camera path + actor `MoveTo`s play and `GFLAG_SET 26` fires by execution. The narration today rides its own presenter timer rather than the timeline's flow; wiring the op `0x45` params into the camera controller (with the param→register mapping above) is the next step.
