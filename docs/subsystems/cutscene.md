...
The *"It was the Seru."* caption appears between `opdeene`'s two crawls, as a centered line over the villager-tableau shot (pinned by the crawl-1 capture, between the creation crawl's last page and the Seru-history crawl's first). It is **not** inline in the `opdeene` MAN - the body carries exactly the two crawl blocks' pages and no other text segment - so it is an external (`4C E1`-style) balloon whose data source is still unpinned; the engine does not yet render it. Open thread: see [`open-rev-eng-threads.md`](../reference/open-rev-eng-threads.md#new-game-opening-chain--narration-roller).
