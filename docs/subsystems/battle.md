...
**Attack** and **Item** are wired; **Arts / Magic** appear in the menu but aren't selectable yet (those paths live in `battle_session` / `spell_menu` and aren't wired into the live loop). Item resolves to `Resolution::OpenItemMenu` — the command session can't run the inventory picker itself (it needs the live inventory + party stats), so it hands off: `live_battle_tick` opens a battle-context `inventory_use::InventoryUseSession` stored on `World::battle_item_menu` (built by `World::build_battle_item_session` from the live inventory + per-party-slot battle HP/MP; targets are party-only). While the item menu is open both the SM and the command session are parked; `World::tick_battle_item_menu` drives it from `World::input`. On a completed use the chosen item is applied authoritatively via `World::use_item`, one copy is removed (`World::consume_item`), a heal/cure popup is surfaced (`battle_hit_fx`), and the action SM is **parked at `EndOfAction`** so the re-arm block cycles to the next combatant — using an item is the actor's whole turn, no strike fires. Backing out of the item menu reopens the command menu for the same actor. Implementation: [`crates/engine-core::battle_input`](../../crates/engine-core/src/battle_input.rs); coverage `crates/engine-core/tests/battle_player_driven.rs` walks into a battle, asserts no strike lands until the player confirms a command, then drives the picker to a monster wipe + loot.

The `legaia-engine play-window` host exposes both as flags: `--live-loop` walks-and-fights through the round trip, and `--player-battle` (which implies `--live-loop`) makes battles player-driven and renders the party/monster HP plus the live command menu / target cursor / item submenu in the HUD. Without a flag, play-window keeps the legacy "explore but never fight" behaviour.
