...
| 8 | 0x80..0x8F | Large multi-purpose dispatcher (party page mirror, conditional jump on `+0x68`, actor model/anim set, actor-search jumps, …). Full body: [nibble-8 multi-purpose dispatcher](#0x4c-nibble-0x800x8f--large-multi-purpose-dispatcher). |
...
- **3** (round 18, 2-byte camera-anchored teleport: `[4C, 0xE3, actor_id]` syncs the resolved actor's position to the active camera)
