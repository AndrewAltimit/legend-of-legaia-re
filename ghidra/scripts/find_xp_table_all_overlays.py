...
# @runtime Jython
#
# Find every function in EVERY program that reads from the retail XP
# table at 0x8007123C using a combined LUI+ADDIU / LUI+load detection.
#
...
print("[{}] scanned {} instructions; hits={}".format(prog.getName(), total, len(hits)))
for fa, refs in sorted(hits.items(), key=lambda kv: -len(kv[1])):
    ...
    print("  [{}] {} @ 0x{:08X}  reads={}".format(prog.getName(), fname, fa or 0, len(refs)))
