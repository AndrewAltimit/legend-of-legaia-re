...
Disc-patching toolkit for a user-supplied Legend of Legaia disc image. Every
feature is built on the same foundation: same-size in-place sector edits with
...
- **The randomizer** - seeded reassignment of gameplay data: monster item
  ...
  Tactical-Arts button combos, doors (scene-transition, house-script and
  `.MAP` kind-0 map doors), shops, casino prizes, starting items, starting
  level, item prices, equipment passive stat bonuses, weapon specialty, and
...
- CLI: `legaia-patcher monster-block --dump/--write` exposes the same loop for
...
- The apply path (`apply::randomize_equip_masks`) emits a same-size in-place SCUS
  ...
  each character's equip picker. `legaia-patcher equip-bonuses` lists the current
...
| `delilas_cast` | The retail cast route: SCUS queue-hook stub + expect-verified word edits in PROT 958/959/960 ([cast-module.md](../../docs/subsystems/cast-module.md)), plus the un-folded stage caves (staging stores `jal` into SCUS-resident repoint code so the player caster walks the FULL retail clip chain; folded two-row fallback per module when the LZS budget refuses; hosted clips are duration-true at every pose rung and carry the source clips' rescaled loop windows, so a stage holds or cycles like the retail enemy instead of replaying from frame 0). `CastRoutePolicy::Install` wires it; the frontend passes `ArenaTaken` when shiny-Seru / show-super-arts / arts-AP own the SCUS arena, and the route **silently downgrades** to the art-side signature, noted in the apply summary. |
...
legaia-patcher drops --input "Legend of Legaia (USA).bin"
...
legaia-patcher chests --input "Legend of Legaia (USA).bin"
...
legaia-patcher randomize --input DISC.bin --seed myrun --chests shuffle
...
legaia-patcher randomize --input DISC.bin --seed myrun --encounters random \
...
legaia-patcher steals --input DISC.bin

# Read-only: list every scene-transition door/exit (home scene -> destination).
legaia-patcher doors --input DISC.bin

# Read-only: list the intra-town (house) door-warp target tiles per scene.
legaia-patcher house-doors --input DISC.bin

# Shuffle intra-town house doors (per-scene class-preserving warp shuffle).
legaia-patcher randomize --input DISC.bin --seed myrun --house-doors shuffle

# Bidirectional door shuffle (walk back the way you came).
legaia-patcher randomize --input DISC.bin --seed myrun --doors shuffle --door-coupling coupled

# Start the new game with 3 random items instead of the fixed Healing Leaf x5.
legaia-patcher randomize --input DISC.bin --seed myrun --starting-items 3

# Start the new game at character level 10 instead of 1.
legaia-patcher randomize --input DISC.bin --seed myrun --starting-level 10

# Read-only: show the new game's current starting bag + level.
legaia-patcher starting-items --input DISC.bin

# Read-only: list what each town store sells / the casino prize exchange.
legaia-patcher shops  --input DISC.bin
legaia-patcher casino --input DISC.bin

# Shuffle drops, plus a low-chance bonus equipment drop on top of every battle.
legaia-patcher randomize --input DISC.bin --seed gear --drops shuffle --equipment-drops

# Randomize what stores sell (quest items excluded) + the casino prizes.
legaia-patcher randomize --input DISC.bin --seed mart --shops random --casino shuffle
...
```bash
cargo test -p legaia-patcher                                   # synthetic only
LEGAIA_DISC_BIN="/path/to/Legend of Legaia (USA).bin" \
    cargo test -p legaia-patcher                               # + disc-gated
```
