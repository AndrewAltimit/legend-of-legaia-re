...
pub(crate) fn cmd_randomize(args: RandomizeArgs) -> Result<()> {
    ...
    let mut manifest = vec![
        "# legaia-patcher run manifest".to_string(),
    ...
    if args.super_arts_pack {
        ...
        let fix = apply::inject_arts_name_fix(
            &mut patcher,
            legaia_patcher::super_arts_pack::ARENA_USED_END_VA,
        )?;
        println!(
            "  + the author's name-length fix: Super/Miracle Art banner re-centred for \
             the installed name (routine at {:#x})",
            fix.routine_va
        );
        manifest.push(
            "super_arts_pack = true  # by ZetaPhoenix, incl. his name-length fix".to_string(),
        );
    } else {
    ...
    if let Some(chest_mode) = chest_mode {
        ...
        let keep_static: std::collections::BTreeSet<u8> = match &args.keep_static_items {
            None => {
                ...
                legaia_patcher::items::default_static_chest_items(&scus)
            }
            Some(list) => list
                .iter()
    ...
    if let Some(arts_mode) = arts_mode {
        ...
        manifest.push(format!(
            "arts = {:?}",
            match arts_mode {
                legaia_patcher::arts::ArtsMode::Shuffle => "shuffle",
                legaia_patcher::arts::ArtsMode::Random => "random",
            }
        ));
        manifest.push(format!(
    ...
    if let Some(hd_mode) = args.house_doors.mode() {
        ...
        if hd_mode == DropMode::Shuffle {
            ...
            // under the same option - see `legaia_patcher::map_door`.
            let map = &report.map;
            println!(
    ...
    if seed_opts.is_active() {
        ...
        if !overflow.is_empty() {
            let guard = legaia_patcher::starting_bag::DEFAULT_GUARD_BIT;
    ...
    if legaia_patcher::starting_level::is_active(args.starting_level) {
...
pub(crate) fn cmd_verify(
    ...
    let applied =
        legaia_patcher::ppf::apply_ppf3(&mut image, &ppf).context("apply PPF to disc image")?;
