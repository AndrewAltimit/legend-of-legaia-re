//! Disc-gated tests for the **shiny Seru** feature (see `legaia_patcher::shiny_seru`):
...
fn composes_with_enemy_ally() {
    ...
    let mut patcher = DiscPatcher::open(disc).expect("open disc");
    apply::inject_enemy_ally(&mut patcher, legaia_patcher::enemy_ally::DEFAULT_PCT).expect("ally");
    ...
    assert_eq!(
        overlay_word(&ov0898, legaia_patcher::enemy_ally::VICTORY_VA),
        legaia_patcher::enemy_ally::VICTORY_PATCHED,
