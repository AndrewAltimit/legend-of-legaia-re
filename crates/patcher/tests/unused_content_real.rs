...
//! - the unused Evil Bat monster ids ([`legaia_patcher::unused::UNUSED_ENEMY_IDS`])
//!   exist and are byte-identical clones of each other and of the in-use Evil
//!   Bat (id 140);
//! - the unused items ([`legaia_patcher::unused::UNUSED_ITEM_IDS`]): "Something
...
use legaia_patcher::unused::{self, UNUSED_ENEMY_IDS, UNUSED_ITEM_IDS};
...
fn seru_bell_name_injection_names_only_the_accessory() {
    ...
    let str_off = legaia_asset::item_names::file_offset_for_va(
        &base_scus,
        legaia_patcher::item_name::SERU_BELL_STRING_VA,
    )
    .unwrap();
