...
use legaia_patcher::translation::{LanguagePack, import_pack};
...
fn translated_then_randomized_composes_and_stays_valid() {
    ...
    let pool = legaia_patcher::items::valid_item_pool(&scus).expect("item pool");
    ...
    legaia_patcher::apply::randomize_drops(&mut patcher, &pool, seed, DropMode::Shuffle)
        .expect("drops");
    legaia_patcher::apply::randomize_encounters(&mut patcher, seed, DropMode::Shuffle, &[])
        .expect("encounters");
    ...
    let _ = legaia_patcher::apply::current_drops(&check).expect("drops still readable");
...
fn shipped_pack_resumes_into_a_working_pack() {
    ...
    let base = legaia_patcher::translation::export_pack(&patcher).expect("export");
