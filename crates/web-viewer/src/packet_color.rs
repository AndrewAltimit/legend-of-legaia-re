...
//!
//! The WebGL shader gets both halves through one attribute:
//!
//! | byte | meaning |
//! |---|---|
...
//! constant, which the renderer sets to the neutral `0x80` triple, so an
...
pub(crate) const NEUTRAL: u8 = legaia_tmd::legaia_prims::MODULATION_NEUTRAL;

/// `[r, g, b, 255]` per vertex for a purely **textured** mesh - the prim
/// colour words straight off the mesh.
///
/// Returns empty for an empty mesh so the JS side's `flat && flat.length`
/// guard keeps its "no stream, use the constant" path.
pub(crate) fn textured(mesh: &VramMesh) -> Vec<u8> {
...
pub(crate) fn hybrid(mesh: &VramMesh, shading: &VertexShading) -> Vec<u8> {
