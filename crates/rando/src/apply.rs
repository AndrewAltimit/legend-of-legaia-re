...
pub struct DropApplyReport {
    ...
    /// (the original drop is kept). Our greedy LZS packer is weaker than
    /// Sony's, so a record already near the `0x14000` slot limit can overflow;
    /// skipping keeps the rest of the patch valid. See [`crate::monster`].
    pub skipped: Vec<u16>,
}
