...
| `item_name` | `NameInjection`: name the otherwise-blank accessory `0xFD` "Seru Bell" so `--unused-items` hands out a presentable item. A same-size SCUS patch — write the string into reclaimable data-segment tail (`legaia_asset::item_names::data_segment_free_tail`) and repoint only `0xFD`'s `name_ptr_slot`, leaving the other empty-name ids blank. |
