...
| `shop` | Shop-inventory randomizer. `ShopTable::parse` reads the field/menu overlay's shop table (fixed `0x60`-byte blocks of 8-byte `[u16 item_id][u16 flag][u32 price]` records, list terminated by `item_id == 0`); `randomize` reassigns every active slot from the global pool of existing shop entries — `Shuffle` permutes the multiset (each shop keeps its item count), `Random` draws with replacement — moving the whole record so each item keeps its real price (no external price table needed); `write_back` rewrites the table region same-size in place. |
