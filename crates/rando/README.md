...
| `arts` | Tactical-Arts button-combo randomizer. Each combo is a glyph string in the static `SCUS_942.54` arts-name table (`DAT_80075EC4`); two pointers reach it and read the **same bytes** — the record's `+8` (menu display) and its `+0x10` description pointer (the in-battle matcher reads the string after the description). So `ArtsEdits` overwrites the combo's **glyph bytes in place** (same-size 2-byte edits), keeping display and trigger in sync (moving the `+8` pointer desyncs them — an emulator playtest caught it). Identical combos are deduplicated across characters, so the editable unit is the distinct combo string; `plan` permutes the distinct strings' contents within each length class. **Input count is preserved** and each character's combos stay unique by construction (every character's arts map to distinct strings). `Shuffle` reassigns existing same-length combos, `Random` writes fresh same-length combos; the Miracle Art (`0xFF09`) strings are left untouched. |
