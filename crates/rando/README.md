...
| `shop` | Town-merchant shop randomizer (what stores sell). A gold shop's stock is **inline in the scene's field-VM script** (the MAN), opened by field-VM op `0x49` (`STATE_RESUME`, the `_DAT_8007B450` menu-register driver) carrying `[u8 count][count× item_id][ASCII name\0]` (pinned from a live Rim Elm Variety Store capture). `SceneShops::locate` walks each MAN record's script with the field-VM disassembler (skipping `0x1F` dialogue) to reach op `0x49` *in real script flow* and validates the inline record (small count, non-zero ids, printable name) — rejecting non-shop `0x49` uses. `set_id` rewrites an item-id byte (same-size); `repack` recompresses the MAN. Global shuffle/random is in `apply`. |
