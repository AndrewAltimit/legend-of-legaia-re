...
| `item_name` | `NameInjection`: name the otherwise-blank accessory `0xFD` "Seru Bell" so `--unused-items` hands out a presentable item. A same-size SCUS patch — write the string into a reserved runtime-**constant** region (`SERU_BELL_STRING_VA = 0x80079900`, pinned for the US build; **not** the data-segment zero-fill tail, which is `.sbss` scratch the game overwrites at runtime so a string there flickers) and repoint only `0xFD`'s `name_ptr_slot`, leaving the other empty-name ids blank. |
