...
| `arts` | Tactical-Arts button-combo randomizer. The combo lives in the static `SCUS_942.54` arts-name table (`DAT_80075EC4`), where each 20-byte record points at `+8` to a glyph string that is the **sole** representation of the combo (so editing it changes both the menu display and the gameplay trigger — see [`art-data.md`](../../docs/formats/art-data.md)). Because identical combos are deduplicated/shared across characters, `ArtsEdits::plan` reassigns each record's `+8` **pointer** to a different existing combo (a same-size 4-byte SCUS edit), never the shared bytes. **Input count is preserved** (a 4-input art stays 4 inputs) and each character's combos stay unique: reassignment runs within each character's per-length groups — `Shuffle` permutes a character's own same-length combos, `Random` draws each a same-length combo from the global pool. The per-character Miracle Art (record idx 0) is left untouched. |
