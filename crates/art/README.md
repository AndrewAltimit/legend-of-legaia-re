...
- `arts_table::name_field` / `NameField` - a record's own `+0xC` name-pointer target with its measured NUL slack, the coordinate a rename writes into. Pointer-based on purpose: substring search corrupts prefix pairs (`"Hurricane"` inside `"Hurricane Kick"`).
...
- `arts_voice::ArtsVoiceTable::parse_from_scus` - decodes the arts-voice cue tables
  (`FUN_8004C140`): the per-character shout file (`clip_file` = `XA2`/`XA4`/`XA6.XA` for
  Vahn/Noa/Gala) and, per art **action constant**, the candidate voice-channel pool the
  retail cue picks a random member of (range table `0x800781A4`, first/second-half
  candidate tables, `dur` table `0x80077A8C`). The parser reads the first-half variant a
  live battle art goes through, and bounds the second-half walk to each character's packed
  record span (`SECOND_HALF_RECORDS` - the three tables sit back-to-back, so an unbounded
  walk decodes the next character's rows as phantom pools). `CAPTURED_ART_CHANNELS` carries
  the capture-witnessed pick per art (recomp-runtime battles instrumented by
