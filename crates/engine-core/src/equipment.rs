...
/// `0x9000`) out of retail's list-node model, and nothing in the engine
/// produces one - the same substitution [`crate::menu_list_rows`] records as
/// family 2 in its module heading, for the two resolvers over that same id
/// space. Adopting the word here would be a change of representation, not a
/// call insertion, so no host is owed one.
