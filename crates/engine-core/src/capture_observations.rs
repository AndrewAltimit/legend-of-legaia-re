...
///
/// - **libcd directory-record cache** window at [`COMPACT_TABLE_ADDR`]:
///   the `CdSearchFile` per-directory cache of PsyQ `CdlFILE` records
///   (records start at `0x801CAE08`; the historical name-first 24-byte
///   parse at `0x801CAE40` was phase-shifted 8 bytes - see
///   `docs/formats/str-fmv-table.md`). [`legaia_asset::str_fmv_table`]
///   still reads the historical window as a capture-forensics helper.
...
/// - **Post-FMV return-scene labels** at [`MID_GAME_LABELS_ADDR`]: seven
///   CDNAME-shape strings (`town0b`, `map01`, `chitei2`, `map02`, `jou`,
///   `uru2`, `town0e`). The master dispatch (`FUN_801CEA3C`) copies the
///   label for the just-played mid-game `fmv_id` into the next-scene
///   name global `0x80084548` - these are the scenes each FMV *returns*
///   to, not the trigger-scene set (distinct from the `op*` opening /
///   `ed*` ending scenes that the field VM dispatches by name).
///
...
///   libcd directory cache at `0x801CAE08` and the FMV dispatch
///   table at `0x801D0A6C` aren't visible from these saves alone.
...
/// **Findings.**
///
/// 1. **`fmv_id` range extends to `0..=8`** - exactly the nine
///    retail slots of the disc-decoded FMV dispatch table at
///    `0x801D0A6C` (23 slots of 32 bytes, `legaia_asset::fmv_dispatch`;
///    slots `9..=22` are dev files absent from the retail disc). The
///    debug menu reaches all nine retail slots.
...
///    the global pool.
///
/// **Per-scene MV-index lift (PRD §2.7) - closed statically.** These
/// saves were generated by debug-menu trigger paths, NOT by stepping
/// the field VM through a per-scene FMV trigger op; they pin the
/// `(fmv_id, game_mode)` tuple across the full `0..=8` range. The
/// per-scene assignment itself is now disc-sourced: the `0x4C 0xE2`
/// ops live LZS-compressed inside each trigger scene's MAN, and
/// `man_field_scripts::scene_fmv_triggers` (pinned by the
/// `scene_fmv_triggers_disc` test) recovers the literal `fmv_id`
/// operands for all eight trigger scenes (`town01`, `garmel`,
/// `deroa`, `chitei2`, `dohaty`, `town0d`, `uru`, `jouine`).
