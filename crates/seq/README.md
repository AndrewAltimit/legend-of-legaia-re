...
the SEQ starts at a non-zero offset. `legaia_engine_core::scene_assets::SceneAssets`
carries the resolved locations (`seq_in_stream_entries`, plus `bgm_seq_offset`
for a given BGM id) so callers slice past the wrapper.
