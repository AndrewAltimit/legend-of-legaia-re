...
| [`vab_bind`](src/vab_bind.rs) | `VabBank::upload(spu, alloc, report, buf)` drops every VAG body into SPU RAM; `VabBank::play_note(spu, voice, prog, note, velocity)` translates a MIDI key into voice config + key-on (the sequencer's key-range path); `VabBank::play_tone(spu, voice, prog, tone_index, note, velocity)` keys an **explicit** tone-region index (the SFX path). |
