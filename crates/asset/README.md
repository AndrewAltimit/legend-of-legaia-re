...
| `mesh_raster` | Software rasteriser for a posed `VramMesh` (textured through PSX VRAM, depth-buffered, framed to fit, optional principal-axis auto-orient) - the site's per-item card thumbnails and the disc-gated contact sheets, where a GPU context is unavailable or unwanted. |
