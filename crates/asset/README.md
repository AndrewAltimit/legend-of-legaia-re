...
| `summon_overlay` | Seru-summon scene-graph record table embedded in a summon code overlay (PROT 905..915; Gimard Tail Fire = 905). `parse_gimard` / `parse_at` / `locate_table_offset` recover the part list (PROT 905: 19 records of `0x58` bytes at `0x180C`, each `[i16 model_sel][u16 flags][0x54-byte move-VM bytecode]`; `-1` = transform node). CLI `asset summon-overlay <PROT>.BIN`. See [`summon-overlay.md`](../../docs/formats/summon-overlay.md). |
