...
- [Structural detectors (for `categorize`)](#structural-detectors-for-categorize)
  ...
  - [Boot / title / menu UI](#boot--title--menu-ui) - `init_pak`, `title_pak`, `menu_glyph_atlas`, `menu_windows`, `save_icon`, `ui_widgets`
...
| `scene_vdf` | Scene bundle type-7 slot: the vertex-morph delta pack (`[u32 count][u32 offsets]` + morph-record sub-entries) the morph stager `FUN_8001C604` consumes. 61 bundles populated. See [`docs/subsystems/field-ambient-fx.md`](../../docs/subsystems/field-ambient-fx.md). |
...
| `interior_page` | The shared interior texture page: a 256×256 4bpp TIM at image `(960,256)` whose declared CLUT block uploads as a flat 256-entry strip on **row 510**. Member 8 of the raw-TOC-entry-0 TIM-pack (PROT.DAT byte `0x11218` - the head region extraction indexing skips), read straight off `PROT.DAT` (`read_from_prot_dat` / `upload_to_vram`). Sampled by town env meshes (town01 incl. the spawn plaza). |
| `system_ui_bundle` | The whole boot-resident **system-UI TIM bundle** = the `prot::timpack`s at raw PROT TOC entries 0/1 (menu-glyph atlas, sprite sheets, cursor parts, six bare atlas row-patch blocks, the raw-entry-1 row-480 UI page). `upload_to_vram` reproduces the retail `FUN_800198E0` boot upload: pack-member order, image at declared rect, CLUT as a flattened `(x, y, w*h, 1)` strip. Layered under every engine scene build via `SceneResources` `BuildOptions::system_ui`. See [`tim-pack.md`](../../docs/formats/tim-pack.md). |
| `player_anm` | Frame-stream ANM container parser (per-(bone,frame) 8-byte entries; frame 0 of idle = rest pose). Decodes the per-scene **NPC / scene-actor** bundles (each scene bundle's type-0x05 "MOVE" section; an NPC's record = its MAN placement `anim_id - 1`), the PROT 0874 §1 party locomotion bundle, and the battle-form banks at PROT 1203. CLI `asset player-anm` / `player-anm-scan`. |
...
| `ringside_still` | The two headerless 16bpp stills (extraction 1221 / 1222): `0x28000` bytes of raw BGR555 uploaded as four 320x64 bands to VRAM `(384, 0)`. The rectangle is immediates in `FUN_801F6B24`, not a field, so the module carries the layout the file does not state. See [`ringside-still.md`](../../docs/formats/ringside-still.md). |
...
| `ui_widgets` | The **widget-class table** (`SCUS_942.54` VA `0x800732A4`, `0x0C` stride, `0x9D` records) the whole 2-D chrome draws itself out of, plus the frame tile-set pool behind it. Per record: layout class, tile-set, signed chain delta, packed CLUT byte, sheet rect and seat bias. A `screen_elements` record's `+0x0E` kind byte is an index into it. See [`battle.md`](../../docs/subsystems/battle.md#the-widget-class-table---where-every-chrome-sprite-comes-from). |
...
| `widget_script` | The **window widget scripts** the window-script VM (`FUN_801D6628`, `legaia_engine_vm::run`) interprets: fixed 4-byte `[opcode][window id][operand u16]` programs resident in the menu overlay's own data segment. `parse_at` validates one program; `scan` recovers every program from the image's `jal` sites + `a0` materialisations. See [`docs/formats/window-script.md`](../../docs/formats/window-script.md). |
...
| `spell_names` | `SCUS_942.54` spell table (`DAT_800754C8`/`DAT_800754D0`, 256 ids swept over a 190-record table): `SpellNameTable::from_scus` → `name(id)` / `mp(id)`. Resolves a monster's global magic-attack ids (`MonsterRecord::magic_attacks`, record `+0x21..=+0x23`) into the on-screen spell name (`0x27` → `Tail Fire`). `SpellEntry` also carries the `+0`/`+1` **class / sub-class** pair the battle-action commit stamps into `actor[+0x1E8]`, which selects a cast's cue group. `name_field` is one row's writable name extent - the record's own `+8` pointer plus measured NUL slack, mirroring `legaia_art::arts_table::name_field`. CLI `asset spell-names <SCUS>`. See [`spell-table.md`](../../docs/formats/spell-table.md). |
