...
| `interior_page` | The shared interior texture page: a 256×256 4bpp TIM at image `(960,256)` whose declared CLUT block uploads as a flat 256-entry strip on **row 510**. Its only raw copy lives in PROT.DAT's unindexed head gap (byte `0x11218`, before entry 0's data), so it is read straight off `PROT.DAT` (`read_from_prot_dat` / `upload_to_vram`). Sampled by town env meshes (town01 incl. the spawn plaza). |
