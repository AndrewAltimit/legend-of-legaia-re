...
| `move_power` | Battle-action **per-move power table** (runtime VA `0x801F4F5C`), the 26-byte-stride table `FUN_801dd0ac` reads for the arts/physical attacker roll. Static battle-overlay data, pinned in PROT 0898 at a fixed raw-entry offset (byte-matched against the in-RAM table across two battle save states). `parse(bytes)` → per-move records; `MoveRecord::power()` = the `+0` field `>> 2` (the roll modulus the kernel derives). Only `+0` is decoded; the rest of each record + the move-id mapping are open. CLI `asset move-power <PROT 0898 .BIN>`. See [`spell-table.md`](../../docs/formats/spell-table.md). |
